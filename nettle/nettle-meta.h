...
/* Ciphers */
typedef void (*nettle_set_key_func)(void *ctx, unsigned length,
                                    const uint8_t *key);
...
#define _NETTLE_CIPHER(name, NAME, keysize)                                    \
  {                                                                            \
#name #keysize, sizeof(struct name##_ctx), NAME##_BLOCK_SIZE, keysize / 8, \
        (nettle_set_key_func)name##_set_key,                                   \
        (nettle_set_key_func)name##_set_key,                                   \
        (nettle_crypt_func)name##_encrypt, (nettle_crypt_func)name##_decrypt,  \
  }
...
#define _NETTLE_CIPHER_FIX(name, NAME, keysize)                                \
  {                                                                            \
#name, sizeof(struct name##_ctx), NAME##_BLOCK_SIZE, keysize / 8,          \
        (nettle_set_key_func)name##_set_key,                                   \
        (nettle_set_key_func)name##_set_key,                                   \
        (nettle_crypt_func)name##_encrypt, (nettle_crypt_func)name##_decrypt,  \
  }
...
typedef void (*nettle_hash_update_func)(void *ctx, unsigned length,
                                        const uint8_t *src);
typedef void (*nettle_hash_digest_func)(void *ctx, unsigned length,
                                        uint8_t *dst);
...
#define _NETTLE_HASH(name, NAME)                                               \
  {                                                                            \
#name, sizeof(struct name##_ctx), NAME##_DIGEST_SIZE, NAME##_DATA_SIZE,    \
...
typedef unsigned (*nettle_armor_encode_update_func)(void *ctx, uint8_t *dst,
                                                    unsigned src_length,
                                                    const uint8_t *src);

typedef unsigned (*nettle_armor_encode_final_func)(void *ctx, uint8_t *dst);

typedef int (*nettle_armor_decode_update_func)(void *ctx, unsigned *dst_length,
                                               uint8_t *dst,
                                               unsigned src_length,
                                               const uint8_t *src);
...
#define _NETTLE_ARMOR(name, NAME)                                              \
  {                                                                            \
#name, sizeof(struct name##_encode_ctx), sizeof(struct name##_decode_ctx), \
...
#define _NETTLE_ARMOR_0(name, NAME)                                            \
  {                                                                            \
#name, 0, sizeof(struct name##_decode_ctx), NAME##_ENCODE_FINAL_LENGTH,    \
        (nettle_armor_init_func)name##_encode_init,                            \
        (nettle_armor_length_func)name##_encode_length,                        \
        (nettle_armor_encode_update_func)name##_encode_update,                 \
        (nettle_armor_encode_final_func)name##_encode_final,                   \
