...
TClntOptIA_PD::TClntOptIA_PD(SPtr<TAddrIA> addrPD, TMsg * parent)
    : TOptIA_PD(addrPD->getIAID(), addrPD->getT1(), addrPD->getT2(), parent), Unicast(false), Iface(-1) {
    ...
    while (ptrPrefix = addrPD->getPrefix()) {
        SubOptions.append(new TOptIAPrefix(ptrPrefix->get(), ptrPrefix->getLength(), zeroTimes ? 0 : ptrPrefix->getPref(),
...
TClntOptIA_PD::TClntOptIA_PD(SPtr<TClntCfgPD> cfgPD, TMsg * parent)
    : TOptIA_PD(cfgPD->getIAID(), cfgPD->getT1(), cfgPD->getT2(), parent), Unicast(false), Iface(-1) {
    cfgPD->firstPrefix();
 ...
 * @param parent
 */
TClntOptIA_PD::TClntOptIA_PD(char * buf, int bufsize, TMsg * parent) : TOptIA_PD(buf, bufsize, parent), Unicast(false) {
    ...
    while (prefix = this->getPrefix()) {
        ...
        case TClntIfaceMgr::PREFIX_MODIFY_ADD:
            ClntAddrMgr().addPrefix(this->DUID, this->Prefix, cfgIface->getName(), this->Iface, IAID_, T1_, T2_,
                                    prefix->getPrefix(), prefix->getPref(), prefix->getValid(), prefix->getPrefixLength(),
                                    false);
            status = ClntIfaceMgr().addPrefix(this->Iface, prefix->getPrefix(), prefix->getPrefixLength(), prefix->getPref(),
                                              prefix->getValid(), scriptParams);
            Log(Debug) << "RENEW(IA_PD) will be sent (T1) after " << T1_ << ", REBIND (T2) after " << T2_ << " seconds."
                       << LogEnd;
            action = "addition";
            break;
        case TClntIfaceMgr::PREFIX_MODIFY_UPDATE:
            ClntAddrMgr().updatePrefix(this->DUID, this->Prefix, cfgIface->getName(), this->Iface, IAID_, T1_, T2_,
                                       prefix->getPrefix(), prefix->getPref(), prefix->getValid(), prefix->getPrefixLength(),
                                       false);
            status = ClntIfaceMgr().updatePrefix(this->Iface, prefix->getPrefix(), prefix->getPrefixLength(), prefix->getPref(),
                                                 prefix->getValid(), scriptParams);
            Log(Debug) << "RENEW(IA_PD) will be sent (T1) after " << T1_ << ", REBIND (T2) after " << T2_ << " seconds."
                       << LogEnd;
            action = "update";
        ...
        case TClntIfaceMgr::PREFIX_MODIFY_DEL:
            ...
            status = ClntIfaceMgr().delPrefix(this->Iface, prefix->getPrefix(), prefix->getPrefixLength(), scriptParams);
            action = "delete";
            break;
        }
...
bool TClntOptIA_PD::isValid() const {
    ...
    for (TOptList::const_iterator it = opts.begin(); it != opts.end(); ++it) {
        ...
        if (prefix && prefix->getPrefix()->linkLocal()) {
            Log(Warning) << "Prefix " << prefix->getPrefix()->getPlain() << " used in IA_PD (IAID=" << IAID_
...
void TClntOptIA_PD::setState(EState state) {
    ...
    if (!cfgPD) {
        Log(Error) << "Unable to find PD with iaid=" << getIAID() << " on the " << cfgIface->getFullName()
                   << " interface (CfgMgr)." << LogEnd;
        return;
    }
