 ...
 * @param parent
 */
TClntOptTA::TClntOptTA(char * buf, int bufsize, TMsg * parent) : TOptTA(buf, bufsize, parent), Iface(-1) {
...
bool TClntOptTA::doDuties() {
    ...
    if (!ta) {
        ...
        ta = new TAddrIA(cfgIface->getName(), this->Iface, IATYPE_TA, SPtr<TIPv6Addr>() /*if unicast, then this->Addr*/,
                         duid->getDUID(), DHCPV6_INFINITY, DHCPV6_INFINITY, this->getIAID());
    ...
    while (optAddr = this->getAddr()) {
        ...
        if (!addr) { // - no such address in DB -
            ...
            Log(Notice) << "Temp. address " << *optAddr->getAddr() << " has been added to " << ptrIface->getName() << "/"
