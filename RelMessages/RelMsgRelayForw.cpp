    ...
    : TRelMsg(iface, addr, 0, 0)  // 0,0 - avoid decoding anything
{
  ...
  if (this->HopCount >= HOP_COUNT_LIMIT) {
    Log(Warning) << "RelayForw with hopLimit " << this->HopCount << " received (max. allowed is "
                 << HOP_COUNT_LIMIT << ". Message dropped." << LogEnd;
    return;
  }

  this->LinkAddr = new TIPv6Addr(data + 2, false);
  this->PeerAddr = new TIPv6Addr(data + 18, false);
  Log(Debug) << "Relaying RELAY_FORW with link:" << LinkAddr->getPlain()
             << ", peer:" << PeerAddr->getPlain() << " and  hop count:" << this->HopCount << "."
             << LogEnd;
  data += 34;
  dataLen -= 34;
...
bool TRelMsgRelayForw::check() { return true; }

std::string TRelMsgRelayForw::getName() const { return "RELAY_FORW"; }
