...
TRelMsg::TRelMsg(int iface, SPtr<TIPv6Addr> addr, char * data, int dataLen) : TMsg(iface, addr, data, dataLen), DestIface(0) {
...
void TRelMsg::decodeOpts(char * buf, int bufSize) {
    ...
    while (pos < bufSize) {
        ...
        if (!allowOptInOpt(this->MsgType, 0, code)) {
            Log(Warning) << "Option " << code << " can't be present in message (type=" << MsgType << ") directly. Ignoring."
                         << LogEnd;
            pos += length;
            continue;
        }
