...
SPtr<TDUID> TAddrClient::getDUID() { return DUID_; }
 ...
 */
void TAddrClient::firstIA() { IAsLst.first(); }
 ...
 * @return next IA
 */
SPtr<TAddrIA> TAddrClient::getIA() { return IAsLst.get(); }
...
void TAddrClient::addIA(SPtr<TAddrIA> ia) {
  if (getIA(ia->getIAID())) {
    Log(Debug) << "Unable to add IA (iaid=" << ia->getIAID() << "), such IA already exists."
               << LogEnd;
    return;
  }
  IAsLst.append(ia);
 ...
 */
int TAddrClient::countIA() { return IAsLst.count(); }
...
SPtr<TAddrIA> TAddrClient::getPD() { return PDLst.get(); }
...
void TAddrClient::firstPD() { PDLst.first(); }

void TAddrClient::addPD(SPtr<TAddrIA> pd) { PDLst.append(pd); }

int TAddrClient::countPD() { return PDLst.count(); }
...
SPtr<TAddrIA> TAddrClient::getTA() { return TALst.get(); }
...
void TAddrClient::firstTA() { TALst.first(); }

void TAddrClient::addTA(SPtr<TAddrIA> ia) { TALst.append(ia); }

int TAddrClient::countTA() { return TALst.count(); }
...
unsigned long TAddrClient::getT1Timeout() {
  ...
  PDLst.first();
  while (ptr = PDLst.get()) {
    if (ptr->getState() != STATE_CONFIGURED) continue;
    if (ts > ptr->getT1Timeout()) ts = ptr->getT1Timeout();
  }
  return ts;
}
...
unsigned long TAddrClient::getLastTimestamp() {
  ...
  IAsLst.first();
  while (ptr = IAsLst.get()) {
    if (ts < ptr->getTimestamp()) ts = ptr->getTimestamp();
  }
...
uint32_t TAddrClient::getSPI() { return SPI_; }

void TAddrClient::setSPI(uint32_t val) { SPI_ = val; }

uint64_t TAddrClient::getReplayDetectionRcvd() { return ReplayDetectionRcvd_; }

void TAddrClient::setReplayDetectionRcvd(uint64_t val) { ReplayDetectionRcvd_ = val; }
...
std::ostream &operator<<(std::ostream &strum, TAddrClient &x) {
  ...
  if (x.DUID_->getLen()) strum << "    " << *x.DUID_;
  if (x.DUID_->getLen() == 1) strum << "  <!-- 1-byte length DUID. DECLINED-ADDRESSES -->" << endl;
