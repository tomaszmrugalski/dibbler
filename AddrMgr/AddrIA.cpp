 ...
 *
 */
TAddrIA::TAddrIA(const std::string & ifacename, int ifindex, TIAType type, SPtr<TIPv6Addr> addr, SPtr<TDUID> duid,
                 unsigned long t1, unsigned long t2, unsigned long id)
    : IAID(id), T1(t1), T2(t2), State(STATE_NOTCONFIGURED), Tentative(ADDRSTATUS_UNKNOWN), Timestamp((unsigned long)time(NULL)),
...
enum EAddrStatus TAddrIA::getTentative() {
    ...
    while (ptrAddr = AddrLst.get()) {
        ...
        case ADDRSTATUS_YES:
            Log(Warning) << "DAD failed. Address " << ptrAddr->get()->getPlain() << " was detected as tentative." << LogEnd;
        ...
        case ADDRSTATUS_UNKNOWN:
            if (ptrAddr->getTimestamp() + DADTIMEOUT < localts) {
                ...
                case LOWLEVEL_TENTATIVE_NO:
                    ...
                    Log(Debug) << "DAD finished successfully. Address " << ptrAddr->get()->getPlain() << " is not tentative."
                               << LogEnd;
                    break;
                default:
...
std::ostream & operator<<(std::ostream & strum, TAddrIA & x) {
    ...
    strum << " IAID=\"" << x.IAID << "\""
          << " state=\"" << StateToString(x.State) << "\" ifacename=\"" << x.Iface_ << "\" iface=\"" << x.Ifindex_ << "\""
          << ">" << endl;
