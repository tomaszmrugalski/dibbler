...
SPtr<TAddrIA> TAddrMgr::parseAddrIA(const char * xmlFile, FILE * f, int t1,int t2,int iaid,int iface, SPtr<TIPv6Addr> unicast)
{
    ...
    while (!feof(f)) {
        ...
        if (strstr(buf,"duid")) {
                  ...
                  /// @todo: Why are we always set this to CONFIRMME? What is the address/prefix
                  /// is still valid and does not require changes?
...
SPtr<TAddrPrefix> TAddrMgr::parseAddrPrefix(const char * xmlFile, char * buf, bool pd)
{
    ...
    if (strstr(buf, "<AddrAddr") || strstr(buf, "<AddrPrefix")) {
        uint32_t timestamp = 0, pref = 0, valid = 0, length = 0;
        addr = 0;
        if ((x=strstr(buf,"timestamp"))) {
            timestamp = atoi(x+11);
        }
