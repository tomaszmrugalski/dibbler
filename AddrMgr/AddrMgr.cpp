        ...
        if(strstr(buf,"<AddrIA ")){
            ...
            if (ia = parseAddrIA(f, t1, t2, iaid, iface)) {
                if (ia->countAddr()) {
                    ...
                    Log(Debug) << "IA (iaid=" << iaid << ") skipped as all addresses are no longer valid." << LogEnd;
                    continue;
                }
        }
        }
        ...
        if (strstr(buf,"<AddrPrefix")) {
            ...
            if (ptrpd && pr) {
                if (verifyPrefix(pr->get())) {
                    ...
                    Log(Debug) << "Prefix " << pr->get()->getPlain() << " does no longer match current configuration. Lease dropped." << LogEnd;
                }
            }
        }
        if (strstr(buf,"</AddrPD>"))
