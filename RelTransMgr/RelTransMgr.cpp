...
TRelTransMgr::TRelTransMgr(const std::string &xmlFile) : XmlFile(xmlFile), IsDone(false) {
  ...
  if (cfgIface->getServerMulticast() || srvUnicast) {
    ...
    if (!addr) {
      Log(Warning) << "No global address defined on the " << iface->getFullName() << " interface."
                   << " Trying to bind link local address, but expect troubles "
                      "with relaying."
                   << LogEnd;
...
void TRelTransMgr::relayMsg(SPtr<TRelMsg> msg) {
  ...
  addr = iface->getGlobalAddr();
  if (!addr) {
    Log(Warning) << "Interface " << iface->getFullName() << " does not have global address."
                 << LogEnd;
  ...
  if (RelCfgMgr().getInterfaceIDOrder() == REL_IFACE_ID_ORDER_NONE) {
    Log(Warning) << "Interface-id option not added (interface-id-order omit used in "
                    "relay.conf). "
  ...
  if (remoteID) {
    ...
    Log(Debug) << "Appended RemoteID with " << remoteID->getVendorDataLen()
               << "-byte long data (option length=" << remoteID->getSize() << ")." << LogEnd;
  }
  ...
  if (RelCfgMgr().getClientLinkLayerAddress()) {
    ...
    if (lladdr) {
      Log(Debug) << "Appended client link-layer address option with " << lladdr->getSize()
                 << " bytes." << LogEnd;
  ...
  if (echo) {
    ...
    int i = 0;
    char tmpBuf[256];
    for (i = 0; i < 255; i++) tmpBuf[i] = 255 - i;
  ...
  while (cfgIface = RelCfgMgr().getIface()) {
    if (cfgIface->getServerUnicast()) {
      ...
      if (!RelIfaceMgr().send(cfgIface->getID(), buf, offset, cfgIface->getServerUnicast(),
                              DHCPSERVER_PORT)) {
    ...
    if (cfgIface->getServerMulticast()) {
      ...
      Log(Notice) << "Relaying encapsulated " << msg->getName() << " message on the "
                  << cfgIface->getFullName() << " interface to multicast (" << addr->getPlain()
                  << ") address, port " << DHCPSERVER_PORT << "." << LogEnd;
...
void TRelTransMgr::relayMsgRepl(SPtr<TRelMsg> msg) {
  ...
  if (!cfgIface) {
    Log(Error) << "Unable to relay message: Invalid interfaceID value:" << msg->getDestIface()
               << LogEnd;
    return;
  }
  ...
  if (!iface) {
    Log(Warning) << "Unable to find interface with interfaceID=" << msg->getDestIface() << LogEnd;
    return;
  }
  ...
  Log(Notice) << "Relaying decapsulated " << msg->getName() << " message on the "
              << iface->getFullName() << " interface to the " << addr->getPlain() << ", port "
              << port << "." << LogEnd;
...
SPtr<TOpt> TRelTransMgr::getLinkAddrFromDuid(SPtr<TOpt> duid_opt) {
  if (!duid_opt) return TOptPtr();  // NULL

  // We need at least option header and duid type
  if (duid_opt->getSize() < 6) return TOptPtr();  // NULL
  ...
  if (len + 4u != duid_opt->getSize()) return TOptPtr();  // NULL
  ...
  switch (duid_type) {
    case DUID_TYPE_LLT: {
      ...
      return SPtr<TOpt>(new TOptGeneric(OPTION_CLIENT_LINKLAYER_ADDR, (const char *)&linkaddr[0],
                                        linkaddr.size(), NULL));
    }
    case DUID_TYPE_LL: {
...
SPtr<TOpt> TRelTransMgr::getLinkAddrFromSrcAddr(SPtr<TRelMsg> msg) {
  ...
  if (!srcAddr || !srcAddr->linkLocal()) return TOptPtr();  // NULL
  ...
  return SPtr<TOpt>(new TOptGeneric(OPTION_CLIENT_LINKLAYER_ADDR, (char *)&mac[0], 8, NULL));
}
...
void TRelTransMgr::shutdown() { IsDone = true; }

bool TRelTransMgr::isDone() { return this->IsDone; }

bool TRelTransMgr::doDuties() { return false; }

char *TRelTransMgr::getCtrlAddr() { return this->ctrlAddr; }

int TRelTransMgr::getCtrlIface() { return this->ctrlIface; }
...
TRelTransMgr::~TRelTransMgr() { Log(Debug) << "RelTransMgr cleanup." << LogEnd; }

void TRelTransMgr::instanceCreate(const std::string &xmlFile) {
  if (Instance) Log(Crit) << "RelTransMgr instance already created. Application error!" << LogEnd;
  Instance = new TRelTransMgr(xmlFile);
}

TRelTransMgr &TRelTransMgr::instance() {
  if (!Instance) {
    Log(Crit) << "RelTransMgr istance not created yet. Application error. "
                 "Emergency shutdown."
              << LogEnd;
    exit(EXIT_FAILURE);
  }
