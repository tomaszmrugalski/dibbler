...
bool TRelTransMgr::openSocket(SPtr<TRelCfgIface> cfgIface) {
    ...
    if (!iface) {
        Log(Crit) << "Unable to find " << cfgIface->getName() << "/" << cfgIface->getID() << " interface in the IfaceMgr."
                  << LogEnd;
        return false;
    }
    ...
    if (cfgIface->getServerMulticast() || srvUnicast) {
        ...
        Log(Notice) << "Creating srv unicast (" << addr->getPlain() << ") socket on the " << iface->getName() << "/"
    ...
    if (cfgIface->getClientMulticast()) {
        ...
        Log(Notice) << "Creating clnt multicast (" << addr->getPlain() << ") socket on the " << iface->getName() << "/"
                    << iface->getID() << " interface." << LogEnd;
        if (!iface->addSocket(addr, DHCPSERVER_PORT, true, false)) {
            Log(Crit) << "Proper socket creation failed." << LogEnd;
            return false;
    ...
    if (clntUnicast) {
        addr = new TIPv6Addr(ALL_DHCP_RELAY_AGENTS_AND_SERVERS, true);
        Log(Notice) << "Creating clnt unicast (" << clntUnicast->getPlain() << ") socket on the " << iface->getName() << "/"
                    << iface->getID() << " interface." << LogEnd;
...
void TRelTransMgr::relayMsg(SPtr<TRelMsg> msg) {
    ...
    if (RelCfgMgr().getClientLinkLayerAddress()) {
        ...
        if (lladdr) {
            Log(Debug) << "Appended client link-layer address option with " << lladdr->getSize() << " bytes." << LogEnd;
    ...
    while (cfgIface = RelCfgMgr().getIface()) {
        ...
        if (cfgIface->getServerMulticast()) {
            ...
            Log(Notice) << "Relaying encapsulated " << msg->getName() << " message on the " << cfgIface->getFullName()
                        << " interface to multicast (" << addr->getPlain() << ") address, port " << DHCPSERVER_PORT << "."
                        << LogEnd;
...
SPtr<TOpt> TRelTransMgr::getLinkAddrFromDuid(SPtr<TOpt> duid_opt) {
    ...
    case DUID_TYPE_LLT: {
        ...
        return SPtr<TOpt>(new TOptGeneric(OPTION_CLIENT_LINKLAYER_ADDR, (const char *)&linkaddr[0], linkaddr.size(), NULL));
    }
    case DUID_TYPE_LL: {
