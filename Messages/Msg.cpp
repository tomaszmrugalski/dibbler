...
TMsg::TMsg(int iface, SPtr<TIPv6Addr> addr, char *&buf, int &bufSize) : NotifyScripts(NULL) {
...
TMsg::TMsg(int iface, SPtr<TIPv6Addr> addr, int msgType) : NotifyScripts(NULL) {
  long tmp = rand() % (255 * 255 * 255);
  setAttribs(iface, addr, msgType, tmp);
}

TMsg::TMsg(int iface, SPtr<TIPv6Addr> addr, int msgType, long transID) : NotifyScripts(NULL) {
...
unsigned long TMsg::getTimeout() { return 0; }

long TMsg::getType() { return MsgType; }

long TMsg::getTransID() { return TransID; }

TOptList &TMsg::getOptLst() { return Options; }
  ...
  switch (auth->getProto()) {
    ...
    case AUTH_PROTO_RECONFIGURE_KEY: {
      if (AuthKey_.size() != RECONFIGURE_KEY_SIZE) {
        Log(Error) << "AUTH: Invalid size of reconfigure-key: expected " << RECONFIGURE_KEY_SIZE
                   << ", actual size " << AuthKey_.size() << LogEnd;
        return;
      }
      if (auth->getAuthDataPtr()) {
        hmac_md5(buffer, len, (char *)&AuthKey_[0], AuthKey_.size(), auth->getAuthDataPtr());
    ...
    case AUTH_PROTO_DIBBLER: {
      ...
      if (getSPI() && !AuthKey_.empty() && UsedDigestType != DIGEST_NONE) {
        ...
        switch (UsedDigestType) {
          case DIGEST_PLAIN:
            memcpy(AuthDigestPtr_, "This is 32-byte plain testkey...",
          ...
          case DIGEST_HMAC_SHA224:
            hmac_sha(buffer, len, (char *)&AuthKey_[0], AuthKey_.size(), (char *)AuthDigestPtr_,
                     224);
            break;
...
void TMsg::firstOption() { NextOpt = Options.begin(); }

int TMsg::countOption() { return Options.size(); }
...
SPtr<TIPv6Addr> TMsg::getRemoteAddr() { return PeerAddr_; }

int TMsg::getIface() { return Iface; }

bool TMsg::isDone() { return IsDone; }
...
TKey TMsg::getAuthKey() { return AuthKey_; }

void TMsg::setSPI(uint32_t val) { SPI_ = val; }

uint32_t TMsg::getSPI() { return SPI_; }

bool TMsg::validateAuthInfo(char *buf, int bufSize, AuthProtocols proto,
  ...
  switch (proto) {
    ...
    case AUTH_PROTO_DELAYED: {
      SPtr<TOptAuthentication> auth = SPtr_cast<TOptAuthentication>(getOption(OPTION_AUTH));
      if (!auth) {
        Log(Warning) << "AUTH: Mandatory AUTH option missing in delayed auth." << LogEnd;
        return false;
      }
      ...
      if (!loadAuthKey()) {
        Log(Warning) << "AUTH: Failed to load delayed auth key with key id=" << std::hex << getSPI()
                     << std::dec << LogEnd;
        return false;
      }
      ...
      if (0 == memcmp(goodAuthInfo, rcvdAuthInfo, DELAYED_AUTH_DIGEST_SIZE)) is_ok = true;
    ...
    case AUTH_PROTO_RECONFIGURE_KEY: {
      if (MsgType != RECONFIGURE_MSG) return true;
      SPtr<TOptAuthentication> auth = SPtr_cast<TOptAuthentication>(getOption(OPTION_AUTH));
      if (!auth) {
      ...
      if (0 == memcmp(goodAuthInfo, rcvdAuthInfo, RECONFIGURE_DIGEST_SIZE)) is_ok = true;
  ...
  if (dt_in_list == false) {
    if (DigestType_ == DIGEST_NONE)
      ...
      Log(Warning) << "Authentication method " << getDigestName(DigestType_) << " not accepted."
                   << LogEnd;
    return false;
  }
  ...
  if (DigestType_ == DIGEST_NONE) {
    ...
    if (0 == memcmp(goodAuthInfo, rcvdAuthInfo, AuthInfoLen)) is_ok = true;

    Log(Debug) << "Auth:Checking using digest method: " << getDigestName(DigestType_) << LogEnd;
    ...
    Log(Error) << "Auth: Digest mode set to " << DigestType_ << ", but AUTH option not set."
               << LogEnd;
    return false;
  }
 ...
 *
 * @param srvIDmandatory - is ServerID option mandatory? (false==ServerID not
 * allowed)
 * @param clntIDmandatory - is ClientID option mandatory?(false==ClientID is
 * optional)
 *
 * @return
 */
bool TMsg::check(bool clntIDmandatory, bool srvIDmandatory) {
  ...
  if (!srvIDmandatory && (srvCnt)) {
    Log(Warning) << "No ServerID option is allowed in the " << this->getName() << " message, but "
                 << srvCnt << " received.";
    status = false;
  }
...
void TMsg::setLocalAddr(SPtr<TIPv6Addr> myaddr) { LocalAddr_ = myaddr; }

SPtr<TIPv6Addr> TMsg::getLocalAddr() { return LocalAddr_; }
