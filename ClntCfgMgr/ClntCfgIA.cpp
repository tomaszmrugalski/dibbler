...
TClntCfgIA::TClntCfgIA()
    : IAID(newID()), T1(CLIENT_DEFAULT_T1), T2(CLIENT_DEFAULT_T2), State(STATE_NOTCONFIGURED), AddrParams(false) {
}
...
TClntCfgIA::TClntCfgIA(SPtr<TClntCfgIA> right, long iaid)
    : IAID(iaid), T1(right->getT1()), T2(right->getT2()), State(STATE_NOTCONFIGURED), ClntCfgAddrLst(right->ClntCfgAddrLst),
      AddrParams(false) {
}
...
ostream & operator<<(ostream & out, TClntCfgIA & ia) {
    out << "        <ia iaid=\"" << ia.IAID << "\" state=\"" << StateToString(ia.State) << "\" t1=\"" << ia.T1 << "\" t2=\""
