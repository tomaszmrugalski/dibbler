...
public:
  ...
  public:
    TOptionStatus()
        : OptionType(0), State(STATE_NOTCONFIGURED), Option(), Always(true){};
    unsigned short OptionType;
    EState State;
    SPtr<TOpt> Option;
    TOpt::EOptionLayout Layout;
    bool Always; // should this option be sent always? (even when already
                 // configured?)
  };
  ...
  void addExtraOption(SPtr<TOpt> extra, TOpt::EOptionLayout layout,
                      bool sendAlways);
