...
bool TClntCfgMgr::parseConfigFile(const std::string &cfgFile) {
  ...
  parser.CfgMgr = this;  // just a workaround to access CfgMgr while still being
                         // in constructor
...
bool TClntCfgMgr::matchParsedSystemInterfaces(ClntParser *parser) {
  ...
  if (cfgIfaceCnt) {
    ...
    while (cfgIface = parser->ClntCfgIfaceLst.get()) {
      ...
      if (!ifaceIface) {
        if (inactiveMode()) {
          Log(Info) << "Interface " << cfgIface->getFullName()
                    << " is not currently available (that's ok, inactive-mode "
                       "enabled)."
                    << LogEnd;
          continue;
        }
        Log(Error) << "Interface " << cfgIface->getFullName() << " specified in " << CLNTCONF_FILE
                   << " is not present or does not support IPv6." << LogEnd;
        return false;
      }
      ...
      // setup default prefix length (used when IPv6 address is added to the
      // interface)
      ...
      if (!ifaceIface->flagUp() || !ifaceIface->countLLAddress()) {
        if (inactiveMode()) {
          ...
          addIface(cfgIface);
          makeInactiveIface(cfgIface->getID(), true, true,
      ...
      if (is_addr_tentative(ifaceIface->getName(), ifaceIface->getID(), tmp) ==
        ...
        Log(Notice) << "Interface " << ifaceIface->getFullName() << " has link-local address "
                    << tmp << ", but it is currently tentative." << LogEnd;
        ...
        Log(Crit) << "Interface " << ifaceIface->getFullName()
                  << " has tentative link-local address (and inactive-mode is "
                     "disabled)."
                  << LogEnd;
        return false;
      }
      ...
      if (obeyRaBits()) {
        if (!ifaceIface->getMBit() && !ifaceIface->getOBit()) {
          Log(Info) << "Interface " << cfgIface->getFullName()
                    ...
                    << "Advertisement with M or O bits set, adding to inactive." << LogEnd;
          addIface(cfgIface);
          makeInactiveIface(cfgIface->getID(), true, true,
                            true);  // move it to inactive list
          continue;
        }
    ...
    while (ifaceIface = ClntIfaceMgr().getIface()) {
      ...
      if (!inactiveMode()) {
        if (!ifaceIface->flagUp()) {
          Log(Notice) << "Interface " << ifaceIface->getFullName() << " is down, ignoring."
                      << LogEnd;
          continue;
        }
      ...
      if (!(ifaceIface->getMacLen() > 5)) {
        Log(Notice) << "Interface " << ifaceIface->getFullName() << " has MAC address length "
                    << ifaceIface->getMacLen() << " (6 or more required), ignoring." << LogEnd;
        continue;
      }

      // ignore disabled Teredo pseudo-interface on Win (and other similar
      // useless junk)
      ...
      if ((ifaceIface->getMacLen() <= 8) &&
        ...
        Log(Notice) << "Interface " << ifaceIface->getFullName() << " has invalid MAC address "
      ...
      Log(Info) << "Interface " << cfgIface->getFullName() << " has been added." << LogEnd;

      if (inactiveMode() && !ifaceIface->flagRunning()) {
        makeInactiveIface(cfgIface->getID(), true, true,
                          true);  // move it to InactiveLst
        Log(Notice) << "Interface " << ifaceIface->getFullName() << " is not operational yet"
                    << " (not running), made inactive." << LogEnd;
      }
      cnt++;
    }
    if (!cnt) {
      Log(Crit) << "Unable to detect any suitable interfaces. If there are any "
                   "interfaces that you"
                << " want to have configured, please specify them in "
                   "client.conf file."
                << LogEnd;
      return false;
    }
...
SPtr<TClntCfgIface> TClntCfgMgr::getIface() { return ClntCfgIfaceLst.get(); }

void TClntCfgMgr::addIface(SPtr<TClntCfgIface> ptr) { ClntCfgIfaceLst.append(ptr); }

void TClntCfgMgr::makeInactiveIface(int ifindex, bool inactive, bool managed, bool otherConf) {
...
void TClntCfgMgr::firstIface() { ClntCfgIfaceLst.first(); }

int TClntCfgMgr::countIfaces() { return ClntCfgIfaceLst.count(); }

bool TClntCfgMgr::getReconfigure() { return Reconfigure; }

void TClntCfgMgr::setReconfigure(bool enable) { Reconfigure = enable; }
...
int TClntCfgMgr::countAddrForIA(long IAID) {
  ...
  while (iface = getIface()) {
    ...
    while (ia = iface->getIA())
      if (ia->getIAID() == IAID) return ia->countAddr();
  }
  return 0;
}
...
bool TClntCfgMgr::setIAState(int iface, int iaid, enum EState state) {
  ...
  if (!ptrIface) {
    Log(Error) << "ClntCfgMgr: Unable to set IA state (id=" << iaid << "):Interface " << iface
               << " not found." << LogEnd;
    return false;
  }
...
bool TClntCfgMgr::validateConfig() {
  if (IsDone) return false;  // Is everything so far ok?
  SPtr<TClntCfgIface> ptrIface;
  ...
  switch (getAuthProtocol()) {
    ...
    case AUTH_PROTO_DELAYED: {
      ...
      if (digests.empty()) {
        Log(Error) << "AUTH: No digests specified. For delayed-auth HMAC-MD5 "
                      "must be used."
                   << LogEnd;
        return false;
      }
      ...
      if (digests[0] != DIGEST_HMAC_MD5) {
        Log(Error) << "AUTH: First digest type for delayed-auth is " << getDigestName(digests[0])
                   << ", but only HMAC-MD5 is allowed for delayed-auth." << LogEnd;
        return false;
      }
      break;
    }
    case AUTH_PROTO_RECONFIGURE_KEY: {
      ...
      if (digests[0] != DIGEST_HMAC_MD5) {
        Log(Error) << "AUTH: First digest type for reconfigure-key is " << getDigestName(digests[0])
                   << ", but only HMAC-MD5 is allowed for delayed-auth." << LogEnd;
        return false;
      }
      break;
...
bool TClntCfgMgr::validateIface(SPtr<TClntCfgIface> ptrIface) {
  ...
  while (ptrIA = ptrIface->getIA()) {
    if (!this->validateIA(ptrIface, ptrIA)) return false;
  }

  SPtr<TClntCfgPD> ptrPD;
  ptrIface->firstPD();
  while (ptrPD = ptrIface->getPD()) {
    if (!this->validatePD(ptrIface, ptrPD)) return false;
  }
...
bool TClntCfgMgr::validateIA(SPtr<TClntCfgIface> ptrIface, SPtr<TClntCfgIA> ptrIA) {
  ...
  while (ptrAddr = ptrIA->getAddr()) {
    if (!this->validateAddr(ptrIface, ptrIA, ptrAddr)) return false;
  }
  return true;
}

bool TClntCfgMgr::validateAddr(SPtr<TClntCfgIface> ptrIface, SPtr<TClntCfgIA> ptrIA,
                               SPtr<TClntCfgAddr> ptrAddr) {
  SPtr<TIPv6Addr> addr = ptrAddr->get();
  if (addr && addr->linkLocal()) {
    Log(Crit) << "Address " << ptrAddr->get()->getPlain() << " specified in IA " << ptrIA->getIAID()
              << " on the " << ptrIface->getName() << "/" << ptrIface->getID()
  ...
  if ((unsigned long)ptrIA->getT1() > (unsigned long)ptrAddr->getValid()) {
    Log(Crit) << "Valid lifetime " << ptrAddr->getValid() << " can't be lower than T1 "
              << ptrIA->getT1() << "(address can't be renewed) in IA " << ptrIA->getIAID()
              << " on the " << ptrIface->getName() << "/" << ptrIface->getName() << " interface."
              << LogEnd;
    return false;
  }
...
bool TClntCfgMgr::validatePD(SPtr<TClntCfgIface> ptrIface, SPtr<TClntCfgPD> ptrPD) {
  ...
  if (ptrPD->getT1() && ptrPD->getT2() && (ptrPD->getT2() < ptrPD->getT1())) {
    Log(Crit) << "Non-zero T1 can't be lower than non-zero T2 for PD " << *ptrPD << " on the "
  ...
  while (ptrPrefix = ptrPD->getPrefix()) {
    if (!this->validatePrefix(ptrIface, ptrPD, ptrPrefix)) return false;
  }
  return true;
}

bool TClntCfgMgr::validatePrefix(SPtr<TClntCfgIface> ptrIface, SPtr<TClntCfgPD> ptrPD,
                                 SPtr<TClntCfgPrefix> ptrPrefix) {
...
SPtr<TClntCfgIface> TClntCfgMgr::getIfaceByIAID(int iaid) {
  ...
  while (iface = getIface()) {
    ...
    iface->firstIA();
    while (ia = iface->getIA())
      if (ia->getIAID() == iaid) return iface;
  }
  return SPtr<TClntCfgIface>();
}
...
bool TClntCfgMgr::setGlobalOptions(ClntParser *parser) {
  ...
  if (getAuthProtocol() == AUTH_PROTO_DIBBLER) {
    ...
    if (SPI_) {
      ...
      Log(Crit) << "Auth: Dibbler protocol configured, but unable to read "
                   "AAA-SPI file:"
    ...
    if (len == 0) {
      Log(Crit) << "Auth: Failed to load AAA key from file " << getAAAKeyFilename(SPI_) << LogEnd;
      return false;
    }
...
bool TClntCfgMgr::getRemoteAutoconf() { return RemoteAutoconf; }
#endif
...
DigestTypes TClntCfgMgr::getDigest() { return Digest_; }

bool TClntCfgMgr::isDone() { return IsDone; }

bool TClntCfgMgr::anonInfRequest() { return AnonInfRequest; }

bool TClntCfgMgr::inactiveMode() { return InactiveMode; }
...
bool TClntCfgMgr::addInfRefreshTime() {
  ...
  firstIface();
  while (ptr = getIface()) {
    if (!ptr->stateless()) return false;
  }

  return true;
}

bool TClntCfgMgr::insistMode() { return InsistMode; }

int TClntCfgMgr::inactiveIfacesCnt() { return InactiveLst.count(); }

SPtr<TClntCfgIface> TClntCfgMgr::checkInactiveIfaces() {
  if (!InactiveLst.count()) return SPtr<TClntCfgIface>();  // NULL
  ...
  while (x = InactiveLst.get()) {
    ...
    if (iface->flagUp() && iface->flagRunning() && iface->getLLAddress()) {
      ...
      makeInactiveIface(x->getID(), false, managed,
                        otherConf);  // move it to active mode
      return x;
    }
  }
...
#ifndef MOD_DISABLE_AUTH
uint32_t TClntCfgMgr::getSPI() { return SPI_; }
...
const std::vector<DigestTypes> &TClntCfgMgr::getAuthAcceptMethods() { return AuthAcceptMethods_; }
#endif

bool TClntCfgMgr::getFQDNFlagS() { return FQDNFlagS; }

bool TClntCfgMgr::useConfirm() { return UseConfirm; }

TClntCfgMgr::~TClntCfgMgr() { Log(Debug) << "ClntCfgMgr cleanup." << LogEnd; }
...
bool TClntCfgMgr::obeyRaBits() { return ObeyRaBits_; }
...
bool HardcodedCfgExample(TClntCfgMgr *cfgMgr, string params) {
  ...
  // Pretend to have parsed empty DNS list (i.e. request DNS server
  // configuration, but don't provide any hints)
  List(TIPv6Addr) dnsList;
  dnsList.clear();
