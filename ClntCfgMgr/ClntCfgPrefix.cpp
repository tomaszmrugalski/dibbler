...
TClntCfgPrefix::TClntCfgPrefix(SPtr<TIPv6Addr> prefix, unsigned long valid, unsigned long pref, unsigned char len)
...
std::ostream & operator<<(std::ostream & out, TClntCfgPrefix & pref) {
    out << "<prefix length=\"" << (unsigned int)pref.PrefixLength << "\" preferred=\"" << pref.Pref << "\" valid=\""
        << pref.Valid << "\">";
