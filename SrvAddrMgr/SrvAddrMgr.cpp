...
TSrvAddrMgr::TSrvAddrMgr(const std::string &xmlfile, bool loadDB) : TAddrMgr(xmlfile, loadDB) {

  this->CacheMaxSize = 999999999;
  this->cacheRead();
}

TSrvAddrMgr::~TSrvAddrMgr() { Log(Debug) << "SrvAddrMgr cleanup." << LogEnd; }

/**
 * @brief adds an address to the client.
 *
 * Adds a single address to the client. If the client is not present in
 * address manager, adds it, too. Also, if client's IA is missing, adds it as
 * well.
 *
 ...
 * @param quiet quiet mode (e.g. used during SOLICIT handling, don't print
 * anything about adding client/IA/address, as it will be deleted immediately
 * anyway)
 *
 * @return true, if addition was successful
 */
bool TSrvAddrMgr::addClntAddr(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr, int iface,
                              unsigned long IAID, unsigned long T1, unsigned long T2,
  ...
  if (!cfgIface) {
    Log(Error) << "Failed to add address: config interface with ifindex=" << iface << " not found."
               << LogEnd;
    return false;
  }
  ...
  if (!ptrIA) {
    ...
    if (!quiet) Log(Debug) << "Adding IA (IAID=" << IAID << ") to addrDB." << LogEnd;
  }

  SPtr<TAddrAddr> ptrAddr;
  ptrIA->firstAddr();
  while (ptrAddr = ptrIA->getAddr()) {
    if (*ptrAddr->get() == *addr) break;
  }

  // address already exists
  if (ptrAddr) {
    Log(Warning) << "Address " << *ptrAddr << " is already assigned to this IA." << LogEnd;
    return false;
  }
  ...
  if (!quiet)
    Log(Debug) << "Adding " << ptrAddr->get()->getPlain() << " to IA (IAID=" << IAID
               << ") to addrDB." << LogEnd;
  return true;
}
...
bool TSrvAddrMgr::delClntAddr(SPtr<TDUID> clntDuid, unsigned long IAID, SPtr<TIPv6Addr> clntAddr,
                              bool quiet) {
  ...
  if (!ptrIA) {  // have we found this IA?
    Log(Warning) << "IA (IAID=" << IAID
                 << ") not assigned to client, cannot delete address and/or IA." << LogEnd;
    return false;
  }
  ...
  if (!quiet) Log(Debug) << "Deleted address " << *clntAddr << " from addrDB." << LogEnd;

  if (!ptrIA->countAddr()) {
    if (!quiet) Log(Debug) << "Deleted empty IA (IAID=" << IAID << ") from addrDB." << LogEnd;
    ptrClient->delIA(IAID);
  }

  if (!ptrClient->countIA() && !ptrClient->countTA() && !ptrClient->countPD()) {
    if (!quiet)
      Log(Debug) << "Deleted empty client (DUID=" << clntDuid->getPlain() << ") from addrDB."
                 << LogEnd;
 ...
 * @return
 */
bool TSrvAddrMgr::addTAAddr(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr, int iface,
                            unsigned long iaid, SPtr<TIPv6Addr> addr, unsigned long pref,
                            unsigned long valid) {
  // find config interface for this ifindex
  SPtr<TSrvCfgIface> cfgIface = SrvCfgMgr().getIfaceByID(iface);
  if (!cfgIface) {
  ...
  while (ta = ptrClient->getTA()) {
    if (ta->getIAID() == iaid) break;
  }

  // have we found this TA?
  if (!ta) {
    ta = new TAddrIA(cfgIface->getName(), iface, IATYPE_TA, clntAddr, clntDuid, DHCPV6_INFINITY,
                     DHCPV6_INFINITY, iaid);
    ptrClient->addTA(ta);
  ...
  // address already exists
  if (ptrAddr) {
    Log(Warning) << "Address " << *ptrAddr << " is already assigned to TA (iaid=" << iaid << ")."
                 << LogEnd;
    return false;
  }
  ...
  ptrAddr = new TAddrAddr(addr, pref, valid);
  ta->addAddr(ptrAddr);
  Log(Debug) << "Adding " << ptrAddr->get()->getPlain() << " to TA (IAID=" << iaid << ") to addrDB."
             << LogEnd;
  return true;
}
 ...
 * @return true if removal was successful
 */
bool TSrvAddrMgr::delTAAddr(SPtr<TDUID> clntDuid, unsigned long iaid, SPtr<TIPv6Addr> clntAddr,
                            bool quiet) {
  // find this client
  SPtr<TAddrClient> ptrClient;
  this->firstClient();
  ...
  ta->delAddr(clntAddr);
  if (!quiet) Log(Debug) << "Deleted temp. address " << *clntAddr << " from addrDB." << LogEnd;

  if (!ta->countAddr()) {
    if (!quiet) Log(Debug) << "Deleted TA (IAID=" << iaid << ") from addrDB." << LogEnd;
    ptrClient->delTA(iaid);
  }

  if (!ptrClient->countIA() && !ptrClient->countTA() && !ptrClient->countPD()) {
    if (!quiet)
      Log(Debug) << "Deleted client (DUID=" << clntDuid->getPlain() << ") from addrDB." << LogEnd;
    this->delClient(clntDuid);
  }
...
bool TSrvAddrMgr::delPrefix(SPtr<TDUID> clntDuid, unsigned long IAID, SPtr<TIPv6Addr> prefix,
                            bool quiet) {
  bool result = TAddrMgr::delPrefix(clntDuid, IAID, prefix, quiet);
  if (result) addCachedEntry(clntDuid, prefix, IATYPE_PD);
  return result;
}
...
unsigned long TSrvAddrMgr::getLeaseCount(SPtr<TDUID> duid) {
  ...
  while (ptrClient = ClntsLst.get()) {
    if ((*ptrClient->getDUID()) == (*duid)) break;
  }
  // Have we found this client?
  if (!ptrClient) {
    return 0;
...
bool TSrvAddrMgr::addrIsFree(SPtr<TIPv6Addr> addr) {
  ...
  while (ptrClient = ClntsLst.get()) {
    ...
    while (ptrIA = ptrClient->getIA()) {
      SPtr<TAddrAddr> ptrAddr;
      if (ptrAddr = ptrIA->getAddr(addr)) return false;
    }
  }
  return true;
}
...
bool TSrvAddrMgr::taAddrIsFree(SPtr<TIPv6Addr> addr) {
  ...
  while (ptrClient = ClntsLst.get()) {
    ...
    ptrClient->firstTA();
    while (ta = ptrClient->getTA()) {
      if (ta->getAddr(addr)) return false;
    }
  }
  return true;
}

void TSrvAddrMgr::getAddrsCount(SPtr<List(TSrvCfgAddrClass)> classes, long *clntCnt, long *addrCnt,
                                SPtr<TDUID> duid, int iface) {
...
void TSrvAddrMgr::doDuties(std::vector<TExpiredInfo> &addrLst,
  ...
  while (ptrClient = this->getClient()) {
    ...
    if (ptrClient->getValidTimeout()) continue;
    ...
    while (ptrIA = ptrClient->getIA()) {
      ...
      while (ptrAddr = ptrIA->getAddr()) {
        ...
        // delClntAddr(ptrClient->getDUID(), ptrIA->getIAID(),  ptrAddr->get(),
        // false);
      }
    }
    ...
    while (pd = ptrClient->getPD()) {
      if (pd->getValidTimeout()) continue;

      pd->firstPrefix();
      while (prefix = pd->getPrefix()) {
        if (prefix->getValidTimeout()) continue;
...
/// @brief Checks if address is still supported in current configuration (used
/// in loadDB)
///
...
/// @brief Checks if prefix is still supported in current configuration (used in
/// loadDB)
///
 ...
 *
 * @param clntDuid
 * @param type type of entry looked for (TYPE_IA for address or TYPE_PD for
 * prefix)
 *
...
SPtr<TIPv6Addr> TSrvAddrMgr::getCachedEntry(SPtr<TDUID> clntDuid, TIAType type) {
  if (!this->CacheMaxSize) return SPtr<TIPv6Addr>();
  SPtr<TSrvCacheEntry> entry;
  this->Cache.first();

  while (entry = this->Cache.get()) {
    if (!entry->Duid) continue;  // something is wrong. VERY wrong. But shut up and continue.
    if ((entry->type == type) && (*entry->Duid == *clntDuid)) {
      Log(Debug) << "Cache: Cached " << (type == IATYPE_IA ? "address" : "prefix")
                 << " for client (DUID=" << clntDuid->getPlain()
                 << ") found: " << entry->Addr->getPlain() << LogEnd;
      return entry->Addr;
    }
  }
...
bool TSrvAddrMgr::delCachedEntry(SPtr<TIPv6Addr> addr, TIAType type) {
  if (!this->CacheMaxSize) return false;

  this->Cache.first();
  SPtr<TSrvCacheEntry> entry;
  while (entry = this->Cache.get()) {
    if (!entry->Addr) continue;  // something is wrong. VERY wrong. But shut up and continue.
    if ((entry->type == type) && (*(entry->Addr) == *addr)) {
      this->Cache.del();
      this->Cache.first();
      Log(Debug) << "Cache: " << (type == IATYPE_IA ? "Address " : "Prefix ") << *addr
                 << " was deleted." << LogEnd;
      return true;
    }
  }
...
bool TSrvAddrMgr::delCachedEntry(SPtr<TDUID> clntDuid, TIAType type) {
  ...
  while (entry = this->Cache.get()) {
    ...
    if ((entry->type == type) && (*(entry->Duid) == *clntDuid)) {
      this->Cache.del();
      this->Cache.first();
      Log(Debug) << "Cache: Entry for client (DUID=" << clntDuid->getPlain() << ") was deleted."
                 << LogEnd;
      return true;
    }
  }
  // delete attempt is done on multiple occasions as a safety precausion, so
  // don't warn if it is missing Log(Debug) << "Cache: Attempt to delete entry
  // for client (DUID=" << clntDuid->getPlain() << ") failed." << LogEnd;
  return false;
}

/**
 * this function adds an address or prefix to a cache. If there is entry for
 * this client, updates it
 *
 * @param clntDuid
 * @param cachedAddr
 * @param type type of entry looked for (TYPE_IA for address or TYPE_PD for
...
void TSrvAddrMgr::addCachedEntry(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> cachedAddr, TIAType type) {
  ...
  if (!this->CacheMaxSize) return;

  // if this address is reserved, don't add it to cache)
  if ((type == IATYPE_IA) && (SrvCfgMgr().addrReserved(cachedAddr))) return;

  // if this prefix is reserved, don't add it to cache)
  if ((type == IATYPE_PD) && (SrvCfgMgr().prefixReserved(cachedAddr))) return;

  SPtr<TSrvCacheEntry> entry;

  // is there an entry for this client, delete it. New entry will be added at
  // the end
...
void TSrvAddrMgr::setCacheSize(int bytes) {
  ...
  Log(Debug) << "Cache: size set to " << bytes << " bytes, 1 cache entry size is " << entrySize
             << " bytes, so maximum " << this->CacheMaxSize
             << " address-client pair(s) may be cached." << LogEnd;
  this->checkCacheSize();
}

/**
 * this function checks if the cache size was not exceeded. If that is so,
 * oldest entries are removed
 *
 */
void TSrvAddrMgr::checkCacheSize() {
  if (this->Cache.count() <= this->CacheMaxSize) return;
...
void TSrvAddrMgr::dump() {
  ...
  if (SrvCfgMgr().getPerformanceMode()) return;
...
void TSrvAddrMgr::cacheDump() {
  ...
  while (x = this->Cache.get()) {
    ...
    f << "\" duid=\"";
    if (x->Duid) f << x->Duid->getPlain();
    f << "\">";
    if (x->Addr) f << x->Addr->getPlain();
    f << "</entry>" << endl;
  }
  f << "</cache>" << endl;
  f.close();
  ...
  while (!f.eof()) {
    ...
    if (((pos = s.find("<cache")) != string::npos)) {
      ...
      if ((pos = s.find("size=\"")) != string::npos) {
        ...
        Log(Debug) << "Cache:" << SRVCACHE_FILE << " file: parsing started, expecting " << entries
                   << " entries." << LogEnd;
      } else {
    ...
    if (s.find("<entry") != string::npos) {
      ...
      type = IATYPE_IA;  // assume that entry is for address (to maintain
      ...
      if ((pos = s.find("type=\"")) != string::npos) {
        ...
        if (typeStr == "addr")
          ...
          Log(Error) << "Invalid entry type in line " << lineno << " in " << SRVCACHE_FILE
                     << LogEnd;
          continue;
        }
      }
    ...
    if (s.find("</cache>") != string::npos) {
      if (!started) {
        Log(Error) << "Cache: Reading file " << SRVCACHE_FILE
                   << " failed: closing tag </cache> found at line " << lineno
