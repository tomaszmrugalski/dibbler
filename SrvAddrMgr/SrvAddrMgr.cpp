 ...
 */
bool TSrvAddrMgr::addClntAddr(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr, int iface, unsigned long IAID, unsigned long T1,
                              unsigned long T2, SPtr<TIPv6Addr> addr, unsigned long pref, unsigned long valid, bool quiet) {
    ...
    if (!quiet)
        Log(Debug) << "Adding " << ptrAddr->get()->getPlain() << " to IA (IAID=" << IAID << ") to addrDB." << LogEnd;
    return true;
}
...
bool TSrvAddrMgr::delClntAddr(SPtr<TDUID> clntDuid, unsigned long IAID, SPtr<TIPv6Addr> clntAddr, bool quiet) {
    ...
    if (!ptrClient) { // have we found this client?
        Log(Warning) << "Client (DUID=" << clntDuid->getPlain() << ") not found in addrDB, cannot delete address and/or client."
                     << LogEnd;
        return false;
    }
    ...
    if (!ptrIA) { // have we found this IA?
        Log(Warning) << "IA (IAID=" << IAID << ") not assigned to client, cannot delete address and/or IA." << LogEnd;
        return false;
    }
    ...
    if (!ptrClient->countIA() && !ptrClient->countTA() && !ptrClient->countPD()) {
        if (!quiet)
            Log(Debug) << "Deleted empty client (DUID=" << clntDuid->getPlain() << ") from addrDB." << LogEnd;
 ...
 * @return
 */
bool TSrvAddrMgr::addTAAddr(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr, int iface, unsigned long iaid, SPtr<TIPv6Addr> addr,
    ...
    if (!ta) {
        ta = new TAddrIA(cfgIface->getName(), iface, IATYPE_TA, clntAddr, clntDuid, DHCPV6_INFINITY, DHCPV6_INFINITY, iaid);
        ptrClient->addTA(ta);
    ...
    // address already exists
    if (ptrAddr) {
        Log(Warning) << "Address " << *ptrAddr << " is already assigned to TA (iaid=" << iaid << ")." << LogEnd;
        return false;
    }
    ...
    ptrAddr = new TAddrAddr(addr, pref, valid);
    ta->addAddr(ptrAddr);
    Log(Debug) << "Adding " << ptrAddr->get()->getPlain() << " to TA (IAID=" << iaid << ") to addrDB." << LogEnd;
    return true;
}
 ...
 * @return true if removal was successful
 */
bool TSrvAddrMgr::delTAAddr(SPtr<TDUID> clntDuid, unsigned long iaid, SPtr<TIPv6Addr> clntAddr, bool quiet) {
    // find this client
    SPtr<TAddrClient> ptrClient;
    this->firstClient();
    while (ptrClient = this->getClient()) {
    ...
    // have we found this TA?
    if (!ta) {
        Log(Warning) << "TA (IAID=" << iaid << ") not assigned to client, cannot delete address and/or IA." << LogEnd;
        return false;
    }
...
void TSrvAddrMgr::getAddrsCount(SPtr<List(TSrvCfgAddrClass)> classes, long * clntCnt, long * addrCnt, SPtr<TDUID> duid,
                                int iface) {
...
/// @param prefixLst
///
void TSrvAddrMgr::doDuties(std::vector<TExpiredInfo> & addrLst, std::vector<TExpiredInfo> & tempAddrLst,
    ...
    while (entry = this->Cache.get()) {
        ...
        if ((entry->type == type) && (*entry->Duid == *clntDuid)) {
            Log(Debug) << "Cache: Cached " << (type == IATYPE_IA ? "address" : "prefix")
                       << " for client (DUID=" << clntDuid->getPlain() << ") found: " << entry->Addr->getPlain() << LogEnd;
            return entry->Addr;
        }
    }
...
bool TSrvAddrMgr::delCachedEntry(SPtr<TIPv6Addr> addr, TIAType type) {
    ...
    while (entry = this->Cache.get()) {
        ...
        if ((entry->type == type) && (*(entry->Addr) == *addr)) {
            this->Cache.del();
            this->Cache.first();
            Log(Debug) << "Cache: " << (type == IATYPE_IA ? "Address " : "Prefix ") << *addr << " was deleted." << LogEnd;
            return true;
        }
    }
...
void TSrvAddrMgr::setCacheSize(int bytes) {
    ...
    Log(Debug) << "Cache: size set to " << bytes << " bytes, 1 cache entry size is " << entrySize << " bytes, so maximum "
               << this->CacheMaxSize << " address-client pair(s) may be cached." << LogEnd;
    this->checkCacheSize();
}
...
void TSrvAddrMgr::cacheRead() {
    ...
    while (!f.eof()) {
        ...
        if (((pos = s.find("<cache")) != string::npos)) {
            ...
            if ((pos = s.find("size=\"")) != string::npos) {
                ...
                Log(Debug) << "Cache:" << SRVCACHE_FILE << " file: parsing started, expecting " << entries << " entries."
                           << LogEnd;
            } else {
        ...
        if (s.find("<entry") != string::npos) {
            ...
            if ((pos = s.find("duid=\"")) != string::npos) {
                ...
                Log(Error) << "Cache: " << SRVCACHE_FILE << " file: missing duid=\"...\" in line " << lineno << "." << LogEnd;
                return;
            }
        }
    ...
    if (this->Cache.count() != entries) {
        Log(Debug) << "Cache: " << SRVCACHE_FILE << " file: " << entries << " entries expected, but " << this->Cache.count()
                   << " found." << LogEnd;
    }
    if (!parsed) {
