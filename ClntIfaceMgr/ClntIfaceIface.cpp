 ...
 */
TClntIfaceIface::TClntIfaceIface(char * name, int id, unsigned int flags, char * mac, int maclen, char * llAddr, int llAddrCnt,
                                 char * globalAddr, int globalAddrCnt, int hwType)
    : TIfaceIface(name, id, flags, mac, maclen, llAddr, llAddrCnt, globalAddr, globalAddrCnt, hwType), TunnelEndpoint(),
      LifetimeTimeout(DHCPV6_INFINITY) {
...
bool TClntIfaceIface::setDNSServerLst(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, List(TIPv6Addr) addrs) {

#ifdef WIN32
    // remove all dns records before the first configuration
    if (!DnsConfigured) {
        Log(Notice) << "Removing all DNS servers on interface " << getFullName() << " (not configured yet)." << LogEnd;
    ...
    while (old = this->DNSServerLst.get()) {
        ...
        if (!found) {
            ...
            Log(Notice) << "Removing DNS server " << *old << " on interface " << this->getName() << "/" << this->getID()
                        << " (no longer valid)." << LogEnd;
    ...
    while (addr = addrs.get()) {
        ...
        if (found) {
            ...
            Log(Info) << "DNS server " << *addr << " is already set on interface " << this->getName() << "/" << this->getID()
                      << ", no change is needed." << LogEnd;
        } else {
            // it's new address,set it!
            Log(Notice) << "Setting up DNS server " << *addr << " on interface " << this->getName() << "/" << this->getID()
                        << "." << LogEnd;
...
bool TClntIfaceIface::setDomainLst(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, List(std::string) domains) {
    ...
    while (old = this->DomainLst.get()) {
        ...
        if (!found) {
            // it's not on the new list, so remove it
            Log(Notice) << "Removing domain " << *old << " from interface " << this->getName() << "/" << this->getID()
                        << " (no longer valid)." << LogEnd;
    ...
    while (domain = domains.get()) {
        ...
        if (found) {
            ...
            Log(Info) << "Domain " << *domain << " is already set on interface " << this->getName() << "/" << this->getID()
                      << ", no change is needed." << LogEnd;
        } else {
            // it's new domain, set it!
            Log(Notice) << "Setting up Domain " << *domain << " on interface " << this->getName() << "/" << this->getID() << "."
                        << LogEnd;
...
bool TClntIfaceIface::setNTPServerLst(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, List(TIPv6Addr) addrs) {
    // remove old addresses
    SPtr<TIPv6Addr> old, addr;
    ...
    while (old = this->NTPServerLst.get()) {
        ...
        if (!found) {
            // it's not on the new list, so remove it
            Log(Notice) << "Removing NTP server " << *old << " on interface " << this->getName() << "/" << this->getID()
                        << " (no longer valid)." << LogEnd;
    ...
    while (addr = addrs.get()) {
        ...
        if (found) {
            // this address is already set
            Log(Info) << "NTP server " << *addr << " is already set on the interface " << this->getName() << "/"
                      << this->getID() << ", no change is needed." << LogEnd;
        } else {
            // it's new address,set it!
            Log(Notice) << "Setting up NTP server " << *addr << " on the interface " << this->getName() << "/" << this->getID()
                        << "." << LogEnd;
...
bool TClntIfaceIface::setTimezone(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, const std::string & timezone) {
    if (timezone == this->Timezone) {
        ...
        Log(Info) << "Timezone " << timezone << " is already set on the interface " << this->getName() << "/" << this->getID()
                  << ", no change is needed." << LogEnd;
        return true;
    }
    this->Timezone = timezone;

    Log(Notice) << "Setting up timezone " << timezone << " (obtained on the interface " << this->getName() << "/"
                << this->getID() << ")." << LogEnd;
...
bool TClntIfaceIface::setSIPServerLst(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, List(TIPv6Addr) addrs) {
    // remove old addresses
    SPtr<TIPv6Addr> old, addr;
    ...
    while (old = this->SIPServerLst.get()) {
        ...
        if (!found) {
            // it's not on the new list, so remove it
            Log(Notice) << "Removing SIP server " << *old << " on interface " << this->getName() << "/" << this->getID()
                        << " (no longer valid)." << LogEnd;
    ...
    while (addr = addrs.get()) {
        ...
        if (found) {
            // this address is already set
            Log(Info) << "SIP server " << *addr << " is already set on interface " << this->getName() << "/" << this->getID()
                      << ", no change is needed." << LogEnd;
        } else {
            // it's new address,set it!
            Log(Notice) << "Setting up SIP server " << *addr << " on interface " << this->getName() << "/" << this->getID()
                        << "." << LogEnd;
...
bool TClntIfaceIface::setSIPDomainLst(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, List(std::string) domains) {
    ...
    while (old = this->SIPDomainLst.get()) {
        ...
        if (!found) {
            // it's not on the new list, so remove it
            Log(Notice) << "Removing SIP domain " << *old << " from interface " << this->getName() << "/" << this->getID()
                        << " (no longer valid)." << LogEnd;
    ...
    while (domain = domains.get()) {
        ...
        if (found) {
            // this domain is already set
            Log(Info) << "SIP domain " << *domain << " is already set on interface " << this->getName() << "/" << this->getID()
                      << ", no change is needed." << LogEnd;
        } else {
            // it's new domain, set it!
            Log(Notice) << "Setting up SIP domain " << *domain << " on interface " << this->getName() << "/" << this->getID()
                        << "." << LogEnd;
...
bool TClntIfaceIface::setNISServerLst(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, List(TIPv6Addr) addrs) {
    ...
    while (old = this->NISServerLst.get()) {
        ...
        if (!found) {
            // it's not on the new list, so remove it
            Log(Notice) << "Removing NIS server " << *old << " on interface " << this->getName() << "/" << this->getID()
                        << " (no longer valid)." << LogEnd;
    ...
    while (addr = addrs.get()) {
        ...
        if (found) {
            // this address is already set
            Log(Info) << "NIS server " << *addr << " is already set on interface " << this->getName() << "/" << this->getID()
                      << ", no change is needed." << LogEnd;
        } else {
            // it's new address,set it!
            Log(Notice) << "Setting up NIS server " << *addr << " on interface " << this->getName() << "/" << this->getID()
                        << "." << LogEnd;
...
bool TClntIfaceIface::setNISDomain(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, const std::string & domain) {
    if (domain == NISDomain) {
        ...
        Log(Info) << "NIS Domain " << NISDomain << " is already set on the interface " << this->getName() << "/"
                  << this->getID() << ", no change is needed." << LogEnd;
        return true;
    }
    this->NISDomain = domain;

    Log(Notice) << "Setting up NIS domain " << domain << " on the interface " << this->getName() << "/" << this->getID() << "."
                << LogEnd;
...
bool TClntIfaceIface::setNISPDomain(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, const std::string & domain) {
    if (domain == this->NISPDomain) {
        // NIS+ Domain has not changed
        Log(Info) << "NIS+ Domain " << NISPDomain << " is already set on the interface " << this->getName() << "/"
                  << this->getID() << ", no change is needed." << LogEnd;
        return true;
    }
...
void TClntIfaceIface::delString(const char * filename, const char * str) {
    ...
    if (!(f = fopen(filename, "r"))) {
        Log(Debug) << "Unable to open file " << filename << " while trying to delete " << str << " line." << LogEnd;
        return;
    }

    if (!(fout = fopen(fileout.c_str(), "w"))) {
        Log(Debug) << "Unable to create/overwrite file " << fileout << " while trying to delete " << str << " line." << LogEnd;
        fclose(f);
        return;
    }
...
void TClntIfaceIface::removeAllOpts() {
    ...
    while (addr = this->DNSServerLst.get()) {
        Log(Notice) << "DNS server " << *addr << " removed from the " << this->getName() << "/" << this->getID()
                    << " interface." << LogEnd;
        this->DNSServerLst.del();
    ...
    while (str = this->DomainLst.get()) {
        Log(Notice) << "Domain " << *str << " removed from the " << this->getName() << "/" << this->getID() << " interface."
                    << LogEnd;
        this->DomainLst.del();
    ...
    while (addr = this->NTPServerLst.get()) {
        Log(Notice) << "NTP server " << *addr << " removed from the " << this->getName() << "/" << this->getID()
                    << " interface." << LogEnd;
        this->NTPServerLst.del();
    ...
    if (this->Timezone.length()) {
        Log(Notice) << "Timezone " << this->Timezone << " unset on the interface " << this->getName() << "/" << this->getID()
                    << "." << LogEnd;
        this->Timezone = "";
    ...
    while (addr = this->SIPServerLst.get()) {
        Log(Notice) << "SIP server " << *addr << " removed from the " << this->getName() << "/" << this->getID()
                    << " interface." << LogEnd;
        this->SIPServerLst.del();
    ...
    while (str = this->SIPDomainLst.get()) {
        Log(Notice) << "SIP domain " << *str << " removed from the " << this->getName() << "/" << this->getID() << " interface."
                    << LogEnd;
        this->SIPDomainLst.del();
    ...
    while (addr = this->NISServerLst.get()) {
        Log(Notice) << "NIS server " << *addr << " removed from the " << this->getName() << "/" << this->getID()
                    << " interface." << LogEnd;
        this->NISServerLst.del();
    ...
    if (this->NISDomain.length()) {
        Log(Notice) << "NIS Domain " << this->NISDomain << " unset on the " << this->getName() << "/" << this->getID()
                    << " interface." << LogEnd;
...
std::ostream & operator<<(std::ostream & strum, TClntIfaceIface & x) {
    ...
    strum << " flags=\"0x" << hex << x.Flags << dec << "\""
          << " mBit=\"" << (x.M_bit_ ? "1" : "0") << "\" oBit=\"" << (x.O_bit_ ? "1" : "0") << "\">" << endl;
    ...
    if (x.LifetimeTimeout == DHCPV6_INFINITY) {
        ...
        strum << "    <lifetime timeout=\"" << x.LifetimeTimeout << "\" timestamp=\"" << x.LifetimeTimestamp << "\" />" << endl;
    }
