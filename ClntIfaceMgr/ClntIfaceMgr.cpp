...
bool TClntIfaceMgr::sendUnicast(int iface, char *msg, int size, SPtr<TIPv6Addr> addr) {
  ...
  if (result == -1) {
    Log(Error) << "Send failed: " << size << " bytes to " << *addr << " on " << Iface->getFullName()
...
TClntIfaceMgr::TClntIfaceMgr(const std::string &xmlFile) : TIfaceMgr(xmlFile, false) {
  ...
  while (ptr != NULL) {
    ...
    SPtr<TIfaceIface> iface = new TClntIfaceIface(
        ptr->name, ptr->id, ptr->flags, ptr->mac, ptr->maclen, ptr->linkaddr, ptr->linkaddrcount,
...
bool TClntIfaceMgr::doDuties() {
  ...
  while (iface = SPtr_cast<TClntIfaceIface>(getIface())) {
    ...
    if (cfgIface) {
      // Log(Debug) << "FQDN State: " << cfgIface->getFQDNState() << " on " <<
      // iface->getFullName() << LogEnd;
      if (cfgIface->getFQDNState() == STATE_INPROCESS) {
        // Here we check if all parameters are set, and do the DNS update if
        // possible
        ...
        if (ClntAddrMgr().countIA() > 0 && DNSSrvLst.count() > 0 && fqdn.size() > 0) {
          ...
          /** @todo: sleep cannot be performed here. What if client has to
...
bool TClntIfaceMgr::fqdnAdd(SPtr<TClntIfaceIface> iface, const std::string &fqdn) {
  ...
  if (ptrAddrIA->countAddr() > 0) {
    ...
    if (proto == TCfgMgr::DNSUPDATE_UDP) proto2 = DNSUpdate::DNSUPDATE_UDP;
    if (proto == TCfgMgr::DNSUPDATE_ANY) proto2 = DNSUpdate::DNSUPDATE_ANY;
    unsigned int timeout = ClntCfgMgr().getDDNSTimeout();

    /* add AAAA record */
    DNSUpdate *act =
        new DNSUpdate(DNSAddr->getPlain(), "", fqdn, addr->getPlain(), DNSUPDATE_AAAA, proto2);
...
bool TClntIfaceMgr::fqdnDel(SPtr<TClntIfaceIface> iface, SPtr<TAddrIA> ia,
  ...
  DNSUpdate *act =
      new DNSUpdate(dns->getPlain(), "", fqdn, myAddr->getPlain(), DNSUPDATE_AAAA_CLEANUP, proto2);
  int result = act->run(timeout);
  act->showResult(result);
  delete act;
 ...
 */
bool TClntIfaceMgr::addPrefix(int iface, SPtr<TIPv6Addr> prefix, int prefixLen, unsigned int pref,
                              unsigned int valid, TNotifyScriptParams *params /*= NULL*/) {
...
bool TClntIfaceMgr::modifyPrefix(int iface, SPtr<TIPv6Addr> prefix, int prefixLen,
  ...
  Log(Notice) << "PD: " << action << " prefix " << prefix->getPlain() << "/" << (int)prefixLen
              << " to all interfaces (prefix will be split to /" << int(prefixLen + 8)
              << " prefixes if necessary)." << LogEnd;
  ...
  for (vector<string>::const_iterator name = ifaceNames.begin(); name != ifaceNames.end(); ++name) {
    ...
    if (x)
      ...
      Log(Warning) << "Interface " << *name << " specified in downlink-prefix-ifaces is missing."
                   << LogEnd;
    }
  }

  if (!skip && (ifaceLst.empty())) {
    // Ok, the sysadmin didn't provide a list of downlink interfaces and didn't
    // tell us to skip, we need to generate the list dynamically.
    ...
    while (x = getIface()) {
      ...
      if (!x->flagRunning()) {
        Log(Debug) << "PD: Interface " << x->getFullName() << " has flag RUNNING not set, ignoring."
                   << LogEnd;
        continue;
      }
      if (!x->flagMulticast()) {
        Log(Debug) << "PD: Interface " << x->getFullName() << " is not multicast capable, ignoring."
                   << LogEnd;
        continue;
      }
      if (!(x->getMacLen() > 5)) {
        Log(Debug) << "PD: Interface " << x->getFullName() << " has MAC address length "
                   << x->getMacLen() << " (6 or more required), ignoring." << LogEnd;
        continue;
      }
      x->firstLLAddress();
      if (!x->getLLAddress()) {
  ...
  for (TIfaceIfaceLst::const_iterator i = ifaceLst.begin(); i != ifaceLst.end(); ++i) {
    ...
    if (numPrefixes > 256) {
      // users with too much time that play with virtual interfaces are out of
      // luck
    ...
    SPtr<TIPv6Addr> subprefix =
        calculateSubprefix(prefix, prefixLen, numPrefixes, infix, subprefixLen);
    ...
    switch (mode) {
      case PREFIX_MODIFY_ADD:
        ...
        break;
      case PREFIX_MODIFY_UPDATE:
        status = prefix_update((*i)->getName(), (*i)->getID(), subprefix->getPlain(), subprefixLen,
                               pref, valid);
        break;
      case PREFIX_MODIFY_DEL:
...
SPtr<TIPv6Addr> TClntIfaceMgr::calculateSubprefix(const SPtr<TIPv6Addr> &prefix, int prefixLen,
  ...
  // operations on very long prefixes: /120 to /127).
  char buf[17];
  memset(buf, 0, 17);
...
void TClntIfaceMgr::redetectIfaces() {
  ...
  while (ptr != NULL) {
    ...
    if ((ptr->flags != iface->getFlags()) || (ptr->m_bit != iface->getMBit()) ||
        (ptr->o_bit != iface->getOBit())) {
      Log(Notice) << "Flags on interface " << iface->getFullName() << " has changed (old=" << hex
                  << iface->getFlags() << ", new=" << ptr->flags << dec
                  << ", M bit:" << (iface->getMBit() ? "1" : "0") << "->"
...
bool TClntIfaceMgr::notifyRemoteScripts(SPtr<TIPv6Addr> rcvdAddr, SPtr<TIPv6Addr> srvAddr,
  ...
  Log(Info) << "Received address " << rcvdAddr->getPlain() << " from remote server located at "
            << srvAddr->getPlain() << LogEnd;
