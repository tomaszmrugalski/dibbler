...
class TClntIfaceMgr : public TIfaceMgr {
  ...
  public:
    ...
    bool updatePrefix(int iface, SPtr<TIPv6Addr> prefix, int prefixLen, unsigned int pref, unsigned int valid,
                      TNotifyScriptParams * params /*= NULL*/);
    bool delPrefix(int iface, SPtr<TIPv6Addr> prefix, int prefixLen, TNotifyScriptParams * params /*= NULL*/);
    ...
    SPtr<TIPv6Addr> calculateSubprefix(const SPtr<TIPv6Addr> & prefix, int prefixLen, int numPrefixes, int i,
                                       int & subprefixLen);

  private:
    bool modifyPrefix(int iface, SPtr<TIPv6Addr> prefix, int prefixLen, unsigned int pref, unsigned int valid,
