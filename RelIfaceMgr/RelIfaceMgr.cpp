 ...
 */
TRelIfaceMgr::TRelIfaceMgr(const std::string &xmlFile) : TIfaceMgr(xmlFile, true) {}

TRelIfaceMgr::~TRelIfaceMgr() { Log(Debug) << "RelIfaceMgr cleanup." << LogEnd; }
...
bool TRelIfaceMgr::send(int ifindex, char *data, int dataLen, SPtr<TIPv6Addr> addr, int port) {
  ...
  if (!ptrSocket) {
    Log(Error) << "Send failed: interface " << iface->getName() << "/" << iface->getID()
               << " has no open sockets." << LogEnd;
    return false;
  }

  // send it!
  if (ptrSocket->send(data, dataLen, addr, port) < 0) return false;
  return true;
}
...
SPtr<TRelMsg> TRelIfaceMgr::decodeRelayForw(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer,
...
SPtr<TRelMsg> TRelIfaceMgr::decodeGeneric(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer, char *buf,
                                          int bufsize) {
  int ifindex = iface->getID();
  return new TRelMsgGeneric(ifindex, peer, buf, bufsize);
}

SPtr<TRelMsg> TRelIfaceMgr::decodeRelayRepl(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer,
  ...
  while (bufsize >= 4) {
    ...
    switch (code) {
      case OPTION_INTERFACE_ID:
        if (bufsize < 4) {
          Log(Warning) << "Truncated INTERFACE_ID option in RELAY_REPL message. "
                          "Message dropped."
                       << LogEnd;
          return SPtr<TRelMsg>();  // NULL
        }
        if (len != 4) {
          Log(Warning) << "Invalid INTERFACE_ID option, expected length " << 4 << ", actual length "
                       << len << "." << LogEnd;
          return SPtr<TRelMsg>();  // NULL
        }
      ...
      default:
        ...
        if (echo && echo->isOption(code)) {
          ...
          Log(Warning) << "Invalid option " << code << " in RELAY_REPL message. Option ignored."
                       << LogEnd;
        }
        buf += len;
        bufsize -= len;
  ...
  Log(Info) << "RELAY_REPL was decapsulated: link=" << linkAddr->getPlain()
            << ", peer=" << peerAddr->getPlain();
  if (ptrIfaceID) Log(Cont) << ", interfaceID=" << ptrIfaceID->getValue();
  Log(Cont) << LogEnd;

  if (!ptrIfaceID) {
    if (!RelCfgMgr().guessMode()) {
      /* guessMode disabled */
      Log(Warning) << "InterfaceID option is missing, guessMode disabled, "
                      "unable to forward. Packet dropped."
                   << LogEnd;
      return SPtr<TRelMsg>();  // NULL
    }
    ...
    if (!tmp) {
      Log(Error) << "Guess-mode failed. Unable to find any interface the message can "
                    "be relayed on. Please send interface-id option in server replies."
                 << LogEnd;
      return SPtr<TRelMsg>();  // NULL
    }
    Log(Notice) << "Guess-mode successful. Using " << tmp->getFullName()
                << " as destination interface." << LogEnd;
  ...
  // inform that this message should be sent to the peerAddr address on the
  // ptrIface interface.
  msg->setDestination(ptrIfaceID->getValue(), peerAddr);
  return msg;
}

SPtr<TRelMsg> TRelIfaceMgr::decodeMsg(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer, char *data,
                                      int dataLen) {
  if (dataLen <= 0) return SPtr<TRelMsg>();  // NULL
...
void TRelIfaceMgr::instanceCreate(const std::string &xmlFile) {
  if (Instance) Log(Crit) << "RelIfaceMgr instance already created. Application error!" << LogEnd;
  Instance = new TRelIfaceMgr(xmlFile);
}

TRelIfaceMgr &TRelIfaceMgr::instance() {
  if (!Instance) {
    Log(Crit) << "RelIfaceMgr istance not created yet. Application error. "
                 "Emergency shutdown."
              << LogEnd;
    exit(EXIT_FAILURE);
  }
