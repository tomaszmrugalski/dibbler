...
bool TRelIfaceMgr::send(int ifindex, char * data, int dataLen, SPtr<TIPv6Addr> addr, int port) {
    ...
    if (!ptrSocket) {
        Log(Error) << "Send failed: interface " << iface->getName() << "/" << iface->getID() << " has no open sockets."
                   << LogEnd;
        return false;
    }
...
SPtr<TRelMsg> TRelIfaceMgr::select(unsigned long timeout) {
    ...
    Log(Debug) << "Received " << dataLen << " bytes on the " << iface->getName() << "/" << iface->getID()
               << " interface (socket=" << sockid << ", addr=" << peer->getPlain() << ", port=" << sock->getPort() << ")."
               << LogEnd;
...
SPtr<TRelMsg> TRelIfaceMgr::decodeRelayForw(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer, char * data, int dataLen) {
    int ifindex = iface->getID();
    SPtr<TRelMsg> msg = new TRelMsgRelayForw(ifindex, peer, data, dataLen);
    return msg;
}

SPtr<TRelMsg> TRelIfaceMgr::decodeGeneric(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer, char * buf, int bufsize) {
    int ifindex = iface->getID();
    return new TRelMsgGeneric(ifindex, peer, buf, bufsize);
}

SPtr<TRelMsg> TRelIfaceMgr::decodeRelayRepl(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer, char * buf, int bufsize) {
    ...
    while (bufsize >= 4) {
        ...
        case OPTION_INTERFACE_ID:
            ...
            if (len != 4) {
                Log(Warning) << "Invalid INTERFACE_ID option, expected length " << 4 << ", actual length " << len << "."
                             << LogEnd;
                return SPtr<TRelMsg>(); // NULL
            }
    ...
    if (!ptrIfaceID) {
        ...
        if (!tmp) {
            Log(Error) << "Guess-mode failed. Unable to find any interface the message can be relayed on. Please send "
                          "interface-id option in server replies."
                       << LogEnd;
            return SPtr<TRelMsg>(); // NULL
        }
...
SPtr<TRelMsg> TRelIfaceMgr::decodeMsg(SPtr<TIfaceIface> iface, SPtr<TIPv6Addr> peer, char * data, int dataLen) {
    if (dataLen <= 0)
        return SPtr<TRelMsg>(); // NULL
