...
%%
%% $Id: dibbler-user-config.tex,v 1.59 2009-04-28 22:15:30 thomson Exp $
%%
...
\item[log-colors] -- (scope: global, type: boolean, default:
            off). Defines if logs printed to console should use
            colors. That feature is used to enhance logs readability.
            As it makes the log files messy on systems that do not
            support colors, it is disabled by default.
...
\begin{description}
 ...
 \item[ds-lite] -- (scope: interface, type: address or FQDN). In
            Dual-Stack Lite networks, client may want to configure
            DS-Lite tunnel. Client may want to obtain information
            about remote tunnel endpoint. There are 2 options that
            allow configuration of that option. One is IPv6 address
            based and the second one uses FQDN. Both options are
            defined in \cite{draft-ds-lite}.
...
\subsubsection{Example 16: Dual-Stack Lite information}
\label{example-server-ds-lite}
Server may provide information about DS-Lite tunnel endpoints to the
clients. 
...
iface "eth0" {
  ...
  option ds-lite address   # request for address only
#  option ds-lite name     # request for FQDN only
#  option ds-lite          # request for both
}
...
\subsubsection{Example 19: DS-Lite tunnel}
