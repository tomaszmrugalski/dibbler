...
\subsubsection{External script}
\label{feature-script}

\Note Support for external scripts was rewritten in 0.8.1RC1
release. Note that mapping prefix and notify scripts were removed.

Support for server-side script was introduced in 0.8.1RC1.
...
"add'', "update'', or "delete''. Actual values of received parameters
are passed as environment variables. In particular, IFNAME and IFINDEX
variables denote interface name and interface index that was used to
communicate with server, respectively. Another essential variable set
is REMOTE\_ADDR. It defines address from which packet originated. That
is client's address (when run on server) and server's address (when
run on client).

Addresses are passed in variables ADDR1, ADDR2 and following. Note
that each ADDR variable does contain triples of
(address,preferred-lifetime,valid-lifetime) separated by spaces.

Prefixes are passed in variables PREFIX1, PREFIX2 and following. Note
that each PREFIX variable does contain quadriplet of (prefix,
prefix-length, preferred-lifetime, valid-lifetime) separated by spaces.

Support for additional options is in progress. Options will be passed
