...
public:
  ...
  bool appendMandatoryOptions(SPtr<TOptOptionRequest> oro,
                              bool includeClientID = true);
  bool appendRequestedOptions(SPtr<TDUID> duid, SPtr<TIPv6Addr> addr, int iface,
                              SPtr<TOptOptionRequest> reqOpt);
  std::string showRequestedOptions(SPtr<TOptOptionRequest> oro);
  bool appendVendorSpec(SPtr<TDUID> duid, int iface, int vendor,
                        SPtr<TOptOptionRequest> reqOpt);
  ...
  void addRelayInfo(SPtr<TIPv6Addr> linkAddr, SPtr<TIPv6Addr> peerAddr, int hop,
  ...
  inline void setMsgType(uint8_t type) { forceMsgType_ = type; }

  /// @brief clears relay information (used in testing only)
  void clearRelayInfo() { RelayInfo_.clear(); }
...
protected:
  ...
  void delFQDN(SPtr<TSrvCfgIface> cfgIface, SPtr<TAddrIA> ptrIA,
               SPtr<TFQDN> fqdn);
  ...
  SPtr<TOptVendorData> RemoteID; // this MAY be set, if message was recevied via
                                 // relay AND relay appended RemoteID
