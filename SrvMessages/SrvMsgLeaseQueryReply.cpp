...
TSrvMsgLeaseQueryReply::TSrvMsgLeaseQueryReply(SPtr<TSrvMsgLeaseQuery> query)
    : TSrvMsg(query->getIface(), query->getRemoteAddr(), LEASEQUERY_REPLY_MSG, query->getTransID()) {
    if (!answer(query)) {
...
void TSrvMsgLeaseQueryReply::appendClientData(SPtr<TAddrClient> cli) {
    ...
    while (ia = cli->getPD()) {
        ...
        while (prefix = ia->getPrefix()) {
            cliData->addOption(
                new TSrvOptIAPrefix(prefix->get(), prefix->getLength(), prefix->getPref(), prefix->getValid(), this));
        }
    }
