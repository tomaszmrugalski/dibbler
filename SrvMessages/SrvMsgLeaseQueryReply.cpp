...
bool TSrvMsgLeaseQueryReply::answer(SPtr<TSrvMsgLeaseQuery> queryMsg) {
    ...
    switch (opt->getOptType()) {
      case OPTION_LQ_QUERY: {
        ...
        switch (q->getQueryType()) {
          ...
          default:
            Options.push_back(
                new TOptStatusCode(STATUSCODE_UNKNOWNQUERYTYPE, "Invalid Query type.", this));
            Log(Warning) << "LQ: Invalid query type (" << q->getQueryType() << " received."
                         << LogEnd;
            return true;
        }
        if (!ok) {
  ...
  if (!count) {
    Options.push_back(
        new TOptStatusCode(STATUSCODE_MALFORMEDQUERY, "Required LQ_QUERY option missing.", this));
    return true;
  }
...
bool TSrvMsgLeaseQueryReply::queryByAddress(SPtr<TSrvOptLQ> q, SPtr<TSrvMsgLeaseQuery> queryMsg) {
  ...
  if (!addr) {
    Options.push_back(
        new TOptStatusCode(STATUSCODE_MALFORMEDQUERY, "Required IAADDR suboption missing.", this));
    return true;
  }
  ...
  if (!cli) {
    Log(Warning) << "LQ: Assignement for client addr=" << addr->getAddr()->getPlain()
                 << " not found." << LogEnd;
    Options.push_back(
        new TOptStatusCode(STATUSCODE_NOTCONFIGURED, "No binding for this address found.", this));
    return true;
  }
...
bool TSrvMsgLeaseQueryReply::queryByClientID(SPtr<TSrvOptLQ> q, SPtr<TSrvMsgLeaseQuery> queryMsg) {
  ...
  if (!duid) {
    Options.push_back(
        new TOptStatusCode(STATUSCODE_UNSPECFAIL, "You didn't send your ClientID.", this));
    return true;
  }
  ...
  if (!cli) {
    Log(Warning) << "LQ: Assignement for client duid=" << duid->getPlain() << " not found."
                 << LogEnd;
    Options.push_back(
        new TOptStatusCode(STATUSCODE_NOTCONFIGURED, "No binding for this DUID found.", this));
    return true;
  }
...
void TSrvMsgLeaseQueryReply::appendClientData(SPtr<TAddrClient> cli) {
  ...
  while (ia = cli->getPD()) {
    ...
    while (prefix = ia->getPrefix()) {
      cliData->addOption(new TSrvOptIAPrefix(prefix->get(), prefix->getLength(), prefix->getPref(),
...
unsigned long TSrvMsgLeaseQueryReply::getTimeout() { return 0; }
void TSrvMsgLeaseQueryReply::doDuties() { IsDone = true; }

string TSrvMsgLeaseQueryReply::getName() const { return "LEASE-QUERY-REPLY"; }
