...
TSrvMsgReconfigure::TSrvMsgReconfigure(int iface, SPtr<TIPv6Addr> clientAddr, int msgType, SPtr<TDUID> clientDuid)
    ...
    if (cli && cli->ReconfKey_.size() > 0) {
        ...
        Log(Debug) << "Auth: Setting reconfigure-key to " << hexToText(&cli->ReconfKey_[0], cli->ReconfKey_.size()) << LogEnd;

        // insert authentication option
        SPtr<TOptAuthentication> optAuth = new TOptAuthentication(AUTH_PROTO_RECONFIGURE_KEY, 1, AUTH_REPLAY_NONE, this);
        TKey tmp(17, 0);
