...
bool TSrvMsgReply::handleConfirmOptions(TOptList & options) {
    ...
    switch (onLink) {
    case ADDRSTATUS_YES: {
        SPtr<TOpt> ptrCode(new TOptStatusCode(STATUSCODE_SUCCESS, "Your addresses are correct for this link! Yay!", this));
    ...
    case ADDRSTATUS_NO: {
        SPtr<TOpt> ptrCode(
            new TOptStatusCode(STATUSCODE_NOTONLINK, "Sorry, those addresses are not valid for this link.", this));
        Options.push_back(ptrCode);
        return true;
    }
...
TSrvMsgReply::TSrvMsgReply(SPtr<TSrvMsgDecline> decline)
    ...
    while (ptrOpt = decline->getOption()) {
        ...
        case OPTION_IA_NA: {
            ...
            if (!ptrIA) {
                Options.push_back(
                    new TSrvOptIA_NA(ptrIA_NA->getIAID(), 0, 0, STATUSCODE_NOBINDING, "No such IA is bound.", this));
                continue;
            }
            ...
            while (subOpt = ptrOpt->getOption()) {
                if (subOpt->getOptType() == OPTION_IAADDR) {
                    ...
                    if (SrvAddrMgr().delClntAddr(ptrClient->getDUID(), ptrIA_NA->getIAID(), addr->getAddr(), false)) {
...
TSrvMsgReply::TSrvMsgReply(SPtr<TSrvMsgRebind> rebind)
    ...
    while (ptrOpt = rebind->getOption()) {
        ...
        case OPTION_IA_NA: {
            ...
            optIA_NA = new TSrvOptIA_NA(rebind_ia, rebind->getRemoteAddr(), ClientDUID, rebind->getIface(), addrCount,
                                        REBIND_MSG, this);
...
TSrvMsgReply::TSrvMsgReply(SPtr<TSrvMsgRelease> release)
    ...
    while (opt = release->getOption()) {
        ...
        case OPTION_IA_NA: {
            ...
            if (!ptrIA) {
                Log(Warning) << "No such IA (iaid=" << clntIA->getIAID() << ") found for client:" << ClientDUID->getPlain()
                             << LogEnd;
                Options.push_back(
                    new TSrvOptIA_NA(clntIA->getIAID(), 0, 0, STATUSCODE_NOBINDING, "No such IA is bound.", this));
                continue;
            }
            ...
            while (subOpt = clntIA->getOption()) {
                ...
                if (SrvAddrMgr().delClntAddr(ClientDUID, clntIA->getIAID(), addr->getAddr(), false)) {
                    ...
                    Log(Warning) << "No such binding found: client=" << ClientDUID->getPlain()
                                 << ", IA (iaid=" << clntIA->getIAID() << "), addr=" << addr->getAddr()->getPlain() << LogEnd;
                };
            };

            // send result to the client
            if (!anyDeleted) {
                SPtr<TOpt> ansIA(new TSrvOptIA_NA(clntIA->getIAID(), clntIA->getT1(), clntIA->getT2(), this));
        ...
        case OPTION_IA_TA: {
            ...
            if (!ptrIA) {
                Log(Warning) << "No such TA (iaid=" << ta->getIAID() << ") found for client:" << ClientDUID->getPlain()
                             << LogEnd;
            ...
            while (subOpt = ta->getOption()) {
                ...
                if (SrvAddrMgr().delTAAddr(ClientDUID, ta->getIAID(), addr->getAddr(), false)) {
                    ...
                    Log(Warning) << "No such binding found: client=" << ClientDUID->getPlain() << ", TA (iaid=" << ta->getIAID()
            ...
            if (!anyDeleted) {
                SPtr<TOpt> answerTA(new TSrvOptTA(ta->getIAID(), STATUSCODE_NOBINDING, "Not every address had binding.", this));
        ...
        case OPTION_IA_PD: {
            ...
            if (!ptrPD) {
                Log(Warning) << "No such PD (iaid=" << pd->getIAID() << ") found for client:" << ClientDUID->getPlain()
                             << LogEnd;
            ...
            while (subOpt = pd->getOption()) {
                ...
                if (SrvAddrMgr().delPrefix(ClientDUID, pd->getIAID(), prefix->getPrefix(), false)) {
                    ...
                    Log(Warning) << "PD: No such binding found: client=" << ClientDUID->getPlain()
                                 << ", PD (iaid=" << pd->getIAID() << "), addr=" << prefix->getPrefix()->getPlain() << LogEnd;
                };
            };
            ...
            if (!anyDeleted) {
                Options.push_back(
                    new TSrvOptIA_PD(pd->getIAID(), 0u, 0u, STATUSCODE_NOBINDING, "Not every address had binding.", this));
            };
            break;
        }
        default:
    ...
    Options.push_back(new TOptStatusCode(STATUSCODE_SUCCESS, "All IAs in RELEASE message were processed.", this));
...
TSrvMsgReply::TSrvMsgReply(SPtr<TSrvMsgRenew> renew)
    ...
    while (ptrOpt = renew->getOption()) {
        switch (ptrOpt->getOptType()) {
        case OPTION_IA_NA: {
            SPtr<TOpt> optIA_NA(new TSrvOptIA_NA(SPtr_cast<TSrvOptIA_NA>(ptrOpt), renew->getRemoteAddr(), ClientDUID,
                                                 renew->getIface(), addrCount, RENEW_MSG, this));
            Options.push_back(optIA_NA);
            break;
        }
        case OPTION_IA_PD: {
            SPtr<TOpt> optPD(new TSrvOptIA_PD(SPtr_cast<TSrvMsg>(renew), SPtr_cast<TSrvOptIA_PD>(ptrOpt), this));
...
TSrvMsgReply::TSrvMsgReply(SPtr<TSrvMsgInfRequest> infRequest)
    : TSrvMsg(infRequest->getIface(), infRequest->getRemoteAddr(), REPLY_MSG, infRequest->getTransID()) {
    ...
    appendMandatoryOptions(ORO);
    if (!appendRequestedOptions(ClientDUID, infRequest->getRemoteAddr(), infRequest->getIface(), ORO)) {
