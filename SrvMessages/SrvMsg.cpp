  ...
  while (pos < bufSize) {
    ...
    if (pos + length > bufSize) {
      Log(Error) << "Malformed option (type=" << code << ", len=" << length
                 << ", offset from beginning of DHCPv6 message="
                 << pos + 4 /* 4=msg-type+trans-id */
    ...
    if (!allowOptInMsg(MsgType, code)) {
      Log(Warning) << "Option " << code << " not allowed in message type=" << MsgType
    ...
    switch (code) {
      ...
      case OPTION_ELAPSED_TIME:
        ptr =
            new TOptInteger(OPTION_ELAPSED_TIME, OPTION_ELAPSED_TIME_LEN, buf + pos, length, this);
        break;
      case OPTION_UNICAST:
      ...
      case OPTION_INFORMATION_REFRESH_TIME:
        ptr = new TOptInteger(OPTION_INFORMATION_REFRESH_TIME, OPTION_INFORMATION_REFRESH_TIME_LEN,
                              buf + pos, length, this);
        break;
      case OPTION_IA_TA:
...
/// Processes options in incoming message and assigns leases and options, if
/// possible. This method may be used in any message that causes server to
/// assign anything, i.e. SOLICIT, REQUEST, RENEW, REBIND (and possibly CONFIRM)
///
...
void TSrvMsg::doDuties() {
  if (!this->getTimeout()) this->IsDone = true;
}
...
void TSrvMsg::addRelayInfo(SPtr<TIPv6Addr> linkAddr, SPtr<TIPv6Addr> peerAddr, int hop,
...
void TSrvMsg::send(int dstPort /* = 0 */) {
  ...
  if (!ptrIface) {
    ...
    if (!cfgIface) {
      Log(Error) << "Can't send message: interface with ifindex=" << this->Iface << " not found."
                 << LogEnd;
      delete[] buf;
      return;
    }
    ...
    if (!ptrIface) {
      Log(Error) << "Can't send message: interface " << cfgIface->getFullName()
                 << " has invalid physical interface defined (ifindex=" << cfgIface->getRelayID()
                 << "." << LogEnd;
      delete[] buf;
      return;
    }
  }
  Log(Notice) << "Sending " << this->getName() << " on " << ptrIface->getFullName() << hex
              << ",transID=0x" << this->getTransID() << dec << ", opts:";
  SPtr<TOpt> ptrOpt;
  this->firstOption();
  while (ptrOpt = this->getOption()) Log(Cont) << " " << ptrOpt->getOptType();
  ...
  if (!RelayInfo_.empty()) {
    ...
    if (RelayInfo_.size() > HOP_COUNT_LIMIT) {
      Log(Error) << "Unable to send message. Got " << RelayInfo_.size() << " relay entries ("
                 << HOP_COUNT_LIMIT << " is allowed maximum." << LogEnd;
      delete[] buf;
      return;
    }
    ...
    Log(Debug) << "Sending " << this->getSize() << "(packet)+" << offset
               << "(relay headers) data on the " << ptrIface->getFullName() << " interface."
               << LogEnd;
  } else {
...
void TSrvMsg::appendReconfigureKey() {
  ...
  if (getOption(OPTION_AUTH)) {
    // yes, there's auth option included already. Let's not use RECONFIGURE_KEY
    // then
    return;
  }
...
void TSrvMsg::processFQDN(SPtr<TSrvMsg> clientMsg, SPtr<TSrvOptFQDN> requestFQDN) {
  ...
  optFQDN = SPtr_cast<TOpt>(
      addFQDN(Iface, requestFQDN, ClientDUID, clntAssignedAddr, hint, doRealUpdate));

  if (optFQDN) Options.push_back(optFQDN);
}
 ...
 * @param doRealUpdate - should the real update be performed (for example if
 * response for SOLICIT is prepare, this should be set to false)
 *
 * @return FQDN option
 */
SPtr<TSrvOptFQDN> TSrvMsg::addFQDN(int iface, SPtr<TSrvOptFQDN> requestFQDN, SPtr<TDUID> clntDuid,
  ...
  Log(Debug) << "Requesting FQDN for client with DUID=" << clntDuid->getPlain()
  ...
  Log(Debug) << "FQDN: Adding FQDN Option in REPLY message: " << fqdnName
             << ", FQDNMode=" << FQDNMode << LogEnd;

  if (FQDNMode == 1 || FQDNMode == 2) {
    // Log(Debug) << "FQDN: Server configuration allows DNS updates for " <<
    // clntDuid->getPlain()
    //           << LogEnd;
    ...
    if (doRealUpdate) {
      Log(Notice) << "FQDN: About to perform DNS Update: IP=" << addr->get()->getPlain()
                  << " and FQDN=" << fqdnName << LogEnd;
      SrvIfaceMgr().addFQDN(cfgIface->getID(), DNSAddr, addr->get(), fqdnName);
    } else {
      Log(Debug) << "FQDN: Skipping DNS Update." << LogEnd;
    }
  } else {
    Log(Debug) << "Server configuration does NOT allow DNS updates for " << clntDuid->getPlain()
               << LogEnd;
...
void TSrvMsg::delFQDN(SPtr<TSrvCfgIface> cfgIface, SPtr<TAddrIA> ptrIA, SPtr<TFQDN> fqdn) {
  ...
  if (!addrAddr) {
    Log(Error) << "Client does not have any addresses asigned to IA (IAID=" << ptrIA->getIAID()
               << ")." << LogEnd;
    return;
  }
...
void TSrvMsg::copyRemoteID(SPtr<TSrvMsg> q) { this->RemoteID = q->getRemoteID(); }
...
void TSrvMsg::copyRelayInfo(SPtr<TSrvMsg> q) { RelayInfo_ = q->RelayInfo_; }
...
void TSrvMsg::appendAuthenticationOption(SPtr<TDUID> duid) {
...
#ifndef MOD_DISABLE_AUTH
  if (!duid) {
    Log(Error) << "Auth: No duid! Probably internal error. Authentication "
                  "option not appended."
               << LogEnd;
    return;
  }
  ...
  switch (SrvCfgMgr().getAuthProtocol()) {
    ...
    case AUTH_PROTO_DELAYED: {
      ...
      if (!key_id) {
        Log(Info) << "AUTH: no key-id specified for client with DUID " << duid->getPlain()
                  << LogEnd;
        return;
      }
      setSPI(key_id);
    ...
    case AUTH_PROTO_RECONFIGURE_KEY:
      ...
      if (MsgType != RECONFIGURE_MSG) return;
    case AUTH_PROTO_DIBBLER:
      ...
      Log(Debug) << "Auth: Dibbler protocol, setting digest type to " << getDigestName(DigestType_)
                 << LogEnd;

      SPtr<TAddrClient> client = SrvAddrMgr().getClient(duid);
      if (client && !client->getSPI() && getSPI()) client->setSPI(getSPI());

      if (getSPI() == 0) {
        Log(Info) << "Auth: no key selected (SPI=0) for this message, will not "
                     "include AUTH option."
                  << LogEnd;
        return;
      }
  ...
  if (!getOption(OPTION_AUTH)) {
    SPtr<TOptAuthentication> auth = new TOptAuthentication(SrvCfgMgr().getAuthProtocol(), algo,
                                                           SrvCfgMgr().getAuthReplay(), this);
 ...
 * @param reqOpts
 *
 * @return true, if any options (conveying configuration paramter) has been
 * appended
 */
bool TSrvMsg::appendRequestedOptions(SPtr<TDUID> duid, SPtr<TIPv6Addr> addr, int iface,
  ...
  if (!reqOpts->count()) return false;

  SPtr<TSrvCfgIface> ptrIface = SrvCfgMgr().getIfaceByID(iface);
  if (!ptrIface) {
    Log(Error) << "Unable to find interface (ifindex=" << iface << "). Something is wrong."
               << LogEnd;
    return false;
  }
  ...
  if (reqOpts->isOption(OPTION_VENDOR_OPTS)) {
    if (appendVendorSpec(duid, iface, 0, reqOpts)) newOptionAssigned = true;
  }
  ...
  if (!ex || !extraOpts.size()) extraOpts = ptrIface->getExtraOptions();

  if (!ex || !forcedOpts.size()) forcedOpts = ptrIface->getForcedOptions();
  ...
  // --- option: KEYGEN ---
#ifndef MOD_DISABLE_AUTH
  if (reqOpts->isOption(OPTION_KEYGEN) &&
      SrvCfgMgr().getDigest() != DIGEST_NONE) {  // && this->MsgType == ADVERTISE_MSG ) {
...
bool TSrvMsg::check(bool clntIDmandatory, bool srvIDmandatory) {
  ...
  if (optSrvID) {
    if (!(*(SrvCfgMgr().getDUID()) == *(optSrvID->getDUID()))) {
      Log(Debug) << "Wrong ServerID value detected. This message is not for "
                    "me. Message ignored."
                 << LogEnd;
      return false;
    }
...
bool TSrvMsg::appendVendorSpec(SPtr<TDUID> duid, int iface, int vendor,
  ...
  if (!vsLst.count()) vsLst = ptrIface->getVendorSpecLst(vendor);
...
///
/// @return true if message is ok (false if it is replayed and should be
/// dropped)
...
void TSrvMsg::setRemoteID(SPtr<TOptVendorData> remoteID) { RemoteID = remoteID; }

SPtr<TOptVendorData> TSrvMsg::getRemoteID() { return RemoteID; }

/**
 * copy status-code to top-level if something is wrong (i.e.
 * status-code!=SUCCESS)
 *
 */
      ...
      case OPTION_IA_NA: {
        if (optLevel = SPtr_cast<TOptStatusCode>(opt->getOption(OPTION_STATUS_CODE))) {
          if (optLevel->getCode() != STATUSCODE_SUCCESS) {
            ...
            SPtr<TOpt> rootLevel(
                new TOptStatusCode(optLevel->getCode(), optLevel->getText(), this));
...
void TSrvMsg::setPhysicalIface(int iface) { physicalIface_ = iface; }

/// @brief returns physical interface index
///
/// This may be different than Iface_ for relayed messages
int TSrvMsg::getPhysicalIface() const { return physicalIface_; }
