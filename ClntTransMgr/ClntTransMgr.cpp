...
TClntTransMgr::TClntTransMgr(const std::string &config)
  ...
  while (iface = ClntCfgMgr().getIface()) {
    if (!populateAddrMgr(iface)) return;
...
bool TClntTransMgr::populateAddrMgr(SPtr<TClntCfgIface> iface) {
  ...
  while (ia = iface->getIA()) {
    if (ClntAddrMgr().getIA(ia->getIAID()))
      continue;  // there is such IA already - read from disk cache
                 // (client-AddrMgr.xml)
    SPtr<TAddrIA> addrIA =
        new TAddrIA(iface->getName(), iface->getID(), IATYPE_IA, SPtr<TIPv6Addr>(), SPtr<TDUID>(),
  ...
  if ((ta = iface->getTA()) && (!ClntAddrMgr().getTA(ta->getIAID()))) {
    ...
    SPtr<TAddrIA> addrTA =
        new TAddrIA(iface->getName(), iface->getID(), IATYPE_TA, SPtr<TIPv6Addr>(), SPtr<TDUID>(),
                    DHCPV6_INFINITY, DHCPV6_INFINITY, ta->getIAID());
  ...
  while (pd = iface->getPD()) {
    if (ClntAddrMgr().getPD(pd->getIAID()))
      continue;  // there is such IA already - read from disk cache
                 // (client-AddrMgr.xml)
    SPtr<TAddrIA> addrPD =
        new TAddrIA(iface->getName(), iface->getID(), IATYPE_PD, SPtr<TIPv6Addr>(), SPtr<TDUID>(),
                    pd->getT1(), pd->getT2(), pd->getIAID());
...
bool TClntTransMgr::openSockets(SPtr<TClntCfgIface> iface) {
  ...
  if (iface->noConfig()) return true;

  // open socket
  SPtr<TIfaceIface> realIface = ClntIfaceMgr().getIfaceByID(iface->getID());
  if (!realIface) {
    Log(Error) << "Interface " << iface->getFullName() << " not present in system." << LogEnd;
    return false;
  }
  if (!realIface->flagUp()) {
    Log(Error) << "Interface " << realIface->getFullName() << " is down. Unable to open socket."
               << LogEnd;
    return false;
  }
  if (!realIface->flagRunning()) {
    Log(Error) << "Interface " << realIface->getFullName() << " is not running." << LogEnd;
    return false;
  }
  ...
  Log(Notice) << "Creating socket (addr=" << *addr << ") on " << iface->getFullName()
              << " interface." << LogEnd;
  if (!realIface->addSocket(addr, DHCPCLIENT_PORT, true, this->BindReuse)) {
    Log(Crit) << "Socket creation (addr=" << *addr << ") on " << iface->getFullName()
              << " interface failed." << LogEnd;
    return false;
  }
...
void TClntTransMgr::removeExpired() {
  ...
  if (ClntAddrMgr().getValidTimeout()) return;
  ...
  while (ptrIA = ClntAddrMgr().getIA()) {
    if (ptrIA->getValidTimeout()) continue;

    ptrIA->firstAddr();
    while (ptrAddr = ptrIA->getAddr()) {
      if (ptrAddr->getValidTimeout()) continue;
      ptrIface = ClntIfaceMgr().getIfaceByID(ptrIA->getIfindex());
      Log(Warning) << "Address " << ptrAddr->get()->getPlain() << " assigned to the "
                   << ptrIface->getFullName() << " interface (in IA_NA " << ptrIA->getIAID()
                   << ") has expired." << LogEnd;
      ...
      Log(Info) << "Expired address " << ptrAddr->get()->getPlain() << " from IA "
                << ptrIA->getIAID() << " has been removed from addrDB." << LogEnd;
    }
  ...
  while (ptrIA = ClntAddrMgr().getTA()) {
    ...
    while (ptrAddr = ptrIA->getAddr()) {
      ...
      Log(Warning) << "Temporary address " << ptrAddr->get()->getPlain() << " assigned to the "
                   << ptrIface->getFullName() << " interface (in IA_TA " << ptrIA->getIAID()
                   << ") has expired." << LogEnd;
      ...
      // be removed in AddrMgr::doDuties() anyway
      ptrIA->delAddr(ptrAddr->get());
      Log(Info) << "Expired temporary address " << ptrAddr->get()->getPlain() << " from IA "
                << ptrIA->getIAID() << " has been removed from addrDB." << LogEnd;
    }
  ...
  while (ptrPD = ClntAddrMgr().getPD()) {
    ...
    while (ptrPrefix = ptrPD->getPrefix()) {
      ...
      Log(Info) << "Expired prefix " << ptrPrefix->get()->getPlain() << "/"
                << ptrPrefix->getLength() << " from IA_PD " << ptrPD->getIAID()
...
/**
 * checks if loaded Address database is sane (i.e. does not reffer to
  ...
  while (ptrIA = ClntAddrMgr().getIA()) {
    ...
    if (!iface) {
      Log(Warning) << "IA (iaid=" << ptrIA->getIAID()
                   << ") loaded from old file, but currently there is no iface "
                      "with ifindex="
                   << ptrIA->getIfindex();
      // IA with non-existent iface, purge iface
      ptrIA->firstAddr();
      while (ptrAddr = ptrIA->getAddr()) ptrIA->delAddr(ptrAddr->get());
...
bool TClntTransMgr::openLoopbackSocket() {
  ...
  // if BindReuse is set, return true from here and dont create new loopback
  // socket to allow multiple instances
  if (this->BindReuse) return true;
...
void TClntTransMgr::doDuties() {
  ...
  while (msg = Transactions.get()) {
    if ((!msg->getTimeout()) && (!msg->isDone())) {
      Log(Info) << "Processing msg (" << msg->getName() << ",transID=0x" << hex << msg->getTransID()
                << dec << ",opts:";
      SPtr<TOpt> ptrOpt;
      msg->firstOption();
  ...
  Transactions.first();
  while (msg = Transactions.get()) {
    if (msg->isDone()) Transactions.del();
  }
  ...
  // This method launch the DNS update, so the checkDecline has to be done
  // before to ensure the ip address is valid
  ClntIfaceMgr().doDuties();

  if (this->Shutdown && !Transactions.count()) this->IsDone = true;
}
...
void TClntTransMgr::shutdown() {
  ...
  while (ClntAddrMgr().countIA()) {
    ...
    while (pd = ClntAddrMgr().getPD()) {
      ...
      if (cfgPD) cfgPD->setState(STATE_DISABLED);
    }
  ...
  if (releasedPDs.count()) this->sendRelease(releasedIAs, SPtr<TAddrIA>(), releasedPDs);
...
///
/// Verifies that received message is valid. Depending on received option, it
/// will send RENEW, INF-REQUEST or REBIND message
///
...
void TClntTransMgr::handleReconfigure(SPtr<TClntMsg> reconfMsg) {
  ...
  // if yes, send specific message, e.g. call sendRenew(), sendRebind() or
  // sendInfRequest()
...
unsigned long TClntTransMgr::getTimeout() {
  ...
  while (ptrMsg = Transactions.get()) {
    if (tmp > ptrMsg->getTimeout()) tmp = ptrMsg->getTimeout();
  }
  if (timeout > tmp) timeout = tmp;

  if (ClntCfgMgr().inactiveIfacesCnt()) {
    if (timeout > INACTIVE_MODE_INTERVAL) timeout = INACTIVE_MODE_INTERVAL;
  }
...
void TClntTransMgr::sendRequest(TOptList requestOptions, int iface) {
  ...
  for (TOptList::iterator opt = requestOptions.begin(); opt != requestOptions.end(); ++opt) {
    if (!allowOptInMsg(REQUEST_MSG, (*opt)->getOptType()) || (*opt)->getOptType() == OPTION_AUTH)
...
void TClntTransMgr::sendRenew() {
  ...
  Log(Info) << "Generating RENEW for " << iaLst.count() << " IA(s) and " << pdLst.count()
            << " PD(s). " << LogEnd;
...
void TClntTransMgr::sendRelease(List(TAddrIA) IALst, SPtr<TAddrIA> ta, List(TAddrIA) pdLst) {
  ...
  Log(Notice) << "Creating RELEASE for " << IALst.count() << " IA(s), " << pdLst.count()
              << " PD(s), " << (ta ? " and TA" : " (no TA)") << " on the "
...
void TClntTransMgr::sendRebind(TOptList requestOptions, int iface) {
  ...
  for (TOptList::iterator opt = requestOptions.begin(); opt != requestOptions.end(); ++opt) {
    if (!allowOptInMsg(REBIND_MSG, (*opt)->getOptType())) opt = requestOptions.erase(opt);
  }

  SPtr<TClntMsg> ptr = new TClntMsgRebind(requestOptions, iface);
  if (!ptr->isDone()) Transactions.append(ptr);
}
...
void TClntTransMgr::checkSolicit() {
  ...
  ClntCfgMgr().firstIface();
  while ((iface = ClntCfgMgr().getIface())) {
    if (iface->noConfig()) continue;

    if (iface->stateless()) continue;
    ...
    while (cfgIA = iface->getIA()) {
      ...
      if (cfgIA->getState() == STATE_NOTCONFIGURED) iaLst.append(cfgIA);
    }
    ...
    if (iface->countTA()) {
      iface->firstTA();
      ta = iface->getTA();
      if (ta->getState() != STATE_NOTCONFIGURED) ta.reset();
    }
    ...
    if (iaLst.count() || ta || pdLst.count()) {
      Log(Info) << "Creating SOLICIT message with " << iaLst.count() << " IA(s), "
                << (ta ? "1" : "no") << " TA and " << pdLst.count() << " PD(s)";
      if (iface->getRapidCommit()) {
        Log(Cont) << " (with rapid-commit)";
      }
      Log(Cont) << " on " << iface->getFullName() << " interface." << LogEnd;
      Transactions.append(new TClntMsgSolicit(iface->getID(), SPtr<TIPv6Addr>(), iaLst, ta, pdLst,
...
void TClntTransMgr::checkConfirm() {
  ...
  while (iface = ClntCfgMgr().getIface()) {
    ...
    while (cfgIA = iface->getIA()) {
      ClntAddrMgr().firstIA();
      while (ptrIA = ClntAddrMgr().getIA()) {
        if (ptrIA->getState() != STATE_CONFIRMME) continue;
    ...
    if (IALst.count()) {
      Log(Info) << "Creating CONFIRM: " << IALst.count() << " IA(s) on " << iface->getFullName()
                << LogEnd;
      Transactions.append(new TClntMsgConfirm(iface->getID(), IALst));
...
void TClntTransMgr::checkInfRequest() {
  ...
  while ((iface = ClntCfgMgr().getIface())) {
    ...
    if (!ifaceIface->getTimeout()) {
      ...
      if (iface->getDomainState() == STATE_CONFIGURED) iface->setDomainState(STATE_NOTCONFIGURED);
      ...
      if (iface->getFQDNState() == STATE_CONFIGURED) iface->setFQDNState(STATE_NOTCONFIGURED);
    ...
    if ((iface->getDNSServerState() == STATE_NOTCONFIGURED) ||
      ...
      Log(Info) << "Creating INFORMATION-REQUEST message on " << iface->getFullName()
                << " interface." << LogEnd;
...
void TClntTransMgr::checkRenew() {
  ...
  if (ClntAddrMgr().getT1Timeout() > 0) return;
  ...
  // Need to be fixed:?? how to deal with mutiple network interfaces.
  while (ia = ClntAddrMgr().getIA()) {
    if ((ia->getT1Timeout() != 0) || (ia->getState() != STATE_CONFIGURED) ||
    ...
    if ((ia->getIfindex() == iaPattern->getIfindex()) && (ia->getDUID() == iaPattern->getDUID())) {
      iaLst.append(ia);
      ia->setState(STATE_INPROCESS);
    }
  }
  ...
  ClntAddrMgr().firstPD();
  while (ia = ClntAddrMgr().getPD()) {
    if ((ia->getT1Timeout() != 0) || (ia->getState() != STATE_CONFIGURED)) continue;
...
void TClntTransMgr::checkDecline() {
  ...
  do {
    ...
    while ((ptrIA = ClntAddrMgr().getIA()) && (!firstIA)) {
      if (ptrIA->getTentative() == ADDRSTATUS_YES) firstIA = ptrIA;
    }
    if (firstIA) {
...
void TClntTransMgr::checkRequest() {
  ...
  while (ia = ClntAddrMgr().getIA()) {
    ...
    if ((cfgIA->getState() != STATE_CONFIGURED)) continue;
    ...
    if (ia->countAddr() >= cfgIA->countAddr()) continue;

    Log(Info) << "IA (iaid=" << ia->getIAID() << ") is configured to get " << cfgIA->countAddr()
              << " addr, but server provided " << ia->countAddr() << ", REQUEST will be sent."
              << LogEnd;
...
bool TClntTransMgr::isDone() { return IsDone; }

char *TClntTransMgr::getCtrlAddr() { return CtrlAddr_; }

int TClntTransMgr::getCtrlIface() { return CtrlIface_; }

TClntTransMgr::~TClntTransMgr() { Log(Debug) << "ClntTransMgr cleanup." << LogEnd; }

void TClntTransMgr::addAdvertise(SPtr<TClntMsg> advertise) { AdvertiseLst.append(advertise); }

void TClntTransMgr::firstAdvertise() { AdvertiseLst.first(); }

SPtr<TClntMsg> TClntTransMgr::getAdvertise() { return AdvertiseLst.get(); }

SPtr<TOpt> TClntTransMgr::getAdvertiseDUID() {
  if (!AdvertiseLst.count()) return TOptPtr();  // NULL
  AdvertiseLst.first();
...
int TClntTransMgr::getAdvertiseLstCount() { return AdvertiseLst.count(); }

int TClntTransMgr::getMaxPreference() {
  if (AdvertiseLst.count() == 0) return -1;
  int max = 0;

  SPtr<TClntMsgAdvertise> ptr;
  AdvertiseLst.first();
  while (ptr = SPtr_cast<TClntMsgAdvertise>(AdvertiseLst.get())) {
    if (max < ptr->getPreference()) max = ptr->getPreference();
  }
  return max;
}
...
void TClntTransMgr::sortAdvertiseLst() {
  ...
  while (AdvertiseLst.count()) {
    ...
    AdvertiseLst.first();
    while (ptr = SPtr_cast<TClntMsgAdvertise>(AdvertiseLst.get())) {
      if (ptr->getPreference() == max) break;
    }

    // did we find it? Then append it on the end of sorted list, and delete from
    // this new.
    if (ptr) {
...
void TClntTransMgr::printAdvertiseLst() { printLst(AdvertiseLst); }
...
bool TClntTransMgr::sanitizeAddrDB() {
  ...
  return ClntAddrMgr().updateInterfacesInfo(currentNameToIndex, currentIndexToName);
}
...
SPtr<TClntTransMgr::TNeighborInfo> TClntTransMgr::neighborInfoGet(int transid) {
  for (TNeighborInfoLst::iterator it = Neighbors.begin(); it != Neighbors.end(); ++it) {
    if ((*it)->transid == transid) return *it;
  }
...
bool TClntTransMgr::updateNeighbors(int ifindex, SPtr<TOptAddrLst> neighbors) {
  ...
  while (addr = neighbors->getAddr()) {
    ...
    if (info) continue;  // this neighbor is already known
...
bool TClntTransMgr::checkRemoteSolicits() {
  ...
  if (!ClntAddrMgr().getPreferredAddr()) {
    // There are no preferred (non-tentative) addresses. Skipping remote
    // solicit.
    return false;
  }

  // There is preferred address: " <<
  // ClntAddrMgr().getPreferredAddr()->getPlain()

  for (TNeighborInfoLst::iterator it = Neighbors.begin(); it != Neighbors.end(); ++it) {
    if ((*it)->state != TNeighborInfo::NeighborInfoState_Added) continue;
...
bool TClntTransMgr::sendRemoteSolicit(SPtr<TNeighborInfo> neighbor) {
  ...
  ClntCfgMgr().firstIface();
  while ((iface = ClntCfgMgr().getIface())) {
    if (iface->getID() == neighbor->ifindex) break;
  }
  if (!iface) {
  ...
  // ignore TA & PD
  Log(Info) << "Creating remote SOLICIT to be transmitted to " << neighbor->srvAddr->getPlain()
            << LogEnd;
  SPtr<TClntCfgTA> ta;     // use empty pointer
  List(TClntCfgPD) pdLst;  // use empty list

  SPtr<TClntMsg> solicit =
      new TClntMsgSolicit(neighbor->ifindex, neighbor->srvAddr, iaLst, ta, pdLst,
                          true /*rapid-commit */, true /* remote autoconf*/);
...
bool TClntTransMgr::processRemoteReply(SPtr<TClntMsg> reply) {
  ...
  if (!neigh) {
    Log(Error) << "Failed to match transmitted remote SOLICIT. Seems like "
                  "bogus remote REPLY."
               << LogEnd;
    return false;
  }
  ...
  while (sol = Transactions.get()) {
    if (sol->getTransID() == neigh->transid) sol->isDone(true);
  }
