...
SPtr<TDUID> TFQDN::getDuid() { return Duid_; }

SPtr<TIPv6Addr> TFQDN::getAddr() { return Addr_; }

string TFQDN::getName() { return Name_; }

bool TFQDN::isUsed() { return Used_; }

void TFQDN::setUsed(bool used) { Used_ = used; }
...
ostream &operator<<(ostream &out, TFQDN &fqdn) {
  ...
  out << " used=\"" << (fqdn.Used_ ? "TRUE" : "FALSE") << "\">" << fqdn.getName() << "</fqdn>"
      << endl;
  return out;
}
