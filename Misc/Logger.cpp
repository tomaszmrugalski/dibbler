...
ostream & logCommon(int x) {
    ...
    static char colors[][10] = {"\033[31m", "\033[31m", "\033[31m", "\033[31m", "\033[33m", "\033[30m", "\033[30m", "\033[37m"};

    logger::curLogEntry = x;

#ifdef LINUX
    static int syslogLevel[] = {LOG_EMERG, LOG_ALERT, LOG_CRIT, LOG_ERR, LOG_WARNING, LOG_NOTICE, LOG_INFO, LOG_DEBUG};
...
ostream & logEmerg() {
    return logger::logCommon(1);
}
ostream & logAlert() {
    return logger::logCommon(2);
}
ostream & logCrit() {
    return logger::logCommon(3);
}
ostream & logError() {
    return logger::logCommon(4);
}
ostream & logWarning() {
    return logger::logCommon(5);
}
ostream & logNotice() {
    return logger::logCommon(6);
}
ostream & logInfo() {
    return logger::logCommon(7);
}
ostream & logDebug() {
    return logger::logCommon(8);
}
...
static void changeLogMode(Elogmode newMode) {
    if (newMode != logger::logmode) {
        if (logger::logFileMode && ((newMode == LOGMODE_FULL) || (newMode == LOGMODE_SHORT) || (newMode == LOGMODE_PRECISE)))
