...
TIPv6Addr::TIPv6Addr(const char *prefix, const char *host, int prefixLength) {
  ...
  Addr[offset] = (prefix[offset] & truncRight[prefixLength % 8]) |
                 (host[offset] & truncLeft[prefixLength % 8]);
  inet_ntop6(Addr, Plain);
}

bool TIPv6Addr::linkLocal() {
  if (this->Addr[0] == 0xfe && this->Addr[1] == 0x80) return true;
  return false;
}

bool TIPv6Addr::multicast() { return Addr[0] == 0xff; }

char *TIPv6Addr::getAddr() { return Addr; }
...
bool TIPv6Addr::operator==(const TIPv6Addr &other) { return !memcmp(this->Addr, other.Addr, 16); }
...
void TIPv6Addr::truncate(int minPrefix, int maxPrefix) {
  ...
  if (minPrefix > 128 || minPrefix < 0 || maxPrefix > 128 || maxPrefix < 0) {
    Log(Error) << "Unable to truncate address: invalid prefix lengths: minPrefix=" << minPrefix
               << ", maxPrefix=" << maxPrefix << LogEnd;
    return;
  }
