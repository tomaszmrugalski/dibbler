...
#define DIBBLER_COPYRIGHT1                                                     \
  "| Dibbler - a portable DHCPv6, version " DIBBLER_VERSION
#define DIBBLER_COPYRIGHT2                                                     \
  "| Authors : Tomasz Mrugalski<thomson(at)klub.com.pl>,Marek "                \
  "Senderski<msend(at)o2.pl>"
#define DIBBLER_COPYRIGHT3                                                     \
...
/* due to poor type usage (char* instead of uint8_t*), we need to stick with
   char* for now. Changing to uint8_t would require rewriting large parts of the
   code */
...
/* This constant defines whether addresses configured on Linux are added with
   ...
   Since adding the address without lifetimes in a safer bet (the Duplicate
   Address Detection will work), this is the default. If you really need to add
   adresses
...
struct link_state_notify_t {
  int ifindex[MAX_LINK_STATE_CHANGES_AT_ONCE]; /* indexes of interfaces that has
                                                  changed.
                                                  Only non-zero values will be
                                                  used */
...
extern int sock_add(char *ifacename, int ifaceid, char *addr, int port,
                    int thisifaceonly, int reuse);
extern int sock_del(int fd);
extern int sock_send(int fd, char *addr, char *buf, int buflen, int port,
                     int iface);
extern int sock_recv(int fd, char *myPlainAddr, char *peerPlainAddr, char *buf,
                     int buflen);
 ...
 * @param mac_len specifies MAC buffer length. Must be set so number of bytes
 * set by the function
 *
 * @return 0 if successful, negative values if there are errors
 */
extern int get_mac_from_ipv6(const char *iface_name, int ifindex,
                             const char *v6addr, char *mac, int *mac_len);
...
extern void
link_state_change_init(volatile struct link_state_notify_t *changed_links,
                       volatile int *notify);
...
extern int sipserver_add(const char *ifname, int ifindex,
                         const char *addrPlain);
extern int sipserver_del(const char *ifname, int ifindex,
                         const char *addrPlain);
...
extern int nisserver_add(const char *ifname, int ifindex,
                         const char *addrPlain);
extern int nisserver_del(const char *ifname, int ifindex,
                         const char *addrPlain);
...
extern int nisplusserver_add(const char *ifname, int ifindex,
                             const char *addrPlain);
extern int nisplusserver_del(const char *ifname, int ifindex,
                             const char *addrPlain);
extern int nisplusdomain_set(const char *ifname, int ifindex,
                             const char *domain);
extern int nisplusdomain_del(const char *ifname, int ifindex,
                             const char *domain);

extern int prefix_add(const char *ifname, int ifindex, const char *prefixPlain,
                      int prefixLength, unsigned long prefered,
                      unsigned long valid);
extern int prefix_update(const char *ifname, int ifindex,
                         const char *prefixPlain, int prefixLength,
                         unsigned long prefered, unsigned long valid);
extern int prefix_del(const char *ifname, int ifindex, const char *prefixPlain,
                      int prefixLength);

char *getAAAKey(uint32_t SPI, uint32_t *len); /* reads AAA key from a file */
char *
getAAAKeyFilename(uint32_t SPI); /* which file? use this function to find out */
 ...
 * @return LOWLEVEL_NO_ERROR if successful, appropriate LOWLEVEL_ERROR_*
 * otherwise
 */
