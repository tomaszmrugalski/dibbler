...
char * inet_ntop4(const char * src, char * dst) {
    ...
    sprintf(tmp, "%u.%u.%u.%u", (const unsigned char)src[0], (const unsigned char)src[1], (const unsigned char)src[2],
...
uint64_t readUint64(const BUFFER_TYPE * buf) {
    uint64_t value = (((uint64_t)(buf[0])) << 56) + (((uint64_t)(buf[1])) << 48) + (((uint64_t)(buf[2])) << 40) +
