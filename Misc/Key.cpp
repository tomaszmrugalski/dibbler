...
bool TSIGKey::setData(const std::string &base64encoded) {
  ...
  if (!status) {
    Log(Warning) << "Failed to base64-decode key. There are trailing unparsed "
                    "characters."
                 << LogEnd;
  }
...
std::string TSIGKey::getPackedData() { return Data_; }

std::string TSIGKey::getBase64Data() { return Base64Data_; }
