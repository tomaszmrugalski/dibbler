...
void TDHCPRelay::run() {
  ...
  while ((!isDone()) && (!RelTransMgr().isDone())) {
    ...
    Log(Notice) << "Received " << msg->getName() << " on " << ptrIface->getName() << "/" << iface;
    ...
    while (ptrOpt = msg->getOption()) {
      ...
      // uncomment this to get detailed info about option lengths Log(Cont) <<
      // "/" << ptrOpt->getSize();
    }
...
bool TDHCPRelay::isDone() { return this->IsDone; }
