...
/*
 * returns true if IfaceMgr has finished it's job (failed to get info about
 * interfaces)
 */
bool TIfaceMgr::isDone() { return IsDone; }

/*
 * rewinds interface list to first interface
 */
void TIfaceMgr::firstIface() { IfaceLst.first(); }

/*
 * gets next interface on list
 */
SPtr<TIfaceIface> TIfaceMgr::getIface() { return IfaceLst.get(); }
...
SPtr<TIfaceIface> TIfaceMgr::getIfaceByName(const std::string &name) {
  ...
  while (ptr = IfaceLst.get()) {
    if (!strcmp(name.c_str(), ptr->getName())) return ptr;
  }
...
SPtr<TIfaceIface> TIfaceMgr::getIfaceBySocket(int fd) {
  ...
  IfaceLst.first();
  while (ptr = IfaceLst.get()) {
    if (ptr->getSocketByFD(fd)) return ptr;
  }
  return SPtr<TIfaceIface>();  // NULL
}
...
int TIfaceMgr::select(unsigned long time, char *buf, int &bufsize, SPtr<TIPv6Addr> peer,
                      SPtr<TIPv6Addr> myaddr) {
  struct timeval czas;
  int result;
  if (time > DHCPV6_INFINITY / 2) time /= 2;

#ifdef BSD
  // For some reason, Darwin kernel doesn't like too large timeout values
  if (time > DHCPV6_INFINITY / 4) time = 3600 * 24 * 7;  // a week is enough
#endif
  ...
  if (result < 0) {
    ...
    Log(Debug) << "Failed to read sockets (select() returned " << result << "), error=" << buf
               << LogEnd;
    return -1;
  }
  ...
  // check if we've received data addressed to us. There's problem with sockets
  // binding. If there are 2 open sockets (one bound to multicast and one to
  // global address), each packet sent on multicast address is also received on
  // unicast socket.
  char anycast[16] = {0};
  if (!iface->flagLoopback() && memcmp(sock->getAddr()->getAddr(), myAddrPacked, 16) &&
      memcmp(sock->getAddr()->getAddr(), anycast, 16)) {
    Log(Debug) << "Received data on address " << myPlainAddr << ", expected " << *sock->getAddr()
               << ", message ignored." << LogEnd;
    bufsize = 0;
    return -1;
  }
#endif
 ...
 */
int TIfaceMgr::countIface() { return IfaceLst.count(); }
 ...
 */
TIfaceMgr::~TIfaceMgr() { closeSockets(); }
  ...
  switch (opt->getOptType()) {
    ...
    case OPTION_IA_PD: {
      ...
      while (SPtr<TOpt> subopt = opt->getOption()) {
        if (subopt->getOptType() == OPTION_IAPREFIX) {
          ...
          params.addPrefix(prefix->getPrefix(), prefix->getPrefixLength(), prefix->getPref(),
                           prefix->getValid());
        }
      }
      break;
    ...
    case OPTION_NEXT_HOP: {
      if (opt->countOption()) {
        ...
        opt->firstOption();
        while (SPtr<TOpt> subopt = opt->getOption()) {
          if (subopt->getOptType() != OPTION_RTPREFIX) continue;  // ignore other options
          params.addParam("OPTION_NEXT_HOP_RTPREFIX", opt->getPlain() + " " + subopt->getPlain());
        }
      } else {
        // no suboptions, just NEXT_HOP (default router, without ::/0 route
        // specified) Will define something like this:
        // OPTION_NEXT_HOP=2001:db8:1::1
...
void TIfaceMgr::notifyScript(const std::string &scriptName, std::string action,
  ...
  Log(Debug) << "About to execute " << scriptName << " script, " << params.envCnt << " variables."
             << LogEnd;
...
void TIfaceMgr::notifyScripts(const std::string &scriptName, SPtr<TMsg> question, SPtr<TMsg> reply,
  ...
  if (!iface) {
    Log(Error) << "Unable to find interface with ifindex=" << ifindex << ". Script NOT called."
               << LogEnd;
    return;
  }
...
// --------------------------------------------------------------------
/*
 * just flush this data in XML format (used for logging and debugging purposes
 * only)
 */
