 ...
 */
TIfaceIface::TIfaceIface(const char *name, int id, unsigned int flags, char *mac, int maclen,
                         char *llAddr, int llAddrCnt, char *globalAddr, int globalCnt, int hwType)
 ...
 */
char *TIfaceIface::getName() { return this->Name; }

/*
 * returns interface ID
 */
int TIfaceIface::getID() { return this->ID; }

string TIfaceIface::getFullName() {
  ostringstream oss;
  oss << this->ID;
  return string(this->Name) + "/" + oss.str();
}

/*
 * return interface flags
 */
unsigned int TIfaceIface::getFlags() { return this->Flags; }
 ...
 */
bool TIfaceIface::flagUp() { return this->Flags & IFF_UP; }

/**
 * returns true if interface is RUNNING
 */
bool TIfaceIface::flagRunning() { return (bool)(this->Flags & IFF_RUNNING); }

/**
 * returns true is interface is MULTICAST capable
 */
bool TIfaceIface::flagMulticast() { return (Flags & IFF_MULTICAST) ? true : false; }

/**
 * returns true is interface is LOOPBACK
 */
bool TIfaceIface::flagLoopback() { return (Flags & IFF_LOOPBACK) ? true : false; }

/**
 * returns MAC length
 */
int TIfaceIface::getMacLen() { return Maclen; }

/**
 * returns MAC
 */
char *TIfaceIface::getMac() { return Mac; }

void TIfaceIface::firstGlobalAddr() { GlobalAddrLst.first(); }

SPtr<TIPv6Addr> TIfaceIface::getGlobalAddr() { return this->GlobalAddrLst.get(); }

unsigned int TIfaceIface::countGlobalAddr() { return this->GlobalAddrLst.count(); }

void TIfaceIface::addGlobalAddr(SPtr<TIPv6Addr> addr) { this->GlobalAddrLst.append(addr); }
 ...
 * returns HW type
 */
int TIfaceIface::getHardwareType() { return this->HWType; }
...
bool TIfaceIface::addAddr(SPtr<TIPv6Addr> addr, long pref, long valid, int prefixLen) {
  Log(Notice) << "Address " << addr->getPlain() << "/" << prefixLen << " added to " << getFullName()
              << " interface." << LogEnd;
...
bool TIfaceIface::updateAddr(SPtr<TIPv6Addr> addr, long pref, long valid) {
  int result;
  Log(Notice) << "Address " << addr->getPlain() << " updated on " << getFullName() << " interface."
              << LogEnd;

  result = ipaddr_update((char *)this->Name, this->ID, (char *)addr->getPlain(), pref, valid,
                         this->PrefixLen);
 ...
 */
char *TIfaceIface::firstLLAddress() { return PresLLAddr = LLAddr; }
...
int TIfaceIface::countLLAddress() { return this->LLAddrCnt; }
 ...
 */
void TIfaceIface::firstSocket() { SocketsLst.first(); }

/*
 * returns next socket from list
 */
SPtr<TIfaceSocket> TIfaceIface::getSocket() { return SocketsLst.get(); }
 ...
 */
int TIfaceIface::countSocket() { return SocketsLst.count(); }
...
SPtr<TIfaceSocket> TIfaceIface::getSocketByAddr(SPtr<TIPv6Addr> addr) {
  ...
  SocketsLst.first();
  while (ptr = SocketsLst.get()) {
    if (*ptr->getAddr() == *addr) return ptr;
  }
  return SPtr<TIfaceSocket>();  // NULL
}
...
void TIfaceIface::setPrefixLength(int len) {
  if (len > 128 || len < 0) {
    Log(Error) << "Invalid length " << len << " set attempt was ignored on the "
