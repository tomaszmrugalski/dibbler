 ...
 */
TIfaceIface::TIfaceIface(const char * name, int id, unsigned int flags, char * mac, int maclen, char * llAddr, int llAddrCnt,
...
bool TIfaceIface::addAddr(SPtr<TIPv6Addr> addr, long pref, long valid, int prefixLen) {
    Log(Notice) << "Address " << addr->getPlain() << "/" << prefixLen << " added to " << getFullName() << " interface."
                << LogEnd;
...
bool TIfaceIface::delAddr(SPtr<TIPv6Addr> addr, int prefixLen) {
    Log(Notice) << "Address " << addr->getPlain() << "/" << prefixLen << " deleted from " << getFullName() << " interface."
                << LogEnd;
...
bool TIfaceIface::updateAddr(SPtr<TIPv6Addr> addr, long pref, long valid) {
    int result;
    Log(Notice) << "Address " << addr->getPlain() << " updated on " << getFullName() << " interface." << LogEnd;

    result = ipaddr_update((char *)this->Name, this->ID, (char *)addr->getPlain(), pref, valid, this->PrefixLen);
...
void TIfaceIface::setPrefixLength(int len) {
    if (len > 128 || len < 0) {
        Log(Error) << "Invalid length " << len << " set attempt was ignored on the " << this->getFullName() << " interface."
                   << LogEnd;
        return;
    }
    this->PrefixLen = len;
...
ostream & operator<<(ostream & strum, TIfaceIface & x) {
    ...
    strum << "  <IfaceIface"
          ...
          << "    <!-- " << (x.flagLoopback() ? "looback" : "no-loopback") << (x.flagRunning() ? " running" : " no-running")
