...
int TIfaceSocket::createSocket(char * iface, int ifaceid, SPtr<TIPv6Addr> addr, int port, bool ifaceonly, bool reuse) {
    ...
    // create socket
    sock = sock_add(this->Iface, this->IfaceID, addr->getPlain(), this->Port, ifaceonly ? 1 : 0, reuse ? 1 : 0);
...
TIfaceSocket::~TIfaceSocket() {
    ...
    Log(Debug) << "Closing socket " << this->FD << " on " << Addr->getPlain() << ":" << Port << " on interface " << Iface << "/"
               << IfaceID << LogEnd;
...
void TIfaceSocket::printError(int error, char * iface, int ifaceid, SPtr<TIPv6Addr> addr, int port) {
    ...
    case LOWLEVEL_ERROR_BIND_IFACE:
        Log(Error) << "Unable to bind socket to interface " << this->Iface << "/" << this->IfaceID << "." << LogEnd;
        break;
    case LOWLEVEL_ERROR_BIND_FAILED:
        Log(Error) << "Unable to bind socket (iface=" << iface << "/" << ifaceid << ", addr=" << addr->getPlain()
                   << ", port=" << this->Port << ")." << LogEnd;
        break;
    case LOWLEVEL_ERROR_MCAST_HOPS:
        Log(Error) << "Unable to set multicast hops. (iface= " << iface << "/" << ifaceid << ", addr=" << *this->Addr
                   << ", port=" << this->Port << ")" << LogEnd;
        break;
