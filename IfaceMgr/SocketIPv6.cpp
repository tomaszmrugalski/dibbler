 ...
 */
TIfaceSocket::TIfaceSocket(char *iface, int ifindex, int port, SPtr<TIPv6Addr> addr, bool ifaceonly,
                           bool reuse) {
  if (this->Count == 0) {
    FD_ZERO(getFDS());
  }
...
enum EState TIfaceSocket::getStatus() { return this->Status; }
 ...
 */
int TIfaceSocket::createSocket(char *iface, int ifaceid, SPtr<TIPv6Addr> addr, int port,
  ...
  // create socket
  sock = sock_add(this->Iface, this->IfaceID, addr->getPlain(), this->Port, ifaceonly ? 1 : 0,
                  reuse ? 1 : 0);
...
int TIfaceSocket::send(char *buf, int len, SPtr<TIPv6Addr> addr, int port) {
  ...
  // extern "C" int sock_send(int fd, char * addr, char * buf, int buflen, int
  // port, int ifaceID);
 ...
 */
int TIfaceSocket::getFD() { return this->FD; }

/**
 * returns interface ID
 */
int TIfaceSocket::getIfaceID() { return this->IfaceID; }

/**
 * returns port
 */
int TIfaceSocket::getPort() { return this->Port; }

/**
 * returns address
 */
SPtr<TIPv6Addr> TIfaceSocket::getAddr() { return this->Addr; }

/**
 * closes socket, and removes its number from FDS
 */
TIfaceSocket::~TIfaceSocket() {
  if (Status != STATE_CONFIGURED) return;

  Log(Debug) << "Closing socket " << this->FD << " on " << Addr->getPlain() << ":" << Port
             << " on interface " << Iface << "/" << IfaceID << LogEnd;
...
void TIfaceSocket::printError(int error, char *iface, int ifaceid, SPtr<TIPv6Addr> addr, int port) {
  ...
  switch (error) {
    case LOWLEVEL_ERROR_UNSPEC:
      Log(Error) << "Unable to create socket. Is IPv6 protocol supported in your system?" << LogEnd;
      break;
    case LOWLEVEL_ERROR_BIND_IFACE:
      Log(Error) << "Unable to bind socket to interface " << this->Iface << "/" << this->IfaceID
                 << "." << LogEnd;
      break;
    case LOWLEVEL_ERROR_BIND_FAILED:
      Log(Error) << "Unable to bind socket (iface=" << iface << "/" << ifaceid
                 << ", addr=" << addr->getPlain() << ", port=" << this->Port << ")." << LogEnd;
      break;
...
std::ostream &operator<<(std::ostream &strum, TIfaceSocket &x) {
  strum << dec << "<IfaceSocket"
        << " fd=\"" << x.getFD() << "\""
        << " port=\"" << x.getPort() << "\""
        << " iface=\"" << x.Iface << "\""
        << " addr=\"" << *x.Addr << "\"";
  if (x.Multicast) strum << " multicast=\"true\"";
  if (x.IfaceOnly) strum << " ifaceonly=\"true\"";
