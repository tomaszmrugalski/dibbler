      ...
      while ((adr = ia->getAddr()) && check) {
        ...
        if (ClientInPool1(adr->get(), iface, PD)) {
          ...
          if (!unicast) {
            ...
            check = false;
            // Can't do anything here if we don't have unicast address of the
            // client. (That's odd. Unicast should be in the database. Are we
            // dealing with a broken database here?)
            break;
          }
...
bool TSrvTransMgr::openSocket(SPtr<TSrvCfgIface> confIface, int port) {
  ...
  if (unicast) {
    /* unicast */
    Log(Notice) << "Creating unicast (" << *unicast << ") socket on " << confIface->getFullName()
                << " interface." << LogEnd;
  ...
  Log(Notice) << "Creating multicast (" << ipAddr->getPlain() << ") socket on "
              << confIface->getFullName() << " (" << iface->getFullName() << ") interface."
              << LogEnd;
  ...
  if (!iface->countLLAddress()) {
    Log(Crit) << "There is no link-local address on " << iface->getFullName() << " defined."
              << LogEnd;
    return false;
  }
  ...
  if (iface->getSocketByAddr(llAddr)) {
    ...
    Log(Notice) << "Creating link-local (" << llAddr->getPlain() << ") socket on "
                << iface->getFullName() << " interface." << LogEnd;
    if (!iface->addSocket(llAddr, port, true, false)) {
      Log(Crit) << "Failed to create link-local socket on " << iface->getFullName() << " interface."
                << LogEnd;
      return false;
    }
  }
    ...
    case SOLICIT_MSG: {
      if (msg->getOption(OPTION_RAPID_COMMIT)) {
        if (!cfgIface->getRapidCommit()) {
          Log(Info) << "SOLICIT with RAPID-COMMIT received, but RAPID-COMMIT is "
                       "disabled on "
...
bool TSrvTransMgr::unicastCheck(SPtr<TSrvMsg> msg) {
  ...
  if (!msg->RelayInfo_.empty()) return true;
  ...
  if (!msg->getLocalAddr()) return true;

  // All good, received on multicast
  if (msg->getLocalAddr()->multicast()) return true;
  ...
  if (!SrvCfgMgr().dropUnicast()) {
    Log(Warning) << "Received message on address " << msg->getLocalAddr()->getPlain()
                 ...
                 << ", but unicast is not allowed on this interface." << LogEnd;
    return true;
  }

  TOptList options;

  SPtr<TOpt> status(new TOptStatusCode(
      STATUSCODE_USEMULTICAST,
      string("Please send your message to multicast, not to ") + msg->getLocalAddr()->getPlain(),
      NULL));
...
void TSrvTransMgr::doDuties() {
  ...
  if (SrvCfgMgr().inactiveMode()) {
    ...
    if (x) openSocket(x, port_);
  }
}

/// @brief Generates parameters for notify script based on expired lease
/// information
///
...
void TSrvTransMgr::notifyExpireInfo(TNotifyScriptParams &params,
  ...
  if (iface) params.addParam("IFACE", iface->getName());

  if (exp.ia->getSrvAddr()) params.addParam("REMOTE_ADDR", exp.ia->getSrvAddr()->getPlain());
  switch (type) {
    case IATYPE_IA:
    case IATYPE_TA: {
  ...
  params.addParam("SRV_OPTION1",
                  exp.client->getDUID()->getPlain());  // set client-id
}
...
void TSrvTransMgr::removeExpired(std::vector<TSrvAddrMgr::TExpiredInfo> &addrLst,
  ...
  for (vector<TSrvAddrMgr::TExpiredInfo>::iterator addr = addrLst.begin(); addr != addrLst.end();
       ++addr) {
    // delete this address
    Log(Notice) << "Address " << *(addr->addr) << " in IA (IAID=" << addr->ia->getIAID()
                << ") in client (DUID=\"" << addr->client->getDUID()->getPlain()
                << "\") has expired." << LogEnd;
  ...
  for (vector<TSrvAddrMgr::TExpiredInfo>::iterator addr = tempAddrLst.begin();
    ...
    // delete this address
    Log(Notice) << "Temp. address " << *(addr->addr) << " in IA (IAID=" << addr->ia->getIAID()
                << ") in client (DUID=\"" << addr->client->getDUID()->getPlain()
                << "\") has expired." << LogEnd;
  ...
  for (vector<TSrvAddrMgr::TExpiredInfo>::iterator prefix = prefixLst.begin();
    ...
    Log(Notice) << "Prefix " << prefix->addr->getPlain() << " in IAID=" << prefix->ia->getIAID()
                << " for client (DUID=" << prefix->client->getDUID()->getPlain() << ") has expired."
                << LogEnd;
...
bool TSrvTransMgr::isDone() { return IsDone; }

char *TSrvTransMgr::getCtrlAddr() { return this->ctrlAddr; }

int TSrvTransMgr::getCtrlIface() { return this->ctrlIface; }
...
TSrvTransMgr::~TSrvTransMgr() { Log(Debug) << "SrvTransMgr cleanup." << LogEnd; }
...
bool TSrvTransMgr::sendReconfigure(SPtr<TIPv6Addr> addr, int iface, int msgType,
                                   SPtr<TDUID> ptrDUID) {
  SPtr<TSrvMsg> reconfigure;
  reconfigure = new TSrvMsgReconfigure(iface, addr, msgType, ptrDUID);
  // reconfigure->send(); // not needed (message will send itself in
  // constructor)
  return true;
}
