...
#ifdef LOWLEVEL_DEBUG
        printf("### iface %d %s (flags:%d) (mo bits:%d)###\n", ifi->ifi_index, (char *)RTA_DATA(tb[IFLA_IFNAME]),
               ifi->ifi_flags, mo_bits);
#endif
 ...
 * @return
 */
int ipaddr_add_or_del(const char * addr, const char * ifacename, int prefixLen, unsigned long preferred, unsigned long valid,
                      int mode) {
    struct rtnl_handle rth;
    struct {
        struct nlmsghdr n;
...
int sock_add(char * ifacename, int ifaceid, char * addr, int port, int thisifaceonly, int reuse) {
    ...
    if (multicast) {
        ...
        memcpy(&mreq6.ipv6mr_multiaddr, &((struct sockaddr_in6 *)res2->ai_addr)->sin6_addr, sizeof(mreq6.ipv6mr_multiaddr));
