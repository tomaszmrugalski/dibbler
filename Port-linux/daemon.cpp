...
int stop(const char * pidfile) {
    ...
    if (sigaction(SIGHUP, &action, NULL) || sigaction(SIGINT, &action, NULL) || sigaction(SIGPIPE, &action, NULL) ||
    ...
    if (ptrace_failed) {
        ...
        while (1) {
            ...
            if (WIFEXITED(p_status) || WIFSIGNALED(p_status) || (WIFSTOPPED(p_status) && WSTOPSIG(p_status) == SIGSTOP)) {
                break;
            }
    ...
    while (1) {
        ...
        ptrace(PTRACE_CONT, pid, NULL, (WIFSTOPPED(p_status) && WSTOPSIG(p_status) != SIGSTOP) ? WSTOPSIG(p_status) : 0);
    }
    cout << "Done." << endl;
