...
pid_t getPID(const char *file) {
  ...
  struct stat buf;
  int i = stat(file, &buf);
  if (i != 0) return LOWLEVEL_ERROR_UNSPEC;

  ifstream pidfile(file);
  if (!pidfile.is_open()) return LOWLEVEL_ERROR_FILE;
  pid_t pid;
  pidfile >> pid;
  return pid;
}
...
int init(const char *pidfile, const char *workdir) {
  ...
  if (pid > 0) {
    ...
    if (snprintf(buf, sizeof(buf), "/proc/%ld", (long int)pid) >= (int)sizeof(buf)) {
      Log(Crit) << "Buffer for string `/proc/" << pid << "' too small" << LogEnd;
      return 0;
    };
    if (!access(buf, F_OK)) {
      if (snprintf(buf, sizeof(buf), "/proc/%ld/exe", (long int)pid) >= (int)sizeof(buf)) {
        Log(Crit) << "Buffer for string `/proc/" << pid << "/exe' too small" << LogEnd;
        return 0;
      }
      ...
      if (len != -1) {
        ...
        if (strstr(cmd, "dibbler") == NULL) {
          Log(Warning) << "Process is running but it is not Dibbler (pid=" << pid << ", name "
                       << cmd << ")." << LogEnd;
        } else {
          Log(Crit) << "Process already running and it seems to be Dibbler (pid=" << pid
                    << ", name " << cmd << ")." << LogEnd;
          return 0;
        }
      } else {
      ...
      Log(Warning) << "Pid file found (pid=" << pid << ", file " << pidfile << "), but process "
                   << pid << " does not exist." << LogEnd;
    }
  }
...
int stop(const char *pidfile) {
  ...
  if (sigaction(SIGHUP, &action, NULL) || sigaction(SIGINT, &action, NULL) ||
      sigaction(SIGPIPE, &action, NULL) || sigaction(SIGTERM, &action, NULL)) {
  ...
  if (ptrace_failed) {
    saved_errno = errno;
    cout << "Attaching to process " << pid << " failed: " << strerror(saved_errno) << endl;
  } else {
    ...
    while (1) {
      ...
      ptrace(PTRACE_CONT, pid, NULL, WIFSTOPPED(p_status) ? WSTOPSIG(p_status) : 0);
    }
  ...
  while (1) {
    ...
    ptrace(PTRACE_CONT, pid, NULL,
           (WIFSTOPPED(p_status) && WSTOPSIG(p_status) != SIGSTOP) ? WSTOPSIG(p_status) : 0);
  }
  cout << "Done." << endl;
