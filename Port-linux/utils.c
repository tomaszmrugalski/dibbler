...
int get_addr_1(inet_prefix * addr, const char * name, int family) {
    ...
    if (strcmp(name, "default") == 0 || strcmp(name, "all") == 0 || strcmp(name, "any") == 0) {
        if (family == AF_DECnet)
            return -1;
        addr->family = family;
...
int get_prefix_1(inet_prefix * dst, char * arg, int family) {
    ...
    if (strcmp(arg, "default") == 0 || strcmp(arg, "any") == 0 || strcmp(arg, "all") == 0) {
        if (family == AF_DECnet)
            return -1;
        dst->family = family;
        dst->bytelen = 0;
...
char * resolve_address(const char * addr, int len, int af) {
    ...
    if (af == AF_INET6 && ((__u32 *)addr)[0] == 0 && ((__u32 *)addr)[1] == 0 && ((__u32 *)addr)[2] == htonl(0xffff)) {
        af = AF_INET;
        addr += 12;
        len = 4;
    }
    ...
    for (n = nht[hash]; n; n = n->next) {
        if (n->addr.family == af && n->addr.bytelen == len && memcmp(n->addr.data, addr, len) == 0)
            return n->name;
    }
...
const char * format_host(int af, int len, const void * addr, char * buf, int buflen) {
    ...
    if (resolve_hosts) {
        ...
        if (len > 0 && (n = resolve_address(addr, len, af)) != NULL)
            return n;
    }
#endif
