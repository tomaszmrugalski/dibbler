        ...
        if (*head != '\0' && *head != comment) {
            ...
            if (strstr(head, keyword) == head) {
                ...
                if (*head != '\0' && *head != comment && isspace((int)(*head))) {
                    ...
                    if (remove_empty) {
                        for (head = keyword_end; *head != '\0' && isspace((int)(*head)); head++)
                            ;
                        if (*head == '\0' || *head == comment)
                            *buf = '\0';
                    }
...
int cfg_file_del(const char * file, const char * keyword, const char * value) {
    ...
    if (error == LOWLEVEL_NO_ERROR) {
        ...
        if (stat(file, &st) || rename(template, file) || chmod(file, st.st_mode))
            error = LOWLEVEL_ERROR_FILE;
    }
...
int dns_add(const char * ifname, int ifaceid, const char * addrPlain) {
    ...
    if (!f && !(f = fopen(RESOLVCONF_FILE, "a+"))) {
        sprintf(error_message(), "Unable to open %s file:%s", RESOLVCONF_FILE, strerror(errno));
...
#define TIMEZONE_FILE_TMP TIMEZONE_FILE ".dibbler"
    ...
    if (unlink(TIMEZONE_FILE_TMP) && errno != ENOENT)
        return LOWLEVEL_ERROR_FILE;
    if (symlink(buf, TIMEZONE_FILE_TMP))
        return LOWLEVEL_ERROR_FILE;
 ...
 */
int prefix_add(const char * ifname, int ifindex, const char * prefixPlain, int prefixLength, unsigned long preferred,
               unsigned long valid) {
    char * argv[5];
    int result;
    char buf[128];
