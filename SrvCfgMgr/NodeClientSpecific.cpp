...
NodeClientSpecific::NodeClientSpecific() : Node(NODE_CLIENT_SPECIFIC) {
    CurrentMsg.reset();
    Type = CLIENT_UNKNOWN;
}
...
NodeClientSpecific::NodeClientSpecific(ClientSpecificType t) : Node(NODE_CLIENT_SPECIFIC) {
    CurrentMsg.reset();
    Type = t;
}
...
void NodeClientSpecific::analyseMessage(SPtr<TSrvMsg> msg) {
    ...
    while (ptrOpt = msg->getOption()) {
        ...
        case OPTION_VENDOR_CLASS:
            ...
            vendor_class_data = "";
            for (std::vector<TOptUserClass::UserClassData>::const_iterator data = vendorclass->userClassData_.begin();
                 data != vendorclass->userClassData_.end(); ++data) {
                vendor_class_data +=
                    std::string(reinterpret_cast<const char *>(&data->opaqueData_[0]), data->opaqueData_.size());
            }
