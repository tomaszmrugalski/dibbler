...
/*
 * is client allowed to use this class? (it can be rejected on DUID or address
 * basis)
 */
bool TSrvCfgAddrClass::clntSupported(SPtr<TDUID> duid, SPtr<TIPv6Addr> clntAddr) {
  ...
  while (range = RejedClnt_.get())
    if (range->in(duid, clntAddr)) return false;
  ...
  if (AcceptClnt_.count()) {
    ...
    while (range = AcceptClnt_.get()) {
      ...
      if (range->in(duid, clntAddr)) return true;
    }
    return false;
  }
...
uint32_t TSrvCfgAddrClass::getT1(uint32_t clntT1) { return chooseTime(T1Min_, T1Max_, clntT1); }

uint32_t TSrvCfgAddrClass::getT2(uint32_t clntT2) { return chooseTime(T2Min_, T2Max_, clntT2); }
...
void TSrvCfgAddrClass::setOptions(SPtr<TSrvParsGlobalOpt> opt) {
  ...
  opt->firstRejedClnt();
  while (statRange = opt->getRejedClnt()) RejedClnt_.append(statRange);

  opt->firstAcceptClnt();
  while (statRange = opt->getAcceptClnt()) AcceptClnt_.append(statRange);
  ...
  if (ClassMaxLease_ > AddrsCount_) ClassMaxLease_ = AddrsCount_;

  AddrParams_ = opt->getAddrParams();
}

bool TSrvCfgAddrClass::addrInPool(SPtr<TIPv6Addr> addr) { return Pool_->in(addr); }

unsigned long TSrvCfgAddrClass::countAddrInPool() { return AddrsCount_; }

SPtr<TIPv6Addr> TSrvCfgAddrClass::getRandomAddr() { return Pool_->getRandomAddr(); }

SPtr<TIPv6Addr> TSrvCfgAddrClass::getFirstAddr() { return Pool_->getAddrL(); }
SPtr<TIPv6Addr> TSrvCfgAddrClass::getLastAddr() { return Pool_->getAddrR(); }

unsigned long TSrvCfgAddrClass::getClassMaxLease() { return ClassMaxLease_; }

unsigned long TSrvCfgAddrClass::getID() { return ID_; }

unsigned long TSrvCfgAddrClass::getShare() { return Share_; }
...
unsigned long TSrvCfgAddrClass::getAssignedCount() { return AddrsAssigned_; }
...
bool TSrvCfgAddrClass::isLinkLocal() {
  ...
  if (addrInPool(addr)) {
    Log(Crit) << "Link local address (fe80:ffff:ffff:ffff:ffff:ffff:ffff:ffff) "
...
SPtr<TSrvOptAddrParams> TSrvCfgAddrClass::getAddrParams() { return AddrParams_; }

ostream &operator<<(ostream &out, TSrvCfgAddrClass &addrClass) {
  out << "    <class id=\"" << addrClass.ID_ << "\" share=\"" << addrClass.Share_ << "\">"
      << std::endl;
  out << "      <!-- total addrs in class: " << addrClass.AddrsCount_
      << ", addrs assigned: " << addrClass.AddrsAssigned_ << " -->" << endl;
  out << "      <T1 min=\"" << addrClass.T1Min_ << "\" max=\"" << addrClass.T1Max_ << "\" />"
      << endl;
  out << "      <T2 min=\"" << addrClass.T2Min_ << "\" max=\"" << addrClass.T2Max_ << "\" />"
      << endl;
  out << "      <pref min=\"" << addrClass.PrefMin_ << "\" max=\"" << addrClass.PrefMax_ << "\" />"
      << endl;
  out << "      <valid min=\"" << addrClass.ValidMin_ << "\" max=\"" << addrClass.ValidMax_
      << "\" />" << endl;
  ...
  while (statRange = addrClass.RejedClnt_.get()) out << *statRange;

  out << "      <!-- accept-only ranges:" << addrClass.AcceptClnt_.count() << " -->" << endl;
  addrClass.AcceptClnt_.first();
  while (statRange = addrClass.AcceptClnt_.get()) out << *statRange;
