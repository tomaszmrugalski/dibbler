...
public:
  ...
  SPtr<TSrvCfgAddrClass> getRandomClass(SPtr<TDUID> clntDuid,
  ...
  // SPtr<TSrvCfgPD> getRandomPrefix(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr>
  // clntAddr);
  long countPD() const;
  ...
  SPtr<TSrvCfgOptions> getClientException(SPtr<TDUID> duid, TMsg *message,
                                          bool quiet = true);
  bool checkReservedPrefix(SPtr<TIPv6Addr> pfx, SPtr<TDUID> duid,
  ...
  SPtr<TFQDN> getFQDNName(SPtr<TDUID> duid, SPtr<TIPv6Addr> addr,
...
private:
  ...
  bool Relay_;
  std::string
      RelayName_; // name of the underlaying physical interface (or other relay)
  int RelayID_;   // ifindex (-1 means this is not a relay)
  SPtr<TSrvOptInterfaceID>
      RelayInterfaceID_; // value of interface-id option (optional)
