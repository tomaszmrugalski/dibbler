...
SPtr<TSrvCfgOptions> TSrvCfgIface::getClientException(SPtr<TDUID> duid, TMsg * parent, bool quiet) {
    ...
    while (x = ExceptionsLst_.get()) {
        if (duid && x->getDuid() && (*(x->getDuid()) == *duid)) {
            if (!quiet)
                Log(Debug) << "Found per-client configuration (exception) for client with DUID=" << x->getDuid()->getPlain()
                           << LogEnd;
            return x;
        }
        SPtr<TOptVendorData> remoteid;
        ...
        if (remoteID && remoteid && (remoteID->getVendor() == remoteid->getVendor()) &&
            ...
            Log(Debug) << "Found per-client configuration (exception) for client with RemoteID: vendor="
                       << remoteid->getVendor() << ", data=" << remoteid->getVendorDataPlain() << "." << LogEnd;
            return x;
        }
        if (peer && x && x->getClntAddr() && *(peer) == *(x->getClntAddr())) {
            Log(Debug) << "Found per-client configuration (exception) for client with link-local=" << peer->getPlain()
                       << LogEnd;
            return x;
        }
    }
...
bool TSrvCfgIface::checkReservedPrefix(SPtr<TIPv6Addr> pfx, SPtr<TDUID> duid, SPtr<TOptVendorData> myRemoteID,
    ...
    while (x = ExceptionsLst_.get()) {
        ...
        if (x->getDuid()) {
            if (*duid == *x->getDuid()) {
                ...
                Log(Debug) << "Prefix " << x->getPrefix()->getPlain() << " is reserved for DUID=" << x->getDuid()->getPlain()
                           << LogEnd;
                return true;
            }
        }
        ...
        if (remoteid) {
            if ((myRemoteID->getVendor() == remoteid->getVendor()) &&
                ...
                Log(Debug) << "Prefix " << x->getPrefix()->getPlain() << "is reserved for remote-id=" << remoteid->getPlain()
                           << LogEnd;
        ...
        if (addr) {
            if (*linkLocal == *addr) {
                ...
                Log(Debug) << "Prefix " << x->getPrefix()->getPlain() << " is reserved for link-local address "
        ...
        Log(Error) << "Found reservation for prefix " << x->getPrefix()->getPlain()
                   << ", but it is misconfigured (no DUID, remote-id nor link-local specified)" << LogEnd;
...
int TSrvCfgIface::getAllowedAddrClassID(SPtr<TDUID> duid, SPtr<TIPv6Addr> clntAddr) {
    ...
    while ((ptrClass = SrvCfgAddrClassLst_.get()) && (cnt < 100)) {
        if (ptrClass->clntSupported(duid, clntAddr) && ptrClass->getClassMaxLease() > ptrClass->getAssignedCount()) {
...
void TSrvCfgIface::addClntAddr(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false*/) {
    ...
    while (ptrClass = getAddrClass()) {
        if (ptrClass->addrInPool(ptrAddr)) {
            ...
            if (quiet)
                return;
            Log(Debug) << "Address usage for class " << ptrClass->getID() << " increased to " << count << "." << LogEnd;
            return;
        }
    }
...
void TSrvCfgIface::delClntAddr(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false*/) {
    ...
    while (ptrClass = getAddrClass()) {
        if (ptrClass->addrInPool(ptrAddr)) {
            ...
            if (quiet)
                return;
            Log(Debug) << "Address usage for class " << ptrClass->getID() << " decreased to " << count << "." << LogEnd;
            return;
        }
    }
...
SPtr<TSrvCfgAddrClass> TSrvCfgIface::getRandomClass(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr) {
    long classid;
    ...
    if (classid > -1) {
        Log(Debug) << "Found prefered class " << classid << " for client (duid = " << *clntDuid << ", addr = " << *clntAddr
                   << ")" << LogEnd;
    ...
    Log(Warning) << "No class is available for client (duid=" << clntDuid->getPlain() << ", addr=" << clntAddr->getPlain()
                 << ")." << LogEnd;
    return SPtr<TSrvCfgAddrClass>(); // NULL
}
...
bool TSrvCfgIface::addClntPrefix(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false */) {
    ...
    while (ptrPD = getPD()) {
        if (ptrPD->prefixInPool(ptrAddr)) {
            ...
            if (quiet)
                return true;
            Log(Debug) << "PD: Prefix usage for class " << ptrPD->getID() << " increased to " << count << "." << LogEnd;
            return true;
        }
    }
...
bool TSrvCfgIface::delClntPrefix(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false */) {
    ...
    while (ptrPD = getPD()) {
        if (ptrPD->prefixInPool(ptrAddr)) {
            ...
            if (quiet)
                return true;
            Log(Debug) << "PD: Prefix usage for class " << ptrPD->getID() << " decreased to " << count << "." << LogEnd;
            return true;
        }
    }
    Log(Warning) << "Unable to decrease address usage: no class found for " << *ptrAddr << LogEnd;
...
SPtr<TFQDN> TSrvCfgIface::getFQDNName(SPtr<TDUID> duid, SPtr<TIPv6Addr> addr, const std::string & hint) {
    ...
    while (foo = FQDNLst_.get()) {
        ...
        if (foo->isUsed()) {
            ...
            if ((foo->getDuid()) && (*foo->getDuid() == *duid) && (foo->getAddr()) && (*foo->getAddr() == *addr)) {
                Log(Debug) << "FQDN: This client (DUID=" << duid->getPlain() << ") has already assigned name " << foo->getName()
                           << " to its address " << foo->getAddr()->getPlain() << "." << LogEnd;
                return foo;
            }

            if ((foo->getName() == hint) && (*foo->getDuid() == *duid)) {
                Log(Debug) << "FQDN: Client requested " << hint << ", it is already assinged to this client. Reusing."
                           << LogEnd;
                return foo;
            }
            continue;
        ...
        if (foo->getName() == hint) {
            ...
            if ((!foo->getDuid()) && (!foo->getAddr())) {
                Log(Debug) << "Client's hint: " << hint << " found in fqdn list, setting fqdn to " << foo->getName() << LogEnd;
                return foo;
            }
        }
    ...
    case UNKKOWN_FQDN_ACCEPT_POOL: {
        if (alternative)
            Log(Info) << "FQDN: Client requested " << hint << ", but assigning other name (" << alternative->getName()
                      << ") from available pool instead." << LogEnd;
        return alternative;
    }
...
void TSrvCfgIface::addTAAddr() {
    ...
    ta = getTA();
    if (!ta) {
        Log(Error) << "Unable to increase TA usage. TA (temporary addresses) is not found on the " << getFullName()
                   << " interface." << LogEnd;
        return;
    }
    ta->incrAssigned();
...
void TSrvCfgIface::delTAAddr() {
    ...
    ta = getTA();
    if (!ta) {
        Log(Error) << "Unable to decrease TA usage. TA (temporary addresses) is not found on the " << getFullName()
                   << " interface." << LogEnd;
        return;
    }
    ta->decrAssigned();
...
bool TSrvCfgIface::addrInSubnet(SPtr<TIPv6Addr> addr) {
    for (std::vector<THostRange>::const_iterator range = Subnets_.begin(); range != Subnets_.end(); ++range) {
        if (range->in(addr)) {
            return true;
        }
    }
...
ostream & operator<<(ostream & out, TSrvCfgIface & iface) {
    ...
    for (std::vector<THostRange>::const_iterator range = iface.Subnets_.begin(); range != iface.Subnets_.end(); ++range) {
        out << "    <subnet>" << range->getAddrL()->getPlain() << "-" << range->getAddrR()->getPlain() << "</subnet>"
            << std::endl;
    }
    ...
    for (TOptList::iterator extra = extraLst.begin(); extra != extraLst.end(); ++extra) {
        out << "      <extraOption type=\"" << (*extra)->getOptType() << "\" length=\"" << (*extra)->getSize() << "\">"
            << ((*extra)->getPlain()) << "</extraOption>" << endl;
    }
    ...
    if (iface.supportFQDN()) {
        ...
        out << "    <fqdnOptions count=\"" << lst->count() << "\" prefix=\"" << iface.getRevDNSZoneRootLength() << "\""
