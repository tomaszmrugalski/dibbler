...
bool TSrvCfgIface::leaseQuerySupport() const { return LeaseQuery_; }
...
SPtr<TSrvCfgOptions> TSrvCfgIface::getClientException(SPtr<TDUID> duid, TMsg *parent, bool quiet) {
  ...
  while (x = ExceptionsLst_.get()) {
    if (duid && x->getDuid() && (*(x->getDuid()) == *duid)) {
      if (!quiet)
        Log(Debug) << "Found per-client configuration (exception) for client "
                      "with DUID="
    ...
    if (remoteID && remoteid && (remoteID->getVendor() == remoteid->getVendor()) &&
        ...
        !memcmp(remoteid->getVendorData(), remoteID->getVendorData(),
                remoteid->getVendorDataLen())) {
      Log(Debug) << "Found per-client configuration (exception) for client "
                    "with RemoteID: vendor="
                 << remoteid->getVendor() << ", data=" << remoteid->getVendorDataPlain() << "."
                 << LogEnd;
      return x;
    }
...
bool TSrvCfgIface::addrReserved(SPtr<TIPv6Addr> addr) {
  ...
  ExceptionsLst_.first();
  while (x = ExceptionsLst_.get()) {
    if ((x->getAddr()) && (*x->getAddr() == *addr)) return true;
  }
  return false;
}
...
unsigned int TSrvCfgIface::removeReservedFromCache() {
  ...
  ExceptionsLst_.first();
  while (x = ExceptionsLst_.get()) {
    if (x->getAddr()) cnt += SrvAddrMgr().delCachedEntry(x->getAddr(), IATYPE_IA);
    if (x->getPrefix()) cnt += SrvAddrMgr().delCachedEntry(x->getPrefix(), IATYPE_PD);
  }
  return cnt;
}
...
bool TSrvCfgIface::prefixReserved(SPtr<TIPv6Addr> prefix) {
  ...
  ExceptionsLst_.first();
  while (x = ExceptionsLst_.get()) {
    if (x->getPrefix() && (*x->getPrefix() == *prefix)) return true;
  }
  return false;
}
...
bool TSrvCfgIface::checkReservedPrefix(SPtr<TIPv6Addr> pfx, SPtr<TDUID> duid,
                                       SPtr<TOptVendorData> myRemoteID, SPtr<TIPv6Addr> linkLocal) {
  // sanity check
  if (!pfx || !duid) {
    // should not happen
  ...
  while (x = ExceptionsLst_.get()) {
    ...
    if (*(x->getPrefix()) != (*pfx)) continue;  // that is not the prefix we are looking for
    ...
    if (x->getDuid()) {
      if (*duid == *x->getDuid()) {
        ...
        Log(Debug) << "Prefix " << x->getPrefix()->getPlain()
                   << " is reserved for DUID=" << x->getDuid()->getPlain() << LogEnd;
        return true;
      }
    }
    ...
    if (remoteid) {
      if ((myRemoteID->getVendor() == remoteid->getVendor()) &&
          ...
          (!memcmp(myRemoteID->getVendorData(), remoteid->getVendorData(),
                   remoteid->getVendorDataLen()))) {
        return false;  // reserved for us!
      } else {
        Log(Debug) << "Prefix " << x->getPrefix()->getPlain()
                   << "is reserved for remote-id=" << remoteid->getPlain() << LogEnd;
    ...
    if (addr) {
      if (*linkLocal == *addr) {
        ...
        Log(Debug) << "Prefix " << x->getPrefix()->getPlain()
                   << " is reserved for link-local address " << addr->getPlain() << LogEnd;
        return true;
      }
    }

    Log(Error) << "Found reservation for prefix " << x->getPrefix()->getPlain()
               << ", but it is misconfigured (no DUID, remote-id nor "
                  "link-local specified)"
               << LogEnd;
...
void TSrvCfgIface::firstAddrClass() { SrvCfgAddrClassLst_.first(); }
...
int TSrvCfgIface::getAllowedAddrClassID(SPtr<TDUID> duid, SPtr<TIPv6Addr> clntAddr) {
  ...
  if (!cnt) return -1;  // this client is not supported by any class
...
void TSrvCfgIface::firstPD() { SrvCfgPDLst_.first(); }

bool TSrvCfgIface::supportPrefixDelegation() const { return SrvCfgPDLst_.count(); }

void TSrvCfgIface::addTA(SPtr<TSrvCfgTA> ta) { SrvCfgTALst_.append(ta); }

void TSrvCfgIface::firstTA() { SrvCfgTALst_.first(); }
SPtr<TSrvCfgTA> TSrvCfgIface::getTA() { return SrvCfgTALst_.get(); }

void TSrvCfgIface::addPD(SPtr<TSrvCfgPD> pd) { SrvCfgPDLst_.append(pd); }
...
SPtr<TSrvCfgTA> TSrvCfgIface::getTA(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr) {
  ...
  SrvCfgTALst_.first();
  while (ta = getTA()) {
    if (ta->clntPrefered(clntDuid, clntAddr)) return ta;
  }

  // prefered not found? Then find first allowed
  SrvCfgTALst_.first();
  while (ta = getTA()) {
    if (ta->clntSupported(clntDuid, clntAddr)) return ta;
  }

  return SPtr<TSrvCfgTA>();  // NULL
}

SPtr<TSrvCfgAddrClass> TSrvCfgIface::getAddrClass() { return SrvCfgAddrClassLst_.get(); }

SPtr<TSrvCfgAddrClass> TSrvCfgIface::getClassByID(unsigned long id) {
  firstAddrClass();
  SPtr<TSrvCfgAddrClass> ptrClass;
  while (ptrClass = getAddrClass()) {
    if (ptrClass->getID() == id) return ptrClass;
  }
...
void TSrvCfgIface::addClntAddr(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false*/) {
  ...
  while (ptrClass = getAddrClass()) {
    if (ptrClass->addrInPool(ptrAddr)) {
      ...
      if (quiet) return;
      Log(Debug) << "Address usage for class " << ptrClass->getID() << " increased to " << count
                 << "." << LogEnd;
      return;
    }
  }
  Log(Warning) << "Unable to increase address usage: no class found for " << *ptrAddr << LogEnd;
}
...
void TSrvCfgIface::delClntAddr(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false*/) {
  ...
  while (ptrClass = getAddrClass()) {
    if (ptrClass->addrInPool(ptrAddr)) {
      ...
      if (quiet) return;
      Log(Debug) << "Address usage for class " << ptrClass->getID() << " decreased to " << count
                 << "." << LogEnd;
      return;
    }
  }
  Log(Warning) << "Unable to decrease address usage: no class found for " << *ptrAddr << LogEnd;
}
...
SPtr<TSrvCfgAddrClass> TSrvCfgIface::getRandomClass(SPtr<TDUID> clntDuid,
  ...
  // if there is class where client is on whitelist, it should be used rather
  // than any other class that would be also suitable
  classid = getPreferedAddrClassID(clntDuid, clntAddr);
  if (classid > -1) {
    Log(Debug) << "Found prefered class " << classid << " for client (duid = " << *clntDuid
               << ", addr = " << *clntAddr << ")" << LogEnd;
  ...
  Log(Warning) << "No class is available for client (duid=" << clntDuid->getPlain()
               << ", addr=" << clntAddr->getPlain() << ")." << LogEnd;
  return SPtr<TSrvCfgAddrClass>();  // NULL
}

long TSrvCfgIface::countAddrClass() const { return SrvCfgAddrClassLst_.count(); }

SPtr<TSrvCfgPD> TSrvCfgIface::getPD() { return SrvCfgPDLst_.get(); }

SPtr<TSrvCfgPD> TSrvCfgIface::getPDByID(unsigned long id) {
  firstPD();
  SPtr<TSrvCfgPD> ptrPD;
  while (ptrPD = getPD()) {
    if (ptrPD->getID() == id) return ptrPD;
  }
...
bool TSrvCfgIface::addClntPrefix(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false */) {
  ...
  while (ptrPD = getPD()) {
    if (ptrPD->prefixInPool(ptrAddr)) {
      ...
      Log(Debug) << "PD: Prefix usage for class " << ptrPD->getID() << " increased to " << count
                 << "." << LogEnd;
      return true;
    }
  }
  Log(Warning) << "Unable to increase prefix usage: no prefix found for " << *ptrAddr << LogEnd;
  return false;
}
...
bool TSrvCfgIface::delClntPrefix(SPtr<TIPv6Addr> ptrAddr, bool quiet /* =false */) {
  ...
  while (ptrPD = getPD()) {
    if (ptrPD->prefixInPool(ptrAddr)) {
      ...
      if (quiet) return true;
      Log(Debug) << "PD: Prefix usage for class " << ptrPD->getID() << " decreased to " << count
                 << "." << LogEnd;
      return true;
    }
  }
...
long TSrvCfgIface::countPD() const { return SrvCfgPDLst_.count(); }

int TSrvCfgIface::getID() const { return ID_; }

string TSrvCfgIface::getName() const { return Name_; }
...
SPtr<TIPv6Addr> TSrvCfgIface::getUnicast() { return Unicast_; }
...
void TSrvCfgIface::setOptions(SPtr<TSrvParsGlobalOpt> opt) {
  ...
  if (opt->supportFQDN()) {
    ...
    Log(Debug) << "FQDN: RevDNS zoneroot lenght set to " << getRevDNSZoneRootLength() << "."
               << LogEnd;
  }
...
void TSrvCfgIface::setNoConfig() { NoConfig_ = true; }

unsigned char TSrvCfgIface::getPreference() const { return Preference_; }

void TSrvCfgIface::setName(const std::string &ifaceName) { Name_ = ifaceName; }

void TSrvCfgIface::setID(int ifaceID) { ID_ = ifaceID; }

bool TSrvCfgIface::getRapidCommit() const { return RapidCommit_; }

void TSrvCfgIface::addAddrClass(SPtr<TSrvCfgAddrClass> addrClass) {
  SrvCfgAddrClassLst_.append(addrClass);
}

long TSrvCfgIface::getIfaceMaxLease() const { return IfaceMaxLease_; }

unsigned long TSrvCfgIface::getClntMaxLease() const { return ClntMaxLease_; }

string TSrvCfgIface::getRelayName() const { return RelayName_; }

int TSrvCfgIface::getRelayID() const { return RelayID_; }

SPtr<TSrvOptInterfaceID> TSrvCfgIface::getRelayInterfaceID() const { return RelayInterfaceID_; }

bool TSrvCfgIface::isRelay() const { return Relay_; }

void TSrvCfgIface::setRelayName(const std::string &name) { RelayName_ = name; }

void TSrvCfgIface::setRelayID(int id) { RelayID_ = id; }

// --- option: FQDN ---
void TSrvCfgIface::setFQDNLst(List(TFQDN) * fqdn) { FQDNLst_ = *fqdn; }

/**
 * this method tries to find a name for a client. It check client's hint, and
 * possible reservations by duid or by address
 *
 * @param duid
 * @param addr
 * @param hint
...
SPtr<TFQDN> TSrvCfgIface::getFQDNName(SPtr<TDUID> duid, SPtr<TIPv6Addr> addr,
  ...
  while (foo = FQDNLst_.get()) {
    ...
    if (foo->isUsed()) {
      ...
      if ((foo->getDuid()) && (*foo->getDuid() == *duid) && (foo->getAddr()) &&
          (*foo->getAddr() == *addr)) {
        Log(Debug) << "FQDN: This client (DUID=" << duid->getPlain()
                   << ") has already assigned name " << foo->getName() << " to its address "
                   << foo->getAddr()->getPlain() << "." << LogEnd;
        return foo;
      }

      if ((foo->getName() == hint) && (*foo->getDuid() == *duid)) {
        Log(Debug) << "FQDN: Client requested " << hint
                   << ", it is already assinged to this client. Reusing." << LogEnd;
        return foo;
      }
      continue;
    }
    ...
    if (duid && (foo->getDuid()) && *(foo->getDuid()) == *duid) {
      Log(Debug) << "FQDN found: " << foo->getName() << " using duid " << duid->getPlain()
                 << LogEnd;
      return foo;
    }
    if (addr && (foo->getAddr()) && *(foo->getAddr()) == *addr) {
      Log(Debug) << "FQDN found: " << foo->getName() << " using address " << addr->getPlain()
                 << LogEnd;
      return foo;
    }

    if (foo->getName() == hint) {
      // client asked for this name. Let's check if client is allowed to get
      // this name.
    ...
    if (!foo->getAddr() && !foo->getDuid()) {
      if (!alternative) alternative = foo;
    }
  }
  ...
  switch (UnknownFQDN_) {
    ...
    case UNKNOWN_FQDN_PROCEDURAL: {
      ...
      while ((j = tmp.find("::")) != std::string::npos) tmp.replace(j, 1, "-");
      while ((j = tmp.find(':')) != std::string::npos) tmp.replace(j, 1, "-");
      tmp = tmp + "." + FQDNDomain_;
...
List(TFQDN) * TSrvCfgIface::getFQDNLst() { return &FQDNLst_; }

bool TSrvCfgIface::supportFQDN() const {
  return FQDNLst_.count() || (UnknownFQDN_ >= UNKNOWN_FQDN_ACCEPT);
}

int TSrvCfgIface::getFQDNMode() const { return FQDNMode_; }

int TSrvCfgIface::getRevDNSZoneRootLength() const { return RevDNSZoneRootLength_; }
...
string TSrvCfgIface::getFQDNModeString() const {
  switch (FQDNMode_) {
    ...
    case 1:
      return "server will update PTR";
    case 2:
      return "server will update PTR and AAAA";
    default:
      return "unknown";
  }
}
...
void TSrvCfgIface::addTAAddr() {
  ...
  ta = getTA();
  if (!ta) {
    Log(Error) << "Unable to increase TA usage. TA (temporary addresses) is "
                  "not found on the "
...
void TSrvCfgIface::delTAAddr() {
  ...
  ta = getTA();
  if (!ta) {
    Log(Error) << "Unable to decrease TA usage. TA (temporary addresses) is "
                  "not found on the "
               << getFullName() << " interface." << LogEnd;
    return;
  }
...
bool TSrvCfgIface::addrInSubnet(SPtr<TIPv6Addr> addr) {
  for (std::vector<THostRange>::const_iterator range = Subnets_.begin(); range != Subnets_.end();
       ++range) {
    if (range->in(addr)) {
      return true;
    }
...
bool TSrvCfgIface::subnetDefined() { return !Subnets_.empty(); }
...
ostream &operator<<(ostream &out, TSrvCfgIface &iface) {
  ...
  for (std::vector<THostRange>::const_iterator range = iface.Subnets_.begin();
    ...
    out << "    <subnet>" << range->getAddrL()->getPlain() << "-" << range->getAddrR()->getPlain()
        << "</subnet>" << std::endl;
  }
  ...
  out << "    <valid min=\"" << iface.ValidMin_ << "\" max=\"" << iface.ValidMax_ << "\" />"
      << endl;
  ...
  out << "    <!-- IA: non-temporary addr class count: " << iface.SrvCfgAddrClassLst_.count()
      << "-->" << endl;
  ...
  out << "    <!-- PD: prefix delegation class count: " << iface.SrvCfgPDLst_.count() << "-->"
      << endl;
  ...
  SPtr<TSrvCfgTA> ta;
  iface.firstTA();
  out << "    <!-- TA: temporary IPv6 addr class count: " << iface.SrvCfgTALst_.count() << "-->"
      << endl;
  ...
  /*    if (iface.supportVendorSpec()) {
          out << "    <vendorSpecList count=\"" << iface.VendorSpec.count() <<
     "\">" << endl; iface.VendorSpec.first(); SPtr<TSrvOptVendorSpec> v; while
     (v = iface.VendorSpec.get()) { out << "      <vendorSpec vendor=\"" <<
     v->getVendor() << "\">" << endl; SPtr<TOpt> sub; v->firstOption(); while
     (sub = v->getOption()) { out << "        <option code=\"" <<
     sub->getOptType()
                      << "\" length=\"" << sub->getSize() << "\">"
  ...
  for (TOptList::iterator extra = extraLst.begin(); extra != extraLst.end(); ++extra) {
    out << "      <extraOption type=\"" << (*extra)->getOptType() << "\" length=\""
        << (*extra)->getSize() << "\">" << ((*extra)->getPlain()) << "</extraOption>" << endl;
  }
  ...
  out << "    <!-- " << iface.ExceptionsLst_.count() << " per-client parameters (exceptions) -->"
      << endl;
...
uint32_t TSrvCfgIface::chooseTime(uint32_t min, uint32_t max, uint32_t proposal) {
  if (proposal < min) return min;
  if (proposal > max) return max;
  return proposal;
}

uint32_t TSrvCfgIface::getT1(uint32_t proposal) { return chooseTime(T1Min_, T1Max_, proposal); }

uint32_t TSrvCfgIface::getT2(uint32_t proposal) { return chooseTime(T2Min_, T2Max_, proposal); }
...
///
/// @return YES, NO or UNKNOWN (if no subnet is defined and addr is outside of
/// pool)
EAddrStatus TSrvCfgIface::confirmAddress(TIAType type, SPtr<TIPv6Addr> addr) {

  string what = "address";
  if (type == IATYPE_PD) what = "prefix";

  Log(Debug) << "Confirm that " << what << " " << addr->getPlain() << " is on-link:";
