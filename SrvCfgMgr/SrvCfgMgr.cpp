...
bool TSrvCfgMgr::parseConfigFile(const std::string &cfgFile) {
  ...
  Log(Debug) << "Bulk-leasequery: enabled=" << (BulkLQAccept ? "yes" : "no")
             << ", TCP port=" << BulkLQTcpPort << ", max conns=" << BulkLQMaxConns
...
bool TSrvCfgMgr::matchParsedSystemInterfaces(SrvParser *parser) {
  ...
  while (cfgIface = parser->SrvCfgIfaceLst.get()) {
    ...
    if (cfgIface->isRelay()) {
      ...
      if (!ifaceIface) {
        Log(Crit) << "Interface " << cfgIface->getFullName() << " defined physical interface as "
                  << cfgIface->getRelayName() << ", but there is no such interface present."
                  << LogEnd;
        return false;
      }
    ...
    if (!ifaceIface->countLLAddress()) {
      if (this->inactiveMode()) {
        Log(Notice) << "Interface " << ifaceIface->getFullName()
                    << " is not operational yet (does not have link scope "
                       "address), skipping it for now."
                    << LogEnd;
        this->addIface(cfgIface);
        this->makeInactiveIface(cfgIface->getID(),
                                true);  // move it to InactiveLst
        continue;
      }
    ...
    // Check if the interface is during bring-up phase (i.e. DAD procedure for
    // link-local addr is not complete yet)
    char tmp[64];
    ...
    if (is_addr_tentative(ifaceIface->getName(), ifaceIface->getID(), tmp) ==
      ...
      if (this->inactiveMode()) {
        Log(Notice) << "Interface " << ifaceIface->getFullName()
                    << " is not operational yet (link-scope address is not "
                       "ready), skipping it for now."
                    << LogEnd;
        addIface(cfgIface);
      ...
      Log(Crit) << "Interface " << ifaceIface->getFullName()
                << " has tentative link-scope address (and inactive-mode is "
                   "disabled)."
                << LogEnd;
      return false;
    }
    ...
    this->addIface(cfgIface);
    Log(Info) << "Interface " << cfgIface->getFullName() << " configuration has been loaded."
              << LogEnd;
  }
  return true;
}

SPtr<TSrvCfgIface> TSrvCfgMgr::getIface() { return this->SrvCfgIfaceLst.get(); }

void TSrvCfgMgr::addIface(SPtr<TSrvCfgIface> ptr) { SrvCfgIfaceLst.append(ptr); }
 ...
 */
int TSrvCfgMgr::inactiveIfacesCnt() { return InactiveLst.count(); }

void TSrvCfgMgr::firstIface() { SrvCfgIfaceLst.first(); }

long TSrvCfgMgr::countIface() { return SrvCfgIfaceLst.count(); }
 ...
 *
 * @returns Interface that becomes ready and has been moved out of inactive
 * list. If no such interface exists, 0 will be returned.
 */
SPtr<TSrvCfgIface> TSrvCfgMgr::checkInactiveIfaces() {
  if (!InactiveLst.count()) return SPtr<TSrvCfgIface>();  // NULL
    ...
    if (iface->flagUp() && iface->flagRunning() && iface->getLLAddress()) {
      ...
      if (is_addr_tentative(iface->getName(), iface->getID(), tmp) == LOWLEVEL_TENTATIVE_YES) {
        Log(Debug) << "Interface " << iface->getFullName()
                   << " is up and running, but link-scope address " << tmp
...
TSrvCfgMgr::~TSrvCfgMgr() { Log(Debug) << "SrvCfgMgr cleanup." << LogEnd; }
...
long TSrvCfgMgr::countAvailAddrs(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr, int iface) {
  ...
  while (ptrClass = ptrIface->getAddrClass()) {
    if (!ptrClass->clntSupported(clntDuid, clntAddr)) continue;
  ...
  if (avail > (ifaceMaxLease - ifaceAssigned)) avail = ifaceMaxLease - ifaceAssigned;
  return (long)avail;
}
...
SPtr<TSrvCfgAddrClass> TSrvCfgMgr::getClassByAddr(int iface, SPtr<TIPv6Addr> addr) {
  ...
  ptrIface->firstAddrClass();
  while (ptrClass = ptrIface->getAddrClass()) {
    if (ptrClass->addrInPool(addr)) return ptrClass;
  }
...
SPtr<TSrvCfgPD> TSrvCfgMgr::getClassByPrefix(int iface, SPtr<TIPv6Addr> addr) {
  ...
  while (ptrClass = ptrIface->getPD()) {
    if (ptrClass->prefixInPool(addr)) return ptrClass;
  }
...
SPtr<TIPv6Addr> TSrvCfgMgr::getRandomAddr(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr,
                                          int iface) {
  SPtr<TSrvCfgIface> ptrIface;
  SPtr<TSrvCfgAddrClass> ptrClass;
  SPtr<TIPv6Addr> any;
...
bool TSrvCfgMgr::isClntSupported(SPtr<TSrvMsg> msg) {
  ...
  if (ptrIface) {
    ...
    ptrIface->firstAddrClass();
    while (ptrClass = ptrIface->getAddrClass()) {
      if (ptrClass->clntSupported(duid, clntAddr)) return true;
      classCnt++;
    }
    ...
    while (pd = ptrIface->getPD()) {
      if (pd->clntSupported(duid, clntAddr, msg)) return true;
      classCnt++;
    }
  }
  if (!classCnt && ptrIface) {
    Log(Warning) << "There are no address class defined on the " << ptrIface->getFullName()
                 << ". Maybe you are trying to configure clients on cascade relay "
                    "interface? "
...
bool TSrvCfgMgr::addrReserved(SPtr<TIPv6Addr> addr) {
  ...
  while (iface = SrvCfgIfaceLst.get()) {
    if (iface->addrReserved(addr)) return true;
  }
  return false;
}
...
bool TSrvCfgMgr::prefixReserved(SPtr<TIPv6Addr> prefix) {
  ...
  SrvCfgIfaceLst.first();
  while (iface = SrvCfgIfaceLst.get()) {
    if (iface->prefixReserved(prefix)) return true;
  }
  return false;
}

bool TSrvCfgMgr::isDone() { return IsDone; }
...
bool TSrvCfgMgr::validateConfig() {
  ...
  while (ptrIface = getIface()) {
    if (!this->validateIface(ptrIface)) return false;
  }
  return true;
}
  ...
  if (!ptrIface->countAddrClass() && !ptrIface->countPD() && !ptrIface->getTA() && !stateless()) {
    if (!ptrIface->isRelay()) {
      Log(Crit) << "Config problem: Interface " << ptrIface->getName() << "/" << ptrIface->getID()
                << ": No class definitions (IA,TA or PD) present, but "
                   "stateless mode not set."
                << LogEnd;
      return false;
    } else {
      Log(Notice) << "Interface " << ptrIface->getFullName()
                  << " has no address or prefix pools defined." << LogEnd;
    }
  }

  if (ptrIface->supportFQDN() && !ptrIface->getExtraOption(OPTION_DNS_SERVERS)) {
    Log(Crit) << "FQDN defined on the " << ptrIface->getFullName()
              << ", but no DNS servers defined."
...
bool TSrvCfgMgr::validateClass(SPtr<TSrvCfgIface> ptrIface, SPtr<TSrvCfgAddrClass> ptrClass) {
  if (ptrClass->isLinkLocal()) {
    Log(Crit) << "One of the classes defined on the " << ptrIface->getName() << "/"
              << ptrIface->getID() << " overlaps with link local addresses." << LogEnd;
    return false;
  }
  ...
  if (ptrClass->getPref(0) > ptrClass->getValid(0x7fffffff)) {
    Log(Crit) << "Prefered time max value (" << ptrClass->getPref(0x7fffffff)
              ...
              << ") on the " << ptrIface->getName() << "/" << ptrIface->getID() << " interface."
              << LogEnd;
    return false;
  }
  if (ptrClass->getT1(0) > ptrClass->getT2(0x7fffffff)) {
    Log(Crit) << "T1 timeout upper bound (" << ptrClass->getPref(0x7fffffff)
              << " can't be lower than T2 lower bound (" << ptrClass->getT2(0) << ") on the "
...
SPtr<TSrvCfgIface> TSrvCfgMgr::getIfaceByID(int iface) {
  ...
  firstIface();
  while (ptrIface = getIface()) {
    if (ptrIface->getID() == iface) return ptrIface;
  }
  Log(Error) << "Invalid interface (ifindex=" << iface << ") specifed: no such interface."
             << LogEnd;
  return SPtr<TSrvCfgIface>();  // NULL
}
...
void TSrvCfgMgr::delClntAddr(int iface, SPtr<TIPv6Addr> addr) {
  ...
  ptrIface = this->getIfaceByID(iface);
  if (!ptrIface) {
    Log(Warning) << "Unable to decrease address usage: unknown interface (id=" << iface << ")"
                 << LogEnd;
    return;
  }
...
void TSrvCfgMgr::addClntAddr(int iface, SPtr<TIPv6Addr> addr) {
  ...
  ptrIface = this->getIfaceByID(iface);
  if (!ptrIface) {
    Log(Warning) << "Unable to increase address usage: unknown interface (id=" << iface << ")"
                 << LogEnd;
    return;
  }
...
void TSrvCfgMgr::addTAAddr(int iface) {
  ...
  ptrIface = this->getIfaceByID(iface);
  if (!ptrIface) {
    Log(Error) << "Unable to increase address usage: interface (id=" << iface << ") not found."
               << LogEnd;
    return;
  }
  ptrIface->addTAAddr();
...
void TSrvCfgMgr::delTAAddr(int iface) {
  ...
  ptrIface = this->getIfaceByID(iface);
  if (!ptrIface) {
    Log(Error) << "Unable to decrease address usage: interface (id=" << iface << ") not found."
               << LogEnd;
    return;
  }
  ptrIface->delTAAddr();
...
bool TSrvCfgMgr::stateless() { return this->Stateless; }

bool TSrvCfgMgr::inactiveMode() { return InactiveMode; }

bool TSrvCfgMgr::guessMode() { return GuessMode; }
...
bool TSrvCfgMgr::setupRelay(SPtr<TSrvCfgIface> cfgIface) {
  ...
  if (!iface) {
    Log(Crit) << "Underlaying interface for " << cfgIface->getFullName() << " with name " << name
              << " is missing." << LogEnd;
    return false;
  }
 ...
 * @return
 */
int TSrvCfgMgr::getCacheSize() { return this->CacheSize; }

ESrvIfaceIdOrder TSrvCfgMgr::getInterfaceIDOrder() { return InterfaceIDOrder; }
...
bool TSrvCfgMgr::decrPrefixCount(int ifindex, SPtr<TIPv6Addr> prefix) {
  ...
  if (!iface) {
    Log(Error) << "Unable to find interface with ifindex=" << ifindex
               << ", prefix deletion aborted." << LogEnd;
    return false;
  }
...
bool TSrvCfgMgr::incrPrefixCount(int ifindex, SPtr<TIPv6Addr> prefix) {
  ...
  if (!iface) {
    Log(Error) << "Unable to find interface with ifindex=" << ifindex
               << ", prefix increase count aborted." << LogEnd;
    return false;
  }
...
void TSrvCfgMgr::setAuthDigests(const DigestTypesLst &types) { DigestTypesLst_ = types; }

DigestTypesLst TSrvCfgMgr::getAuthDigests() { return DigestTypesLst_; }

enum DigestTypes TSrvCfgMgr::getDigest() {
  if (DigestTypesLst_.empty()) return DIGEST_NONE;
...
/// @todo: Fix this! We check if message is from accepted client, but don't do
/// anything with that info
...
void TSrvCfgMgr::setReconfigureSupport(bool reconf) { Reconfigure_ = reconf; }

bool TSrvCfgMgr::getReconfigureSupport() { return Reconfigure_; }
...
/**
 * sets pool usage counters (used during bringup, after AddrDB is loaded from
 * file)
 *
 */
void TSrvCfgMgr::setCounters() {
  ...
  Log(Debug) << "Increased pools usage: currently " << iaCnt << " address(es) and " << pdCnt
             << " prefix(es) are leased." << LogEnd;
}
...
TSrvCfgMgr &TSrvCfgMgr::instance() {
  if (!Instance) {
    Log(Crit) << "SrvCfgMgr not initalized yet. Application error. Emergency "
                 "shutdown."
              << LogEnd;
    exit(EXIT_FAILURE);
  }
...
// Bulk-LeaseQuery
void TSrvCfgMgr::bulkLQAccept(bool enabled) { BulkLQAccept = enabled; }

void TSrvCfgMgr::bulkLQTcpPort(unsigned short portNumber) { BulkLQTcpPort = portNumber; }

void TSrvCfgMgr::bulkLQMaxConns(unsigned int maxConnections) { BulkLQMaxConns = maxConnections; }

void TSrvCfgMgr::bulkLQTimeout(unsigned int timeout) { BulkLQTimeout = timeout; }

/// Sets DNS server address suitable for DNS Update
///
/// @param ddnsAddress DNS server address
void TSrvCfgMgr::setDDNSAddress(SPtr<TIPv6Addr> ddnsAddress) { FqdnDdnsAddress = ddnsAddress; }
...
SPtr<TIPv6Addr> TSrvCfgMgr::getDDNSAddress(int iface) {
  if (FqdnDdnsAddress) return FqdnDdnsAddress;

  SPtr<TSrvCfgIface> ptrIface = this->getIfaceByID(iface);
  if (!ptrIface) {
    Log(Warning) << "No global DNS Update address specified and can't find "
                    "dns-address on "
  ...
  if (!opt) {
    Log(Error) << "DDNS: DNS Update aborted. DNS server address is not "
  ...
  DNSSrvLst.first();
  if (DNSSrvLst.count()) DNSAddr = DNSSrvLst.get();
...
void TSrvCfgMgr::setPerformanceMode(bool mode) { PerformanceMode_ = mode; }

bool TSrvCfgMgr::getPerformanceMode() { return PerformanceMode_; }

void TSrvCfgMgr::dropUnicast(bool drop) { DropUnicast_ = drop; }

bool TSrvCfgMgr::dropUnicast() { return DropUnicast_; }
