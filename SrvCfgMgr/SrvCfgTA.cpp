...
TSrvCfgTA::TSrvCfgTA()
    : Pref(SERVER_DEFAULT_TA_PREF_LIFETIME),
      Valid(SERVER_DEFAULT_TA_VALID_LIFETIME),
      ClassMaxLease(SERVER_DEFAULT_CLASS_MAX_LEASE),
      AddrsAssigned(0),
      AddrsCount(0) {
  ID = staticID++;
}
...
bool TSrvCfgTA::clntSupported(SPtr<TDUID> clntDuid, SPtr<TIPv6Addr> clntAddr) {
  ...
  while (range = RejedClnt.get())
    if (range->in(clntDuid, clntAddr)) return false;
  ...
  if (AcceptClnt.count()) {
    ...
    while (range = AcceptClnt.get()) {
      ...
      if (range->in(clntDuid, clntAddr)) return true;
    }
    return false;
  }
...
bool TSrvCfgTA::clntSupported(SPtr<TDUID> duid, SPtr<TIPv6Addr> clntAddr, SPtr<TSrvMsg> msg) {
  ...
  if (allowClientClassLst.count()) return false;
  return true;
}
...
unsigned long TSrvCfgTA::getPref() { return this->Pref; }

unsigned long TSrvCfgTA::getValid() { return this->Valid; }
...
void TSrvCfgTA::setOptions(SPtr<TSrvParsGlobalOpt> opt) {
  ...
  opt->firstRejedClnt();
  while (statRange = opt->getRejedClnt()) this->RejedClnt.append(statRange);

  // copy white-list
  opt->firstAcceptClnt();
  while (statRange = opt->getAcceptClnt()) this->AcceptClnt.append(statRange);
  ...
  if (this->ClassMaxLease > this->AddrsCount) this->ClassMaxLease = this->AddrsCount;
...
unsigned long TSrvCfgTA::countAddrInPool() { return this->AddrsCount; }

SPtr<TIPv6Addr> TSrvCfgTA::getRandomAddr() { return Pool->getRandomAddr(); }

unsigned long TSrvCfgTA::getClassMaxLease() { return ClassMaxLease; }

unsigned long TSrvCfgTA::getID() { return this->ID; }
...
unsigned long TSrvCfgTA::getAssignedCount() { return this->AddrsAssigned; }

bool TSrvCfgTA::addrInPool(SPtr<TIPv6Addr> addr) { return Pool->in(addr); }

ostream &operator<<(ostream &out, TSrvCfgTA &addrClass) {
  out << "    <taClass id=\"" << addrClass.ID << "\" pref=\"" << addrClass.Pref << "\" valid=\""
      << addrClass.Valid << "\">" << endl;
  ...
  out << "      <!-- reject-clients (black list) ranges:" << addrClass.RejedClnt.count() << " -->"
      << endl;
  addrClass.RejedClnt.first();
  while (statRange = addrClass.RejedClnt.get()) out << *statRange;

  out << "      <!-- accept-only (white list) ranges:" << addrClass.AcceptClnt.count() << " -->"
      << endl;
