...
/* Useful links:
 msg_control info: http://docs.oracle.com/cd/E23824_01/html/821-1466/recvmsg-3xnet.html#scrolltoc
...
void if_print(struct iface * iface_ptr) {
    ...
    printf("Interface %s, index=%i type=%x flags=%x\n", iface_ptr->name, iface_ptr->id, iface_ptr->hardwareType,
           iface_ptr->flags);
    ...
    for (tmp = 0; tmp < iface_ptr->linkaddrcount; tmp++) {
        ...
        for (tmpInt = 0; tmpInt < 16; tmpInt += 2) {
            printf("%02x%02x:", (unsigned char)iface_ptr->linkaddr[tmpInt + tmp * 16],
    ...
    printf("\t%s Global IPv6 address count: %i, address ", iface_ptr->name, iface_ptr->globaladdrcount);
    tmpInt = 0;
    for (tmp = 0; tmp < iface_ptr->globaladdrcount; tmp++) {
        printf("\t%i=", tmp);
        for (tmpInt = 0; tmpInt < 16; tmpInt += 2) {
            printf("%02x%02x:", (unsigned char)iface_ptr->globaladdr[tmpInt + tmp * 16],
        ...
        for (iface_ptr = iface_lst; iface_ptr != NULL; iface_ptr = iface_ptr->next) {
            ...
            case AF_LINK: {
                ...
                if (linkInfo->sdl_alen > 1) {
                    memcpy(iface_ptr->mac, LLADDR(linkInfo), linkInfo->sdl_alen);
...
int ipaddr_add(const char * ifacename, int ifaceid, const char * addr, unsigned long pref, unsigned long valid,
               int prefixLength) {
    char buf[512];
    int status;
...
int sock_add(char * ifacename, int ifaceid, char * addr, int port, int thisifaceonly, int reuse) {
    ...
    /* just because I can't make Solaris version of strncasecmp to work */
    if (strlen(addr) >= 2 && (tolower(addr[0]) == 'f') && (tolower(addr[1]) == 'f'))
        multicast = 1;
    else
        multicast = 0;

#ifdef LOWLEVEL_DEBUG
    printf("LOWLEVEL_DEBUG iface: %s(id=%d), addr=%s, port=%d, ifaceonly=%d reuse=%d###\n", ifacename, ifaceid, addr, port,
           thisifaceonly, reuse);
    fflush(stdout);
#endif
