...
TSrvService::TSrvService()
  ...
  // Vista and above have IPv6 in the TCP stack and it's not a standalone
  // service.
...
EServiceState TSrvService::ParseStandardArgs(int argc, char *argv[]) {
  ...
  if (!dirFound) {
    // Log(Notice) << "-d parameter missing. Trying to load server.conf from
  ...
  if (tmp[0] == '.' && tmp[1] == '.') return status;  // relative directory
  if (tmp[0] == '\\' && tmp[1] == '\\') {
  ...
  if (*(tmp + 1) != ':') {
    Log(Notice) << "Invalid or not absolute directory [" << ServiceDir
                << "]: second character is not a ':'. "
                << "Please use full absolute"
                << " pathname, e.g. C:\\dibbler" << LogEnd;
  }
  if (*(tmp + 2) != '\\' && *(tmp + 2) != '/') {
    Log(Notice) << "Invalid or absolute directory [" << ServiceDir
                << "]: third character is not a '/' nor '\\'. "
                << "Please use full absolute"
                << " pathname, e.g. C:\\dibbler" << LogEnd;
  }
  return status;
...
void TSrvService::OnStop() { srvPtr->stop(); }
...
void TSrvService::Run() {
  ...
  if (!server.isDone()) server.run();
}

void TSrvService::setState(EServiceState status) { this->status = status; }
