...
EServiceState TClntService::ParseStandardArgs(int argc, char *argv[]) {
  ...
  if (!dirFound) {
    // Log(Notice) << "-d parameter missing. Trying to load client.conf from
  ...
  if (ServiceDir.length() < 3) {
    Log(Crit) << "Invalid directory [" << ServiceDir
              << "]: too short. Please use full absolute pathname, e.g. C:\\dibbler" << LogEnd;
    return INVALID;
  }
...
void TClntService::OnShutdown() { clntPtr->stop(); }

void TClntService::OnStop() { clntPtr->stop(); }
...
void TClntService::Run() {
  ...
  if (!client.isDone()) client.run();
}

void TClntService::setState(EServiceState status) { this->status = status; }
