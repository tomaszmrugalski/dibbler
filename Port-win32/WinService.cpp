...
TWinService::TWinService(const char *serviceName, const char *dispName, DWORD serviceType,
                         char *dependencies, char *descr) {
  ServicePtr = this;
...
void TWinService::LogEvent(WORD wType, DWORD dwID, const char *pszS1, const char *pszS2,
                           const char *pszS3) {
  const char *ps[3];
  ps[0] = pszS1;
  ps[1] = pszS2;
...
bool TWinService::IsInstalled() { return this->IsInstalled(this->ServiceName); }
...
bool TWinService::Install() {
  ...
  SC_HANDLE hService = CreateService(hSCM, ServiceName, DisplayName, SERVICE_ALL_ACCESS,
                                     SERVICE_WIN32_OWN_PROCESS, ServiceType, SERVICE_ERROR_NORMAL,
...
bool TWinService::RunService() {
  SERVICE_TABLE_ENTRY st[] = {{ServiceName, ServiceMain}, {NULL, NULL}};
...
bool TWinService::OnInit() { return true; }
...
bool TWinService::OnUserControl(DWORD dwOpcode) { return false; }

int TWinService::getStatus() { return Status.dwCurrentState; }

bool TWinService::isRunning() { return isRunning(this->ServiceName); }
...
bool TWinService::verifyPort() {
  ...
  if ((verinfo.dwMajorVersion == 5) && (verinfo.dwMinorVersion == 1)) {
    Log(Notice) << "Windows XP detected (majorVersion=" << verinfo.dwMajorVersion
                << ", minorVersion=" << verinfo.dwMinorVersion << "), so this is proper port."
                << LogEnd;
    ok = true;
  }
  ...
  if (!ok) {
    ...
    Log(Notice) << "Unsupported systems (there's separate Dibbler version for "
                   "those systems):"
                << LogEnd;
