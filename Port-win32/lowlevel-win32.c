...
void error_message_set(int errCode) {
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, NULL, errCode,
                  MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)tmp, ERROR_MESSAGE_SIZE - 10, NULL);
...
extern struct iface * if_list_get() {
    ...
    buflen = 0;
    GetAdaptersAddresses(AF_INET6, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER, NULL,
                         (PIP_ADAPTER_ADDRESSES)buffer, &buflen);
    ...
    while (adaptaddr) {
        ...
        WideCharToMultiByte(CP_ACP, 0, adaptaddr->FriendlyName, -1, iface->name, MAX_IFNAME_LENGTH, NULL, NULL);
...
extern int ipaddr_add(const char * ifacename, int ifaceid, const char * addr, unsigned long pref, unsigned long valid,
                      int prefixLen) {
    ...
    if (i) {
        i = _spawnl(_P_WAIT, netshPath, netshPath, arg1, arg2, "set", arg4, arg5, arg6, arg7, arg8, NULL);
    }
    return i;
}

int ipaddr_update(const char * ifacename, int ifindex, const char * addr, unsigned long pref, unsigned long valid,
                  int prefixLength) {
...
int sock_send(int fd, char * addr, char * buf, int buflen, int port, int iface) {
    ...
    if (IN6_IS_ADDR_LINKLOCAL((struct in6_addr *)packaddr) || IN6_IS_ADDR_SITELOCAL((struct in6_addr *)packaddr))
...
extern int dns_add(const char * ifname, int ifaceid, const char * addrPlain) {
    ...
    if (i == 0) {
        ...
        sprintf(Message, "%s %s %s %s %s %s %s returned non-zero returncode %d", netshPath, arg1, arg2, arg3, arg4, arg5, arg6,
                i);
        return LOWLEVEL_ERROR_UNSPEC;
    }
}
...
int prefix_add(const char * ifname, int ifindex, const char * prefixPlain, int prefixLength, unsigned long prefered,
               unsigned long valid) {
...
int prefix_update(const char * ifname, int ifindex, const char * prefixPlain, int prefixLength, unsigned long prefered,
                  unsigned long valid) {
