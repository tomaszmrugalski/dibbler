...
class NakedSrvIfaceMgr : public TSrvIfaceMgr {
  public:
    ...
    NakedSrvIfaceMgr(const std::string & xmlFile) : TSrvIfaceMgr(xmlFile) {
...
class NakedSrvAddrMgr : public TSrvAddrMgr {
  public:
    NakedSrvAddrMgr(const std::string & config, bool load_db) : TSrvAddrMgr(config, load_db) {
...
class NakedSrvCfgMgr : public TSrvCfgMgr {
  public:
    NakedSrvCfgMgr(const std::string & config, const std::string & dbfile) : TSrvCfgMgr(config, dbfile) {
...
class NakedSrvTransMgr : public TSrvTransMgr {
  public:
    NakedSrvTransMgr(const std::string & xmlFile, int port) : TSrvTransMgr(xmlFile, port) {
        TSrvTransMgr::Instance = this;
    }

    virtual void sendPacket(SPtr<TSrvMsg> msg);

    SrvMsgList & getMsgLst() {
        return MsgLst_;
    }
...
class ServerTest : public ::testing::Test {
  public:
    ...
    SPtr<TSrvMsg> appendOptions(const SPtr<TSrvMsg> & msg, bool include_ia, bool include_client_id, bool include_pd);

    /// @brief creates SOLICIT message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createSolicit(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates REQUEST message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createRequest(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates RENEW message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createRenew(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates REBIND message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createRebind(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates RELEASE message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createRelease(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates DECLINE message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createDecline(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates CONFIRM message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createConfirm(bool include_clntid = false, bool include_ia = false, bool include_pd = false);

    /// @brief creates INF-REQUEST message and IA_NA, IA_TA, IA_PD options
    SPtr<TSrvMsg> createInfRequest(bool include_clntid = false, bool include_ia = false, bool include_pd = false);
    ...
    void prefixText(const std::string & config, const TOptPtr & pd_to_be_sent, const std::string & min_range,
                    const std::string & max_range, uint8_t expected_prefix_len, uint32_t expected_iaid, uint32_t expected_t1,
                    uint32_t expected_t2, uint32_t expected_pref, uint32_t expected_valid);

    SPtr<TSrvMsg> sendAndReceive(SPtr<TSrvMsg> clntMsg, unsigned int expectedMsgCount = 1);

    bool checkIA_NA(SPtr<TSrvOptIA_NA> ia, SPtr<TIPv6Addr> minRange, SPtr<TIPv6Addr> maxRange, uint32_t iaid, uint32_t t1,
                    uint32_t t2, uint32_t pref, uint32_t valid);

    bool checkIA_PD(SPtr<TSrvOptIA_PD> ia, SPtr<TIPv6Addr> minRange, SPtr<TIPv6Addr> maxRange, uint32_t iaid, uint32_t t1,
                    uint32_t t2, uint32_t pref, uint32_t valid, uint8_t prefixLen);

    void addRelayInfo(const std::string & linkAddr, const std::string & peerAddr, uint8_t hopCount, const TOptList & echoList);

    void sendHex(const std::string & src_addr, uint16_t src_port, const std::string & dst_addr, uint16_t dst_port,
                 const std::string & iface_name, const std::string & hex_data);
