...
public:
  ...
  ~NakedSrvIfaceMgr() { TSrvIfaceMgr::Instance = NULL; }
  virtual bool send(int iface, char *msg, int size, SPtr<TIPv6Addr> addr,
                    int port);
...
public:
  ...
  ~NakedSrvAddrMgr() { TSrvAddrMgr::Instance = NULL; }
};
...
public:
  ...
  ~NakedSrvCfgMgr() { TSrvCfgMgr::Instance = NULL; }
};
...
public:
  ...
  ~NakedSrvTransMgr() { TSrvTransMgr::Instance = NULL; }
...
public:
  ...
  /// @brief Creates IA_NA, IA_TA, IA_PD, client-id and appends them if
  /// necessary
  ...
  SPtr<TSrvMsg> createSolicit(bool include_clntid = false,
                              bool include_ia = false, bool include_pd = false);

  /// @brief creates REQUEST message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createRequest(bool include_clntid = false,
                              bool include_ia = false, bool include_pd = false);

  /// @brief creates RENEW message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createRenew(bool include_clntid = false,
                            bool include_ia = false, bool include_pd = false);

  /// @brief creates REBIND message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createRebind(bool include_clntid = false,
                             bool include_ia = false, bool include_pd = false);

  /// @brief creates RELEASE message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createRelease(bool include_clntid = false,
                              bool include_ia = false, bool include_pd = false);

  /// @brief creates DECLINE message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createDecline(bool include_clntid = false,
                              bool include_ia = false, bool include_pd = false);

  /// @brief creates CONFIRM message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createConfirm(bool include_clntid = false,
                              bool include_ia = false, bool include_pd = false);

  /// @brief creates INF-REQUEST message and IA_NA, IA_TA, IA_PD options
  SPtr<TSrvMsg> createInfRequest(bool include_clntid = false,
                                 bool include_ia = false,
  ...
  TOptPtr createPrefix(const std::string &addr_txt, uint8_t len, uint32_t pref,
                       uint32_t valid);

  /// @brief Conducts prefix delegation assignment test
  ///
  /// Sends SOLCIT with IA_PD, checks that the ADVERTISE contains proper
  /// response with expected values, then sends REQUEST and repeats the checks
  /// for received REPLY.
  ///
  /// @param config
  /// @param pd_to_be_sent
