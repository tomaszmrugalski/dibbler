...
bool TRelCfgMgr::parseConfigFile(const std::string &cfgFile) {
  ...
  if (guessMode())
    Log(Debug) << "Guess-mode enabled. RELAY-REPL from the server without "
                  "interface-id option will be accepted."
               << LogEnd;
  else
    Log(Debug) << "Guess-mode disabled. RELAY-REPL from the server without "
                  "interface-id option will be dropped."
               << LogEnd;
...
SPtr<TRelCfgIface> TRelCfgMgr::getIface() { return this->IfaceLst.get(); }

void TRelCfgMgr::addIface(SPtr<TRelCfgIface> ptr) { IfaceLst.append(ptr); }

void TRelCfgMgr::firstIface() { IfaceLst.first(); }

long TRelCfgMgr::countIface() { return IfaceLst.count(); }

TRelCfgMgr::~TRelCfgMgr() { Log(Debug) << "RelCfgMgr cleanup." << LogEnd; }

bool TRelCfgMgr::isDone() { return IsDone; }

bool TRelCfgMgr::guessMode() { return GuessMode; }
...
bool TRelCfgMgr::validateConfig() {
  ...
  if (this->countIface() < 2) {
    Log(Warning) << "There is only one interface defined. At least two is a "
...
bool TRelCfgMgr::validateIface(SPtr<TRelCfgIface> ptrIface) {
  if ((ptrIface->getInterfaceID() == (signed int)DHCPV6_INFINITY) &&
    ...
    Log(Crit) << "Client address defined without Interface-ID on the " << ptrIface->getFullName()
              << " interface." << LogEnd;
    return false;
  }
...
SPtr<TRelCfgIface> TRelCfgMgr::getIfaceByInterfaceID(int iface) {
  ...
  this->firstIface();
  while (ptrIface = this->getIface()) {
    if (ptrIface->getInterfaceID() == iface) return ptrIface;
  }
  Log(Error) << "There is no interface with interfaceID=" << iface << " in the CfgMgr." << LogEnd;
  return SPtr<TRelCfgIface>();  // NULL
}

ERelIfaceIdOrder TRelCfgMgr::getInterfaceIDOrder() { return InterfaceIDOrder; }

SPtr<TOptVendorData> TRelCfgMgr::getRemoteID() { return RemoteID; }

SPtr<TRelOptEcho> TRelCfgMgr::getEcho() { return Echo; }

void TRelCfgMgr::instanceCreate(const std::string &cfgFile, const std::string &xmlFile) {
  if (Instance) Log(Crit) << "RelCfgMgr instance already created. Application error!" << LogEnd;
  Instance = new TRelCfgMgr(cfgFile, xmlFile);
}

TRelCfgMgr &TRelCfgMgr::instance() {
  if (!Instance) {
    Log(Crit) << "RelCfgMgr instance not created yet. Application error. "
                 "Emergency shutdown."
              << LogEnd;
    exit(EXIT_FAILURE);
  }
...
void TRelCfgMgr::setRelayID(SPtr<TOpt> relayID) { RelayID_ = relayID; }

SPtr<TOpt> TRelCfgMgr::getRelayID() { return RelayID_; }

void TRelCfgMgr::setClientLinkLayerAddress(bool enabled) { ClientLinkLayerAddress_ = enabled; }

bool TRelCfgMgr::getClientLinkLayerAddress() { return ClientLinkLayerAddress_; }
