...
bool TRelCfgMgr::matchParsedSystemInterfaces(List(TRelCfgIface) * lst) {
    ...
    while (cfgIface = lst->get()) {
        ...
        Log(Info) << "Interface " << cfgIface->getName() << "/" << cfgIface->getID() << " configuration has been loaded."
                  << LogEnd;
    }
    return true;
}
...
bool TRelCfgMgr::validateIface(SPtr<TRelCfgIface> ptrIface) {
    if ((ptrIface->getInterfaceID() == (signed int)DHCPV6_INFINITY) &&
        ...
        Log(Crit) << "Client address defined without Interface-ID on the " << ptrIface->getFullName() << " interface."
                  << LogEnd;
        return false;
    }
    if (!ptrIface->getServerUnicast() && !ptrIface->getServerMulticast() && !ptrIface->getClientUnicast() &&
        !ptrIface->getClientMulticast()) {
        Log(Warning) << "Both server and client support is defined on the " << ptrIface->getName() << "/" << ptrIface->getID()
                     << " interface." << LogEnd;
    }
