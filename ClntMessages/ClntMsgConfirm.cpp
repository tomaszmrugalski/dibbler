...
TClntMsgConfirm::TClntMsgConfirm(unsigned int iface, List(TAddrIA) iaLst) : TClntMsg(iface, SPtr<TIPv6Addr>(), CONFIRM_MSG) {
...
void TClntMsgConfirm::answer(SPtr<TClntMsg> reply) {
    ...
    default:
        Log(Warning) << "REPLY for CONFIRM received with invalid (" << status->getCode() << ") status code." << LogEnd;
        break;
    }
    return;
}
...
void TClntMsgConfirm::addrsRejected() {
    ...
    while (ptrOpt = this->getOption()) {
        ...
        // not only the address should be rejected, but also the original IA should be deleted and using a new IA to process
        // SOLICIT
