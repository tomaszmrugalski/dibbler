...
TClntMsgRenew::TClntMsgRenew(List(TAddrIA) IALst, List(TAddrIA) PDLst)
  ...
  IALst.first();
  while (ia = IALst.get()) {
    if (timeout > ia->getT2Timeout()) timeout = ia->getT2Timeout();
...
void TClntMsgRenew::answer(SPtr<TClntMsg> Reply) {
  ...
  Reply->firstOption();
  // for each option in message... (there should be only one IA option, as we
  // send separate RENEW for each IA, but we check all options anyway)
  while (opt = Reply->getOption()) {
    switch (opt->getOptType()) {
        ...
        if (ptrOptIA->getStatusCode() != STATUSCODE_SUCCESS) {
          if (ptrOptIA->getStatusCode() == STATUSCODE_NOBINDING) {
            ...
            Log(Warning) << "Received IA (iaid=" << ptrOptIA->getIAID() << ") with status code "
                         << StatusCodeToString(status->getCode()) << ": " << status->getText()
                         << LogEnd;
            break;
          }
        }
      ...
      case OPTION_RECONF_MSG:
        Log(Warning) << "Illegal option (" << opt->getOptType() << ") in received REPLY message."
                     << LogEnd;
        break;
      default:
  ...
  // Here we received answer from our server, which updated the "whole
  // information" There is no use to send Rebind even if server realesed some
  // addresses/IAs in such a case new Solicit message should be sent
  IsDone = true;
}
...
void TClntMsgRenew::doDuties() {
  ...
  if (!MRD) {
    Log(Notice) << "RENEW remains unanswered and timeout T2 reached, so REBIND "
                   "will be sent."
                << LogEnd;
...
std::string TClntMsgRenew::getName() const { return "RENEW"; }
