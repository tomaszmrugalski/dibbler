...
TClntMsgSolicit::TClntMsgSolicit(int iface, SPtr<TIPv6Addr> addr, List(TClntCfgIA) iaLst, SPtr<TClntCfgTA> ta,
                                 List(TClntCfgPD) pdLst, bool rapid, bool remoteAutoconf)
...
void TClntMsgSolicit::answer(SPtr<TClntMsg> msg) {
    ...
    case REPLY_MSG: {
        if (!this->getOption(OPTION_RAPID_COMMIT)) {
            Log(Warning) << "REPLY received, but SOLICIT was sent without RAPID_COMMIT. Ignoring." << LogEnd;
            return;
        }
        if (!msg->getOption(OPTION_RAPID_COMMIT)) {
            Log(Warning) << "REPLY as answer for SOLICIT received without RAPID_COMMIT. Ignoring." << LogEnd;
            return;
        }
    ...
    default:
        Log(Warning) << "Invalid message type (" << msg->getType() << ") received as answer for SOLICIT message." << LogEnd;
        return;
    }
}
...
bool TClntMsgSolicit::shallRejectAnswer(SPtr<TClntMsg> msg) {
    ...
    if (this->getOption(OPTION_IA_NA)) {
        ...
        if (!ia) {
            ...
            if (st && st->getCode() != STATUSCODE_SUCCESS) {
                Log(Notice) << "IA_NA has status code!=SUCCESS: " << st->getCode() << "(" << st->getText() << "). Ignored."
                            << LogEnd;
                iaOk = false;
            }
        }
    ...
    if (this->getOption(OPTION_IA_TA)) {
        ...
        if (!ta) {
            ...
            if (st && st->getCode() != STATUSCODE_SUCCESS) {
                Log(Notice) << "IA_TA has status code!=SUCCESS: " << st->getCode() << "(" << st->getText() << "). Ignored."
                            << LogEnd;
                taOk = false;
            }
        }
    ...
    while (opt_sent = getOption()) {
        ...
        while (opt_rcvd = msg->getOption()) {
            ...
            while (ppref = pdResp->getPrefix()) {
                if (!ppref->isValid()) {
                    Log(Warning) << "Option IA_PREFIX from IA_PD " << pdResp->getIAID() << " is not valid." << LogEnd;
                    // RFC 3633, section 10:
