...
TClntMsgSolicit::TClntMsgSolicit(int iface, SPtr<TIPv6Addr> addr, List(TClntCfgIA) iaLst,
                                 SPtr<TClntCfgTA> ta, List(TClntCfgPD) pdLst, bool rapid,
                                 bool remoteAutoconf)
    : TClntMsg(iface, addr, SOLICIT_MSG) {
  IRT = SOL_TIMEOUT;
  MRT = SOL_MAX_RT;
  MRC = 0;  // these both below mean there is no ending condition and
            // transactions
  ...
  if (ta) {
    ...
    if (!remoteAutoconf) ta->setState(STATE_INPROCESS);
  ...
  while (pd = pdLst.get()) {
    ...
    if (!remoteAutoconf) pd->setState(STATE_INPROCESS);
  ...
  if (rapid) Options.push_back(new TOptEmpty(OPTION_RAPID_COMMIT, this));

  // RECONF-ACCEPT is added in TClntMsg::appendRequestedOptions()

  // append and switch to INPROCESS state
  if (!remoteAutoconf) appendTAOptions(true);

  // append options specified in the config file
  if (!remoteAutoconf) appendRequestedOptions();
  ...
  switch (msg->getType()) {
    case ADVERTISE_MSG: {
      if (this->getOption(OPTION_RAPID_COMMIT)) {
        Log(Info) << "Server responded with ADVERTISE instead of REPLY, probably "
                     "does not support"
                     " RAPID-COMMIT."
                  << LogEnd;
      }
      ...
      if (prefOpt && (prefOpt->getValue() == 255)) {
        Log(Info) << "ADVERTISE message with preference set to 255 received, so "
                     "wait time for"
                     " other possible ADVERTISE messages is skipped."
                  << LogEnd;
    ...
    case REPLY_MSG: {
      if (!this->getOption(OPTION_RAPID_COMMIT)) {
        Log(Warning) << "REPLY received, but SOLICIT was sent without "
                        "RAPID_COMMIT. Ignoring."
                     << LogEnd;
        return;
      }
      if (!msg->getOption(OPTION_RAPID_COMMIT)) {
        Log(Warning) << "REPLY as answer for SOLICIT received without "
                        "RAPID_COMMIT. Ignoring."
                     << LogEnd;
        return;
      }
...
bool TClntMsgSolicit::shallRejectAnswer(SPtr<TClntMsg> msg) {
  ...
  if (this->getOption(OPTION_IA_NA)) {
    /// @todo Check if proper IAIDs are returned, also if all IA were answered
    /// (if requested
    ...
    if (!ia) {
      ...
      if (st && st->getCode() != STATUSCODE_SUCCESS) {
        Log(Notice) << "IA_NA has status code!=SUCCESS: " << st->getCode() << "(" << st->getText()
                    << "). Ignored." << LogEnd;
        iaOk = false;
      }
    }
    if (iaOk) somethingAssigned = true;
  }
  ...
  if (this->getOption(OPTION_IA_TA)) {
    ...
    if (!ta) {
      ...
      if (st && st->getCode() != STATUSCODE_SUCCESS) {
        Log(Notice) << "IA_TA has status code!=SUCCESS: " << st->getCode() << "(" << st->getText()
                    << "). Ignored." << LogEnd;
        taOk = false;
      }
    }
    if (taOk) somethingAssigned = true;
  }
  ...
  while (opt_sent = getOption()) {
    if (opt_sent->getOptType() != OPTION_IA_PD) continue;  // ignore all options except IA_PD
    ...
    while (opt_rcvd = msg->getOption()) {
      ...
      if (!pdResp->getOption(OPTION_IAPREFIX)) {
        Log(Notice) << "Received PD with IAID = " << pdResp->getIAID()
                    << " without "
                       "any prefixes."
                    << LogEnd;
        pdcnt--;
      ...
      while (ppref = pdResp->getPrefix()) {
        if (!ppref->isValid()) {
          Log(Warning) << "Option IA_PREFIX from IA_PD " << pdResp->getIAID() << " is not valid."
                       << LogEnd;
          // RFC 3633, section 10:
  ...
  if (pdOk) somethingAssigned = true;

  if (!somethingAssigned) return true;  // this advertise does not offers us anything

  if (!ClntCfgMgr().insistMode()) return false;  // accept this advertise

  // insist-mode enabled. We MUST get everything we wanted or we reject this
  // answer
...
bool TClntMsgSolicit::check() { return false; }

std::string TClntMsgSolicit::getName() const { return "SOLICIT"; }
