 ...
 */
TClntMsgRelease::TClntMsgRelease(int iface, SPtr<TIPv6Addr> addr, List(TAddrIA) iaLst,
  ...
  while (x = iaLst.get()) {
    ...
    if (!ptrIface) {
      Log(Warning) << "Unable to find interface with ifindex " << x->getIfindex()
                   << " while creating RELEASE." << LogEnd;
      continue;
    }
    x->firstAddr();
  ...
  if (ta) {
    ...
    if (ptrIface) {
      ...
      Log(Warning) << "Unable to find interface with ifindex " << ta->getIfindex()
                   << " while creating RELEASE." << LogEnd;
    }
  }
...
void TClntMsgRelease::answer(SPtr<TClntMsg> rep) {
  ...
  if (!msgSrvID) {
    Log(Error) << "Sent RELEASE did not contain SERVER-ID. That seems to be my "
                  "fault. Sorry."
               << LogEnd;
  }
...
bool TClntMsgRelease::check() { return false; }

std::string TClntMsgRelease::getName() const { return "RELEASE"; }
