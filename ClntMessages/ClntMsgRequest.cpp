 ...
 */
TClntMsgRequest::TClntMsgRequest(TOptList opts, int iface) : TClntMsg(iface, SPtr<TIPv6Addr>(), REQUEST_MSG) {
    IRT = REQ_TIMEOUT;
    MRT = REQ_MAX_RT;
    MRC = REQ_MAX_RC;
    MRD = 0;
    ...
    Log(Info) << "Creating REQUEST. Backup server list contains " << backupCount << " server(s)." << LogEnd;
