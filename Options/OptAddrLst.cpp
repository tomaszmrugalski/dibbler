...
char *TOptAddrLst::storeSelf(char *buf) {
  ...
  AddrLst.first();
  while (addr = AddrLst.get()) buf = addr->storeSelf(buf);
  return buf;
}

size_t TOptAddrLst::getSize() { return 4 + 16 * AddrLst.count(); }

void TOptAddrLst::firstAddr() { this->AddrLst.first(); }

SPtr<TIPv6Addr> TOptAddrLst::getAddr() { return this->AddrLst.get(); }

bool TOptAddrLst::isValid() const { return this->Valid; }
