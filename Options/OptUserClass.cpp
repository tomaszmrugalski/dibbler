...
TOptUserClass::TOptUserClass(uint16_t type, const char * buf, unsigned short buf_len, TMsg * parent) : TOpt(type, parent) {
    Valid = parseUserData(buf, buf_len);
}

TOptUserClass::TOptUserClass(uint16_t type, TMsg * parent) : TOpt(type, parent) {
    Valid = true;
}

size_t TOptUserClass::getSize() {
    size_t len = 4;
    for (std::vector<UserClassData>::const_iterator data = userClassData_.begin(); data != userClassData_.end(); ++data) {
        len += 2; // len field
