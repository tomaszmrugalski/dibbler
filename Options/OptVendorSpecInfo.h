...
public:
  ...
  TOptVendorSpecInfo(uint16_t code, uint32_t enterprise,
                     uint16_t sub_option_code, const char *data, int dataLen,
                     TMsg *parent);
  TOptVendorSpecInfo(uint16_t code, uint32_t enterprise,
                     uint16_t sub_option_code, SPtr<TIPv6Addr> addr,
                     TMsg *parent);
  TOptVendorSpecInfo(uint16_t code, uint32_t enterprise,
                     uint16_t sub_option_code, const std::string &str,
                     TMsg *parent);
...
protected:
  /// @brief utility function that appends sub-option with specified code and
  /// data
  ///
  ...
  void createSuboption(uint16_t sub_option_code, const char *data,
                       size_t data_len);
