...
TOptAuthentication::TOptAuthentication(char *buf, size_t buflen, TMsg *parent)
  ...
  switch (proto) {
    ...
    case AUTH_PROTO_DIBBLER:
      ...
      break;
    default: {
      Log(Warning) << "Auth: unsupported protocol type " << proto << " received in auth option."
                   << LogEnd;
      Valid = false;
      return;
    }
  ...
  switch (proto_) {
    ...
    case AUTH_PROTO_NONE:
      ...
      if (Parent) Parent->setAuthDigestPtr(buf, buflen);
      break;
    case AUTH_PROTO_DELAYED: {
      // We can have 2 cases here:
      // 1. this is SOLICIT and client sends no data, buflen should be zero
      // 2. this is something else, so the data format should follow
      // section 21.4.1
      ...
      // The next 4 bytes (RFC3315 never says that it's exactly 4 bytes. Nah,
      // every implementor will just guess the same size)
      if (Parent) Parent->setSPI(readUint32(buf));
      buf += sizeof(uint32_t);
      buflen -= sizeof(uint32_t);

      // The rest should be 16 bytes of of HMAC-MD5 data
      if (Parent) Parent->setAuthDigestPtr(buf,
                                           buflen);  // that should be the last 16 bytes.
      break;
    }
    case AUTH_PROTO_RECONFIGURE_KEY: {
      data_ = std::vector<uint8_t>(buf, buf + buflen);
      if (Parent) Parent->setAuthDigestPtr(buf + 1, buflen);
...
uint8_t TOptAuthentication::getAlgorithm() const { return algo_; }

AuthReplay TOptAuthentication::getRDM() const { return rdm_; }

TOptAuthentication::TOptAuthentication(AuthProtocols proto, uint8_t algo, AuthReplay rdm,
                                       TMsg *parent)
...
void TOptAuthentication::setRDM(AuthReplay value) { rdm_ = value; }
...
bool TOpt::doDuties() { return true; }
...
AuthProtocols TOptAuthentication::getProto() const { return proto_; }

void TOptAuthentication::setReplayDetection(uint64_t value) { replay_ = value; }

uint64_t TOptAuthentication::getReplayDetection() { return replay_; }

void TOptAuthentication::setPayload(const std::vector<uint8_t> &data) { data_ = data; }

void TOptAuthentication::getPayload(std::vector<uint8_t> &data) { data = data_; }

bool TOptAuthentication::doDuties() { return true; }
