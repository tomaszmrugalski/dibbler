...
TOptVendorSpecInfo::TOptVendorSpecInfo(uint16_t type, char * buf, int n, TMsg * parent) : TOpt(type, parent), Vendor_(0) {
    ...
    while (n >= 4) {
        ...
        n -= sizeof(uint16_t);
        if (optionLen > n) {
            Log(Warning) << "Malformed vendor-spec info option. Suboption " << optionCode << " truncated." << LogEnd;
            Valid = false;
            return;
        }
    ...
    if (n) {
        Log(Warning) << "Extra " << n << " bytes, after parsing suboption " << optionCode << " in vendor-spec info option."
                     << LogEnd;
        Valid = false;
        return;
    }
...
TOptVendorSpecInfo::TOptVendorSpecInfo(uint16_t code, uint32_t enterprise, uint16_t sub_option_code, const char * data,
                                       int dataLen, TMsg * parent)
    : TOpt(code, parent), Vendor_(enterprise) {
    createSuboption(sub_option_code, data, dataLen);
}

void TOptVendorSpecInfo::createSuboption(uint16_t sub_option_code, const char * data, size_t data_len) {
    if (sub_option_code) {
...
TOptVendorSpecInfo::TOptVendorSpecInfo(uint16_t code, uint32_t enterprise, uint16_t sub_option_code, SPtr<TIPv6Addr> addr,
                                       TMsg * parent)
    : TOpt(code, parent), Vendor_(enterprise) {
    createSuboption(sub_option_code, addr->getAddr(), 16);
}

TOptVendorSpecInfo::TOptVendorSpecInfo(uint16_t code, uint32_t enterprise, uint16_t sub_option_code, const std::string & str,
                                       TMsg * parent)
    : TOpt(code, parent), Vendor_(enterprise) {
