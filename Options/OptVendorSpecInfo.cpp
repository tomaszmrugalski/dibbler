...
TOptVendorSpecInfo::TOptVendorSpecInfo(uint16_t type, char *buf, int n, TMsg *parent)
  ...
  while (n >= 4) {
    ...
    n -= sizeof(uint16_t);
    if (optionLen > n) {
      Log(Warning) << "Malformed vendor-spec info option. Suboption " << optionCode << " truncated."
                   << LogEnd;
      Valid = false;
      return;
    }
...
bool TOptVendorSpecInfo::isValid() const { return true; }

uint32_t TOptVendorSpecInfo::getVendor() { return Vendor_; }
