...
TOptDomainLst::TOptDomainLst(int type, List(std::string) strLst, TMsg * parent) : TOpt(type, parent) {
    StringLst = strLst;
}

TOptDomainLst::TOptDomainLst(int type, const std::string & domain, TMsg * parent) : TOpt(type, parent) {
...
TOptDomainLst::TOptDomainLst(int type, const char * buf, unsigned short bufsize, TMsg * parent) : TOpt(type, parent) {
    int len;
    string domain = "";
    ...
    while (bufsize) {
        len = (char)*buf;
        if (len > bufsize) {
            Log(Debug) << "Option parsing failed. String length is specified as " << len << ", but remaining buffer is only "
                       << bufsize << " bytes long." << LogEnd;
            StringLst.clear();
            return;
        }
        if (len == 0) {
