...
int TOpt::getSubOptSize() {
  ...
  SubOptions.first();
  TOptPtr ptr;
  while (ptr = SubOptions.get()) size += ptr->getSize();
  return size;
}
...
void TOpt::firstOption() { SubOptions.first(); }

TOptPtr TOpt::getOption() { return SubOptions.get(); }

TOptPtr TOpt::getOption(int optType) {
  firstOption();
  TOptPtr opt;
  while (opt = getOption()) {
    if (opt->getOptType() == optType) return opt;
  }
  return TOptPtr();
}

void TOpt::addOption(TOptPtr opt) { SubOptions.append(opt); }

int TOpt::countOption() { return SubOptions.count(); }

void TOpt::setParent(TMsg *Parent) { this->Parent = Parent; }

void TOpt::delAllOptions() { SubOptions.clear(); }

bool TOpt::isValid() const { return Valid; }
...
std::string TOpt::getPlain() { return "[generic]"; }
...
/// @brief Parses options or suboptions, creates appropriate objects and store
/// them
...
/// @param placeId specifies location of the message (option number for option
/// parsing
...
bool TOpt::parseOptions(TOptContainer &options, const char *buf, size_t len, TMsg *parent,
  ...
  while (len > 0) {
    ...
    if (optLen > len) {
      Log(Warning) << "Truncated suboption " << optType << " in " << place << " " << placeId
                   << LogEnd;
      return false;
    }
