...
TOptFQDN::TOptFQDN(const char *buf, int bufsize, TMsg *parent) : TOpt(OPTION_FQDN, parent) {
  ...
  Log(Debug) << "FQDN: FQDN option received: fqdn name=" << (fqdn_.length() ? fqdn_ : "[empty]")
             << LogEnd;
}

TOptFQDN::~TOptFQDN() { return; }

void TOptFQDN::setNFlag(bool flag) { flag_N_ = flag; }

void TOptFQDN::setSFlag(bool flag) { flag_S_ = flag; }

void TOptFQDN::setOFlag(bool flag) { flag_O_ = flag; }

std::string TOptFQDN::getFQDN() const { return fqdn_; }
 ...
 * We also have to add 4 for the header (option type and size) and 1 for the
 * flags. 2 + 1 + 4 = 7
 *
...
size_t TOptFQDN::getSize() {
  if (fqdn_.length()) {
    // the final 0 should be present only for full fqdn, not for partial
    // hostname we distinguish between them by dot . presence
    ...
    // rfc4704 : A client MAY also leave the Domain Name field empty if it
    // desires the server to provide a name.
    return 5;
}
...
char *TOptFQDN::storeSelf(char *buffer) {
  ...
  *buffer = copy.length();  // copy.length()==0 => the data ends with dot. (full
                            // fqdn forced)
  buffer++;
  if (copy.empty()) {
    return buffer;
...
bool TOptFQDN::getNFlag() const { return flag_N_; }

bool TOptFQDN::getSFlag() const { return flag_S_; }

bool TOptFQDN::getOFlag() const { return flag_O_; }

bool TOptFQDN::doDuties() { return true; }
