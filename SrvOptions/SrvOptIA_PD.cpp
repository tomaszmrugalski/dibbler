...
TSrvOptIA_PD::TSrvOptIA_PD(uint32_t iaid, uint32_t t1, uint32_t t2, int Code, const std::string & Text, TMsg * parent)
 ...
 */
TSrvOptIA_PD::TSrvOptIA_PD(char * buf, int bufsize, TMsg * parent) : TOptIA_PD(buf, bufsize, parent), PDLength(0) {
    int pos = 0;
    ...
    while (pos < bufsize) {
        ...
        if ((code > 0) && (code <= 26)) // was 24
        {
            ...
            if (allowOptInOpt(parent->getType(), OPTION_IA_PD, code)) {
                ...
                Log(Warning) << "Illegal option received (type=" << code << ") in an IA_PD option." << LogEnd;
            }
        } else {
...
bool TSrvOptIA_PD::existingLease() {
    ...
    while (prefix = pd->getPrefix()) {
        ...
        SPtr<TOpt> optPrefix =
            new TSrvOptIAPrefix(prefix->get(), prefix->getLength(), prefix->getPref(), prefix->getValid(), this->Parent);
...
bool TSrvOptIA_PD::assignPrefix(SPtr<TSrvMsg> clientMsg, SPtr<TIPv6Addr> hint, bool fake) {
    ...
    while (prefix = prefixLst.get()) {
        ...
        SrvAddrMgr().addPrefix(this->ClntDuid, this->ClntAddr, cfgIface->getName(), Iface, IAID_, T1_, T2_, prefix, Prefered,
                               Valid, this->PDLength, false);
    ...
    SubOptions.append(new TOptStatusCode(STATUSCODE_NOPREFIXAVAIL, "Unable to provide any prefixes. Sorry.", this->Parent));
    return false;
}
...
TSrvOptIA_PD::TSrvOptIA_PD(SPtr<TSrvMsg> clientMsg, SPtr<TSrvOptIA_PD> queryOpt, TMsg * parent)
    ...
    case DECLINE_MSG:
        ...
        break;
    default: {
        Log(Warning) << "Unknown message type (" << msgType << "). Cannot generate OPTION_PD." << LogEnd;
 ...
 */
void TSrvOptIA_PD::solicitRequest(SPtr<TSrvMsg> clientMsg, SPtr<TSrvOptIA_PD> queryOpt, SPtr<TSrvCfgIface> ptrIface,
                                  bool fake) {
    ...
    if (!queryOpt->countPrefixes()) {
        ...
        SPtr<TSrvOptIAPrefix> hintPrefix = SPtr_cast<TSrvOptIAPrefix>(queryOpt->getOption(OPTION_IAPREFIX));
...
bool TSrvOptIA_PD::assignFixedLease(SPtr<TSrvOptIA_PD> req) {
    ...
    while (pool = iface->getPD()) {
        ...
        SrvAddrMgr().addPrefix(ClntDuid, this->ClntAddr, iface->getName(), Iface, IAID_, T1_, T2_, reservedPrefix, pref, valid,
                               ex->getPrefixLen(), false);
    ...
    Log(Info) << "Reserved out-of-pool address " << reservedPrefix->getPlain() << static_cast<unsigned int>(ex->getPrefixLen())
...
List(TIPv6Addr) TSrvOptIA_PD::getFreePrefixes(SPtr<TSrvMsg> clientMsg, SPtr<TIPv6Addr> cli_hint) {
    ...
    if (ptrPD->getAssignedCount() >= ptrPD->getTotalCount()) {
        Log(Error) << "PD: Unable to grant any prefixes: Already asigned " << ptrPD->getAssignedCount() << " out of "
                   << ptrPD->getTotalCount() << "." << LogEnd;
        return lst; // empty list
    }
    ...
    if (*anyaddr == *cli_hint) {
        Log(Debug) << "PD: Client requested unspecified (" << *cli_hint << ") prefix. Hint ignored." << LogEnd;
        validHint = false;
    }

    // is it multicast address (ff...)?
    if ((*(cli_hint->getAddr())) == 0xff) {
        Log(Debug) << "PD: Client requested multicast (" << *cli_hint << ") prefix. Hint ignored." << LogEnd;
        validHint = false;
    }
    ...
    if (!memcmp(cli_hint->getAddr(), linklocal, 2)) {
        Log(Debug) << "PD: Client requested link-local (" << *cli_hint << ") prefix. Hint ignored." << LogEnd;
        validHint = false;
    }
