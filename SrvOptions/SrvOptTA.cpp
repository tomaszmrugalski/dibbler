 ...
 */
TSrvOptTA::TSrvOptTA(char * buf, int bufsize, TMsg * parent) : TOptTA(buf, bufsize, parent), OrgMessage(parent->getType()) {
...
void TSrvOptTA::solicitRequest(SPtr<TSrvMsg> clientMsg, SPtr<TSrvOptTA> queryOpt, bool solicit) {
    ...
    if (willAssign > addrsMax - addrsAssigned) {
        Log(Notice) << "Client got " << addrsAssigned << " and requested " << willAssign << " more, but limit for a client is "
                    << addrsMax << LogEnd;
        willAssign = addrsMax - addrsAssigned;
    }

    if (willAssign > addrsAvail) {
        Log(Notice) << willAssign << " addrs " << (solicit ? "would" : "will") << " be assigned, but only" << addrsAssigned
                    << " is available." << LogEnd;
        willAssign = addrsAvail;
    }

    Log(Info) << "Client has " << addrsAssigned << " addrs, " << addrsAvail << " is available, limit for client is " << addrsMax
              << ", " << willAssign << " will be assigned." << LogEnd;
    if (!willAssign) {
        SubOptions.append(new TOptStatusCode(STATUSCODE_NOADDRSAVAIL, "Sorry, buddy. No temporary addresses for you", Parent));
    ...
    while (safety < SERVER_MAX_TA_RANDOM_TRIES) {
        ...
        if (SrvAddrMgr().taAddrIsFree(addr)) {
            if ((this->OrgMessage == REQUEST_MSG)) {
                ...
                SrvAddrMgr().addTAAddr(this->ClntDuid, this->ClntAddr, this->Iface, IAID_, addr, ta->getPref(), ta->getValid());
