    ...
    switch (code) {
      ...
      default:
        Log(Warning) << "Option " << code << "not supported "
                     << " in message (type=" << parent->getType() << ") in this version of server."
                     << LogEnd;
        break;
    }
    pos += length;
 ...
 * - REQUEST
 */
TSrvOptTA::TSrvOptTA(SPtr<TSrvOptTA> queryOpt, SPtr<TSrvMsg> clientMsg, int msgType, TMsg *parent)
  ...
  switch (msgType) {
    ...
    case CONFIRM_MSG:
      ...
      break;
    default:
      Log(Warning) << "Option TA in not supported in message " << MsgTypeToString(msgType) << "."
                   << LogEnd;
      return;
  }
}

TSrvOptTA::TSrvOptTA(int iaid, TMsg *parent) : TOptTA(iaid, parent) {}
...
void TSrvOptTA::solicitRequest(SPtr<TSrvMsg> clientMsg, SPtr<TSrvOptTA> queryOpt, bool solicit) {
  ...
  if (willAssign > addrsMax - addrsAssigned) {
    Log(Notice) << "Client got " << addrsAssigned << " and requested " << willAssign
                << " more, but limit for a client is " << addrsMax << LogEnd;
    willAssign = addrsMax - addrsAssigned;
  }

  if (willAssign > addrsAvail) {
    Log(Notice) << willAssign << " addrs " << (solicit ? "would" : "will")
                << " be assigned, but only" << addrsAssigned << " is available." << LogEnd;
    willAssign = addrsAvail;
  }

  Log(Info) << "Client has " << addrsAssigned << " addrs, " << addrsAvail
            << " is available, limit for client is " << addrsMax << ", " << willAssign
            << " will be assigned." << LogEnd;
  if (!willAssign) {
    SubOptions.append(new TOptStatusCode(STATUSCODE_NOADDRSAVAIL,
                                         "Sorry, buddy. No temporary addresses for you", Parent));
  ...
  if (!optAddr) {
    Log(Error) << "No temporary address found. Server is NOT configured with "
                  "TA option."
               << LogEnd;
    SPtr<TOptStatusCode> ptrStatus;
    ptrStatus = new TOptStatusCode(
        STATUSCODE_NOADDRSAVAIL,
...
SPtr<TSrvOptIAAddress> TSrvOptTA::assignAddr(SPtr<TSrvMsg> clientMsg) {
  ...
  while (ta = ptrIface->getTA()) {
    if (!ta->clntSupported(ClntDuid, ClntAddr, clientMsg)) continue;
    break;
  }
  ...
  while (safety < SERVER_MAX_TA_RANDOM_TRIES) {
    ...
    if (SrvAddrMgr().taAddrIsFree(addr)) {
      if ((this->OrgMessage == REQUEST_MSG)) {
        ...
        SrvAddrMgr().addTAAddr(this->ClntDuid, this->ClntAddr, this->Iface, IAID_, addr,
                               ta->getPref(), ta->getValid());
...
bool TSrvOptTA::doDuties() { return true; }
