...
/// @param parent Pointer to parent message.
TSrvOptIA_NA::TSrvOptIA_NA(long iaid, long t1, long t2, int code, const std::string &text,
                           TMsg *parent)
    : TOptIA_NA(iaid, t1, t2, parent), Iface(parent->getIface()) {
...
///
/// @param buf pointer to beginning of a buffer containing IA_NA (without type
/// and option fields)
  ...
  while (pos < bufsize) {
    ...
    if ((code > 0) && (code <= 24)) {
      if (allowOptInOpt(parent->getType(), OPTION_IA_NA, code)) {
        ...
        Log(Warning) << "Illegal option received (type=" << code << ") in an IA_NA option."
                     << LogEnd;
      }
    } else {
      Log(Warning) << "Unknown option received (type=" << code << ") in an IA_NA option." << LogEnd;
    };
    pos += length;
  }
}

/// This constructor is used to create IA option as an aswer to a SOLICIT,
/// SOLICIT (RAPID_COMMIT) or REQUEST.
///
/// @param queryOpt
/// @param queryMsg
/// @param parent
TSrvOptIA_NA::TSrvOptIA_NA(SPtr<TSrvOptIA_NA> queryOpt, SPtr<TSrvMsg> queryMsg, TMsg *parent)
  ...
  if (renew(queryOpt, false)) {
    Log(Info) << "Previous binding for client " << ClntDuid->getPlain()
              << ", IA(iaid=" << queryOpt->getIAID() << ") found and renewed." << LogEnd;
    return;
  }
  ...
  SubOptions.append(
      new TOptStatusCode(STATUSCODE_NOADDRSAVAIL, "No more addresses for you. Sorry.", Parent));
}
    ...
    switch (opt->getOptType()) {
      case OPTION_IAADDR: {
        ...
        if (candidate = getAddressHint(queryMsg, hint)) {
          if (assignAddr(candidate, optAddr->getPref(), optAddr->getValid(), quiet)) return true;
        }
        continue;
      }
      default:
...
///
/// This method may delete entry from cache if it finds out that entry is used
/// by someone else or is no longer valid (i.e. updated config has different
/// pool definitions). That is step 6 of lease assignment policy.
///
/// @param quiet should the assignment messages be logged (it shouldn't for
/// solicit)
///
  ...
  if (candidate = SrvAddrMgr().getCachedEntry(ClntDuid, IATYPE_IA)) {
    ...
    if (pool) {
      ...
      if (SrvAddrMgr().addrIsFree(candidate) && !SrvCfgMgr().addrReserved(candidate)) {
        if (assignAddr(candidate, pool->getPref(), pool->getValid(), quiet)) return true;
        // WTF? Address is free, buy we can't assign it?
        Log(Error) << "Failed to assign cached address that seems unused. Strange." << LogEnd;
        return false;
      }
      Log(Info) << "Unfortunately, " << candidate->getPlain() << " is already used or reserved."
                << LogEnd;
      SrvAddrMgr().delCachedEntry(candidate, IATYPE_IA);
      return false;
    } else {
      Log(Warning) << "Cache: Cached address " << *candidate << " found, but it is no longer valid."
                   << LogEnd;
      SrvAddrMgr().delCachedEntry(candidate, IATYPE_IA);
      return false;
    }  // else
...
///
/// @param req client's IA_NA (used for trying to assign as close
/// T1,T2,pref,valid values as possible)
///
/// @return true, if assignment was successful, false if there are no
/// fixed-leases reserved
bool TSrvOptIA_NA::assignFixedLease(SPtr<TSrvOptIA_NA> req, bool quiet) {
  ...
  // we've got fixed address, yay! Let's try to calculate its preferred and
  // valid lifetimes
  ...
  while (pool = iface->getAddrClass()) {
    ...
    if (!pool->addrInPool(reservedAddr)) continue;
    ...
    SrvAddrMgr().addClntAddr(ClntDuid, ClntAddr, Iface, IAID_, T1_, T2_, reservedAddr, pref, valid,
                             quiet);
...
bool TSrvOptIA_NA::assignAddr(SPtr<TIPv6Addr> addr, uint32_t pref, uint32_t valid, bool quiet) {
  ...
  if (ptrClass) {
    ...
    // Class not found. This is out-of-pool assignment. The address does not
    // belong to any specified pool, so we need to pick the values from the
  ...
  Log(Info) << "Client " << ClntDuid->getPlain() << " got " << *addr << " (IAID=" << IAID_
            << ", pref=" << pref << ",valid=" << valid << ")." << LogEnd;
...
SPtr<TIPv6Addr> TSrvOptIA_NA::getExceptionAddr() {
  ...
  SPtr<TSrvCfgOptions> ex = ptrIface->getClientException(ClntDuid, Parent, false
                                                         /* false = verbose */);
  if (ex) return ex->getAddr();

  return SPtr<TIPv6Addr>();
}

// constructor used only in RENEW, REBIND, DECLINE and RELEASE
TSrvOptIA_NA::TSrvOptIA_NA(SPtr<TSrvOptIA_NA> queryOpt, SPtr<TIPv6Addr> clntAddr,
                           SPtr<TDUID> clntDuid, int iface, unsigned long &addrCount, int msgType,
                           TMsg *parent)
  ...
  switch (msgType) {
    case SOLICIT_MSG:
      // this->solicit(cfgMgr, addrMgr, queryOpt, clntAddr, clntDUID,iface,
      // addrCount);
      break;
    case REQUEST_MSG:
      // this->request(cfgMgr, addrMgr, queryOpt, clntAddr, clntDUID, iface,
      // addrCount);
      break;
    case RENEW_MSG:
    ...
    case DECLINE_MSG:
      ...
      break;
    default: {
      Log(Warning) << "Unknown message type (" << msgType << "). Cannot generate OPTION_IA_NA."
                   << LogEnd;
      SubOptions.append(
 ...
 * @param complainIfMissing specifies if potential warnings should be printed or
 * not
 *
...
bool TSrvOptIA_NA::renew(SPtr<TSrvOptIA_NA> queryOpt, bool complainIfMissing) {
  ...
  if (!ptrClient) {
    if (complainIfMissing) {
      ...
      Log(Info) << "Unable to RENEW binding for IA(iaid=" << queryOpt->getIAID()
                << ", client=" << ClntDuid->getPlain() << ": No such client." << LogEnd;
    }
    return false;
  }
  ...
  if (!ptrIA) {
    if (complainIfMissing) {
      ...
      Log(Info) << "Unable to RENEW binding for IA(iaid=" << queryOpt->getIAID()
                << ", client=" << ClntDuid->getPlain() << ": No such IA." << LogEnd;
    }
    return false;
  }
  ...
  while (ptrAddr = ptrIA->getAddr()) {
    ...
    optAddr =
        new TSrvOptIAAddress(ptrAddr->get(), ptrAddr->getPref(), ptrAddr->getValid(), this->Parent);
    SubOptions.append(SPtr_cast<TOpt>(optAddr));
  }
  ...
  ptrStatus = new TOptStatusCode(STATUSCODE_SUCCESS,
                                 "Address(es) renewed. Greetings from planet Earth", this->Parent);
  SubOptions.append(SPtr_cast<TOpt>(ptrStatus));

  return true;
}

void TSrvOptIA_NA::rebind(SPtr<TSrvOptIA_NA> queryOpt, unsigned long &addrCount) {
  // find that client in addrdb
  ...
  // finally send greetings and happy OK status code
  SPtr<TOptStatusCode> ptrStatus;
  ptrStatus = new TOptStatusCode(STATUSCODE_SUCCESS, "Greetings from planet Earth", this->Parent);
  SubOptions.append(SPtr_cast<TOpt>(ptrStatus));
}

void TSrvOptIA_NA::release(SPtr<TSrvOptIA_NA> queryOpt, unsigned long &addrCount) {}

void TSrvOptIA_NA::decline(SPtr<TSrvOptIA_NA> queryOpt, unsigned long &addrCount) {}

bool TSrvOptIA_NA::doDuties() { return true; }
...
SPtr<TIPv6Addr> TSrvOptIA_NA::getAddressHint(SPtr<TSrvMsg> clientReq, SPtr<TIPv6Addr> hint) {
  ...
  if (!ptrClass) return SPtr<TIPv6Addr>();  // NULL

  if (!ptrClass->clntSupported(ClntDuid, ClntAddr, clientReq)) return SPtr<TIPv6Addr>();  // NULL

  // If the Class is valid and support the Client (based on duid, addr,
  // clientclass)
  ...
  if (assignedCnt >= classMaxLease) {
    Log(Debug) << "Requested address (" << *hint
               << ") belongs to supported class, which has reached its limit (" << assignedCnt
               << " assigned, " << classMaxLease << " max lease)." << LogEnd;
    // this class is useless
  } else {
    Log(Debug) << "Requested address (" << *hint << ") belongs to supported class, but is used."
               << LogEnd;
    do {
...
bool TSrvOptIA_NA::assignRandomAddr(SPtr<TSrvMsg> queryMsg, bool quiet) {
  ...
  if (!pool) {
    Log(Warning) << "Unable to find any suitable (allowed, non-full) class for "
                    "this client."
                 << LogEnd;
    return 0;
  }
  ...
  if (pool->clntSupported(ClntDuid, ClntAddr, queryMsg) &&
    ...
    while (safety < SERVER_MAX_IA_RANDOM_TRIES) {
      ...
      if (SrvAddrMgr().addrIsFree(candidate) && !SrvCfgMgr().addrReserved(candidate)) break;
    ...
    if (safety < SERVER_MAX_IA_RANDOM_TRIES) {
      ...
      Log(Error) << "Unable to randomly choose address after " << SERVER_MAX_IA_RANDOM_TRIES
                 << " tries." << LogEnd;
      return false;
    }
  }
...
bool TSrvOptIA_NA::assignSequentialAddr(SPtr<TSrvMsg> clientMsg, bool quiet) {
  // Random pool failed. That really should not happen. We are most probably not
  // supporting this client or are completely out of leases. Last chance:
  // iterate over all pools and find suitable lease:
  SPtr<TSrvCfgIface> ptrIface = SrvCfgMgr().getIfaceByID(Iface);
  if (!ptrIface) {
    return 0;
  }
  ...
  while (pool = ptrIface->getAddrClass()) {
    if (!pool->clntSupported(ClntDuid, ClntAddr, clientMsg)) continue;
    if (pool->getAssignedCount() >= pool->getClassMaxLease()) continue;
    break;

    SPtr<TIPv6Addr> candidate = pool->getFirstAddr();
    for (candidate = pool->getFirstAddr(); candidate != pool->getLastAddr(); ++(*candidate)) {
      if (!SrvAddrMgr().addrIsFree(candidate)) continue;
      if (assignAddr(candidate, pool->getPref(), pool->getValid(), quiet)) return true;
    }
  }
