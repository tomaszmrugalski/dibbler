...
TSrvOptIAAddress::TSrvOptIAAddress(char *buf, int bufsize, TMsg *parent)
  ...
  while (pos < bufsize) {
    ...
    if ((code > 0) && (code <= 24)) {
      ...
      Log(Warning) << "Unknown option in option IAADDR( optType=" << code << "). Option ignored."
                   << LogEnd;
    };
    pos += length;
  }
...
TSrvOptIAAddress::TSrvOptIAAddress(SPtr<TIPv6Addr> addr, unsigned long pref, unsigned long valid,
                                   TMsg *parent)
    : TOptIAAddress(addr, pref, valid, parent) {}

bool TSrvOptIAAddress::doDuties() { return true; }
