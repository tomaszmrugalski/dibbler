...
bool parseCmdLine(ReqCfgMgr *a, int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strncmp(argv[i], "-addr", 5)) {
      if (argc == i) {
        Log(Error) << "Unable to parse command-line. -addr used, but actual "
                      "address is missing."
                   << LogEnd;
        return false;
      }
    ...
    if (!strncmp(argv[i], "-duid", 5)) {
      if (argc == i) {
        Log(Error) << "Unable to parse command-line. -duid used, but actual "
                      "DUID is missing."
                   << LogEnd;
        return false;
      }
    ...
    if (!strncmp(argv[i], "-i", 2)) {
      if (argc == i) {
        Log(Error) << "Unable to parse command-line. -i used, but actual "
    ...
    if (!strncmp(argv[i], "-timeout", 8)) {
      if (argc == i) {
        Log(Error) << "Unable to parse command-line. -timeout used, but actual "
    ...
    if (!strncmp(argv[i], "-dstaddr", 7)) {
      if (argc == i) {
        Log(Error) << "Unable to parse command-line. -dstaddr used, but actual "
    ...
    if (!strncmp(argv[i], "--help", 5) || !strncmp(argv[i], "-h", 5) ||
        !strncmp(argv[i], "/help", 5) || !strncmp(argv[i], "-?", 2) || !strncmp(argv[i], "/?", 2)) {
      return false;
    }
