...
ReqTransMgr::ReqTransMgr(TIfaceMgr * ifaceMgr) : CfgMgr(NULL) {
    IfaceMgr = ifaceMgr;
}
...
bool ReqTransMgr::SendMsg() {
    ...
    Log(Debug) << "Transmitting data on the " << Iface->getFullName() << " interface to " << dstAddr->getPlain() << " address."
               << LogEnd;
...
bool ReqTransMgr::ParseOpts(int msgType, int recurseLevel, char * buf, int bufLen) {
    ...
    while (pos < bufLen) {
        ...
        if (pos + length > bufLen) {
            Log(Error) << linePrefix << "Truncated option (type=" << code << ", len=" << length << " received in message << "
                       << msgType << ". Option ignored." << LogEnd;
            pos += length;
            continue;
        }
