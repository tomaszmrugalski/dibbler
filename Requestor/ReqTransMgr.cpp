...
ReqTransMgr::ReqTransMgr(TIfaceMgr *ifaceMgr) : CfgMgr(NULL) { IfaceMgr = ifaceMgr; }

void ReqTransMgr::SetParams(ReqCfgMgr *cfgMgr) { CfgMgr = cfgMgr; }
...
bool ReqTransMgr::BindSockets() {
  ...
  Log(Debug) << "Socket " << Socket->getFD() << " created on the " << loopback->getFullName()
             << " interface." << LogEnd;
  ...
  Log(Debug) << "Socket " << Socket->getFD() << " created on the " << Iface->getFullName()
             << " interface." << LogEnd;
...
bool ReqTransMgr::SendMsg() {
  ...
  if (CfgMgr->addr) {
    Log(Debug) << "Creating ADDRESS-based query. Asking for " << CfgMgr->addr << " address."
               << LogEnd;
    // Address based query
...
bool ReqTransMgr::ParseOpts(int msgType, int recurseLevel, char *buf, int bufLen) {
  std::ostringstream o;
  for (int i = 0; i < recurseLevel; i++) o << "  ";
  ...
  while (pos < bufLen) {
    if (pos + 4 > bufLen) {
      Log(Error) << linePrefix << "Message " << msgType << " truncated. There are "
                 << (bufLen - pos) << " bytes left to parse. Bytes ignored." << LogEnd;
      return false;
    }
    ...
    if (!allowOptInMsg(msgType, code)) {
      Log(Warning) << linePrefix << "Invalid option received: Option " << code
                   << " not allowed in message type " << msgType << ". Ignored." << LogEnd;
      pos += length;
      continue;
    }
    if (!recurseLevel && !allowOptInOpt(msgType, 0, code)) {
      Log(Warning) << "Invalid option received: Option " << code << " not allowed in message type "
                   << msgType << " as a base option (as suboption only permitted). Ignored."
                   << LogEnd;
      pos += length;
      continue;
    }
    ...
    switch (code) {
      ...
      case OPTION_CLIENT_DATA:
        ...
        Log(Info) << linePrefix << "Option " << name << "(code=" << code << "), len=" << length
                  << LogEnd;
