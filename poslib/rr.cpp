...
int rr_len(char prop, message_buff &buff, int ix, int len) {
  ...
  switch (prop) {
    ...
    case '7':                         /* ipv6 address + prefix */
      ...
      if (buff.msg[ix] != 0) /* domain name nessecary */
...
void rr_read(u_int16 RRTYPE, unsigned char *&RDATA, uint16_t &RDLEN, message_buff &buff, int ix,
             int len) {
...
void rr_write(u_int16 RRTYPE, unsigned char *RDATA, uint16_t RDLEN, stl_string &dnsmessage,
...
stl_string rr_property_to_string(char type, const unsigned char *&RDATA, int RDLENGTH,
                                 domainname &zone) {
  char buff[128];
  ...
  switch (type) {
    ...
    case 'h':  // character strings
      ...
      return ret;
    case '6':
      sprintf(buff, "%x:%x:%x:%x:%x:%x:%x:%x", RDATA[0] * 256 + RDATA[1], RDATA[2] * 256 + RDATA[3],
              RDATA[4] * 256 + RDATA[5], RDATA[6] * 256 + RDATA[7], RDATA[8] * 256 + RDATA[9],
              RDATA[10] * 256 + RDATA[11], RDATA[12] * 256 + RDATA[13],
              RDATA[14] * 256 + RDATA[15]);
      RDATA += 16;
      return buff;
    case 'w':  // well-known services
...
stl_string rr_torelstring(u_int16 RRTYPE, const unsigned char *_RDATA, int RDLENGTH,
                          domainname zone) {
  rr_type *info = rrtype_getinfo(RRTYPE);
  char *ptr;
