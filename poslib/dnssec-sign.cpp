...
void verify_signature(DnsRR *check_tsig, DnsRR *message_tsig, stl_string key,
  ...
  if (!message_tsig)
    throw PException(true, "Unsigned answer to a signed message (key %s)",
                     check_tsig->NAME.tocstr());
  ...
  if (rr_alg_name != alg_name || check_tsig->NAME != message_tsig->NAME) {
    ...
    throw PException(true,
                     "Key name/algorithm does not match: question signed with %s, answer "
                     "signed with %s",
                     check_tsig->NAME.tocstr(), message_tsig->NAME.tocstr());
  }

  unsigned char *rr_macpos = rr_getdata(message_tsig->RDATA, DNS_TYPE_TSIG, 3),
                *macpos = rr_getdata(check_tsig->RDATA, DNS_TYPE_TSIG, 3);
  uint16_t rr_maclen = uint16_value(rr_macpos), maclen = uint16_value(macpos);

  if ((message.msg[3] & 15) == RCODE_NOTAUTH) {
    /* error! */
    uint16_t err = rr_getshort(message_tsig->RDATA, DNS_TYPE_TSIG, 5);
    if (err == RCODE_BADSIG)
      throw PException(true, "Question signed with %s had bad signature",
                       check_tsig->NAME.tocstr());
      ...
      throw PException(true, "Unknown sign error: %d", err);
  }
  ...
  if (((clienttime > servertime) && (clienttime - servertime > fudge)) ||
    ...
    throw PException(true, "Answer sign time invalid (answer time %d, real time %d)",
                     uint48_value(rr_getdata(message_tsig->RDATA, DNS_TYPE_TSIG, 1)), clienttime);
  ...
  if (maclen) ext = message_buff(macpos, maclen + 2);
...
// void hmac_md5_update (struct hmac_md5_ctx *md5, int len, const unsigned char*
// ptr) {
//  print_buff (len, ptr);
...
stl_string base64_encode(const char *buff, int bufflen) {
  ...
  len = base64_encode_update(&str, (uint8_t *)res, len,
                             (uint8_t *)buff);  // TODO: args?
...
void tsig_from_string(DnsRR *&tsig_rr, stl_string &sign_key, const char *keystring) {
  ...
  if (!ptr)
    throw PException(true, "%s is not a valid key string (should be keyname:key)", keystring);
