...
TClntAddrMgr &TClntAddrMgr::instance() { return *Instance; }
 ...
 *
 */
TClntAddrMgr::TClntAddrMgr(SPtr<TDUID> clientDUID, bool useConfirm, const std::string &xmlFile,
                           bool loadDB)
...
unsigned long TClntAddrMgr::getT1Timeout() { return Client->getT1Timeout(); }
unsigned long TClntAddrMgr::getT2Timeout() { return Client->getT2Timeout(); }
unsigned long TClntAddrMgr::getPrefTimeout() { return Client->getPrefTimeout(); }
unsigned long TClntAddrMgr::getValidTimeout() { return Client->getValidTimeout(); }

unsigned long TClntAddrMgr::getTimeout() {
  unsigned long val, val2;
  val = this->getT1Timeout();
  if ((val2 = this->getT2Timeout()) < val) val = val2;
  ...
  //  val = val2;
  if ((val2 = this->getValidTimeout()) < val) val = val2;
  return val;
}
...
void TClntAddrMgr::firstIA() { Client->firstIA(); }

SPtr<TAddrIA> TClntAddrMgr::getIA() { return Client->getIA(); }

bool TClntAddrMgr::delIA(long IAID) { return Client->delIA(IAID); }

void TClntAddrMgr::addIA(SPtr<TAddrIA> ptr) { Client->addIA(ptr); }

int TClntAddrMgr::countIA() { return Client->countIA(); }
...
void TClntAddrMgr::setIA2Confirm(volatile link_state_notify_t *changedLinks) {
  ...
  while (ptrIA = this->getIA()) {
    ...
    for (int i = 0; i < MAX_LINK_STATE_CHANGES_AT_ONCE; i++)
      if (changedLinks->ifindex[i] == ifindex) found = true;
...
void TClntAddrMgr::firstPD() { Client->firstPD(); }

SPtr<TAddrIA> TClntAddrMgr::getPD() { return Client->getPD(); }

bool TClntAddrMgr::delPD(long IAID) { return Client->delPD(IAID); }

void TClntAddrMgr::addPD(SPtr<TAddrIA> ptr) { Client->addPD(ptr); }

int TClntAddrMgr::countPD() { return Client->countPD(); }
...
void TClntAddrMgr::firstTA() { Client->firstTA(); }

SPtr<TAddrIA> TClntAddrMgr::getTA() { return Client->getTA(); }
...
void TClntAddrMgr::addTA(SPtr<TAddrIA> ptr) { Client->addTA(ptr); }

bool TClntAddrMgr::delTA(unsigned long iaid) { return Client->delTA(iaid); }

int TClntAddrMgr::countTA() { return Client->countTA(); }
...
bool TClntAddrMgr::addPrefix(SPtr<TDUID> srvDuid, SPtr<TIPv6Addr> srvAddr,
                             const std::string &ifacename, int iface, unsigned long IAID,
  ...
  return TAddrMgr::addPrefix(ptrClient, srvDuid, srvAddr, ifacename, iface, IAID, T1, T2, prefix,
                             pref, valid, length, quiet);
}

bool TClntAddrMgr::updatePrefix(SPtr<TDUID> srvDuid, SPtr<TIPv6Addr> srvAddr,
                                const std::string &ifname, int iface, unsigned long IAID,
                                unsigned long T1, unsigned long T2, SPtr<TIPv6Addr> prefix,
                                unsigned long pref, unsigned long valid, int length, bool quiet) {
  if (!prefix) {
  ...
  return TAddrMgr::updatePrefix(ptrClient, srvDuid, srvAddr, iface, IAID, T1, T2, prefix, pref,
                                valid, length, quiet);
}
