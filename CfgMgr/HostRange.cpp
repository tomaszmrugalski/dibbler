...
bool THostRange::in(SPtr<TDUID> duid, SPtr<TIPv6Addr> addr) const {
    if (isAddrRange_) {
        if ((*addr <= *AddrL_) && (*AddrL_ <= *addr))
            return true;
        if ((*addr <= *AddrR_) && (*AddrR_ <= *addr))
            return true;
        if (*addr <= *AddrL_)
            return false;
        if (*AddrR_ <= *addr)
...
std::ostream & operator<<(std::ostream & out, THostRange & range) {
    if (range.isAddrRange_) {
        ...
        if (range.AddrL_ && range.AddrR_)
            out << "      <range type=\"addr\" min=\"" << *range.AddrL_ << "\" max=\"" << *range.AddrR_ << "\"/>" << std::endl;
    } else {
        // DUID range
        if (range.DUIDL_ && range.DUIDR_) {
            out << "      <range type=\"duid\">" << std::endl
                << "         " << *range.DUIDL_ << "         " << *range.DUIDR_ << "      </range>" << std::endl;
        }
    }
    return out;
}
