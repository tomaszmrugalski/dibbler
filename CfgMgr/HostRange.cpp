  ...
  if (isAddrRange_) {
    ...
    *diff = (*AddrR_) - (*AddrL_);
    ...
    unsigned long retVal =
        addr[12] * 256 * 256 * 256 + addr[13] * 256 * 256 + addr[14] * 256 + addr[15];
    if (retVal != DHCPV6_INFINITY) return retVal + 1;
    return retVal;
  } else
    return 0;
}

int THostRange::getPrefixLength() const { return PrefixLength_; }

void THostRange::setPrefixLength(int len) { PrefixLength_ = len; }

THostRange::~THostRange(void) {}

SPtr<TIPv6Addr> THostRange::getAddrL() const { return AddrL_; }

SPtr<TIPv6Addr> THostRange::getAddrR() const { return AddrR_; }
...
std::ostream &operator<<(std::ostream &out, THostRange &range) {
  if (range.isAddrRange_) {
    ...
    if (range.AddrL_ && range.AddrR_)
      out << "      <range type=\"addr\" min=\"" << *range.AddrL_ << "\" max=\"" << *range.AddrR_
          << "\"/>" << std::endl;
  } else {
    // DUID range
    if (range.DUIDL_ && range.DUIDR_) {
      out << "      <range type=\"duid\">" << std::endl
          << "         " << *range.DUIDL_ << "         " << *range.DUIDR_ << "      </range>"
          << std::endl;
    }
  }
  return out;
