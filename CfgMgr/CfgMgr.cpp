...
TCfgMgr::TCfgMgr()
    : LogLevel(DEFAULT_LOGLEVEL), IsDone(false), DUIDType(DUID_TYPE_LLT),                         /* default DUID type: LLT */
      DUIDEnterpriseNumber(-1), DdnsProto(DNSUPDATE_TCP), DDNSTimeout_(DNSUPDATE_DEFAULT_TIMEOUT) // default is 1000 ms

#ifndef MOD_DISABLE_AUTH
      // Authentication
      ,
      AuthProtocol_(AUTH_PROTO_NONE), AuthAlgorithm_(0), AuthReplay_(AUTH_REPLAY_NONE),
...
void TCfgMgr::copyFile(const std::string & cfgFile, const std::string & oldCfgFile) {
    ...
    newF.open(cfgFile.c_str(), ios::in | ios::binary);
    if (newF.fail())
        Log(Error) << "Can't open file " << cfgFile << ", when trying to save new config file." << LogEnd;
    else {
        newF.seekg(0, ios::end);
        ...
        if (oOldF.fail()) {
            Log(Error) << "Can't open file " << oldCfgFile << ", when trying to save new config file" << LogEnd;
            newF.close();
        } else
...
bool TCfgMgr::setDUID(const std::string & filename, TIfaceMgr & ifaceMgr) {
    ...
    while ((!found) && (realIface = ifaceMgr.getIface())) {
        ...
        if (!realIface->getMac()) {
            Log(Debug) << "DUID creation: Interface " << realIface->getFullName() << " skipped: no link addresses." << LogEnd;
            continue;
        }
        if (realIface->getMacLen() < 6) {
            Log(Debug) << "DUID creation: Interface " << realIface->getFullName() << " skipped: MAC length is "
                       << realIface->getMacLen() << ", but at least 6 is required." << LogEnd;
            continue;
        }
        if (realIface->flagLoopback()) {
            Log(Debug) << "DUID creation: Interface " << realIface->getFullName() << " skipped: Interface is loopback."
                       << LogEnd;
            continue;
        }
        if (!memcmp(realIface->getMac(), buf, realIface->getMacLen())) {
            Log(Debug) << "DUID creation: Interface " << realIface->getFullName() << " skipped: MAC is all zero. " << LogEnd;
            continue;
        }
        if (!realIface->flagUp()) {
            Log(Debug) << "DUID creation: Interface " << realIface->getFullName() << " skipped: Interface is down." << LogEnd;
            continue;
        }
    ...
    if (found) {
        if (this->generateDUID(filename, realIface->getMac(), realIface->getMacLen(), realIface->getHardwareType())) {
            if (this->DUIDType != DUID_TYPE_EN)
                Log(Notice) << "DUID creation: generated using " << realIface->getFullName() << " interface." << LogEnd;
            Log(Info) << "My DUID is " << this->DUID->getPlain() << "." << LogEnd;
            return true;
        } else {
            Log(Crit) << "DUID creation: generation attempt based on " << realIface->getFullName() << " interface failed."
                      << LogEnd;
            return false;
        }
    }
