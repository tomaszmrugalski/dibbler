...
void TCfgMgr::setDDNSProtocol(DNSUpdateProtocol proto) { DdnsProto = proto; }
  ...
  if (!newF.fail()) {
    if (!oldF.fail()) {
      ...
      if ((newF.tellg()) != oldF.tellg())
        ...
        while ((!newF.eof()) && (!oldF.eof()) && (!newConf)) newConf = (newF.get() != oldF.get());
      }
      newF.close();
      oldF.close();
    } else {
    ...
    Log(Error) << "Can't open file " << cfgFile << ". Config comparision aborted." << LogEnd;
  }
  return newConf;
}
...
void TCfgMgr::copyFile(const std::string &cfgFile, const std::string &oldCfgFile) {
  ...
  newF.open(cfgFile.c_str(), ios::in | ios::binary);
  if (newF.fail())
    Log(Error) << "Can't open file " << cfgFile << ", when trying to save new config file."
               << LogEnd;
  else {
    newF.seekg(0, ios::end);
    ...
    if (oOldF.fail()) {
      Log(Error) << "Can't open file " << oldCfgFile << ", when trying to save new config file"
                 << LogEnd;
      newF.close();
    } else
      // if both opened try to copy
      while (newLen-- > 0) oOldF.put(newF.get());
  }
  newF.close();
  oOldF.close();
}
 ...
 *
 * This function also checks if DUID value exist and checks the correctness of
 * this file.
 *
...
bool TCfgMgr::loadDUID(const std::string &duidFile) {
  ...
  Log(Debug) << "DUID's value = " << DUID->getPlain() << " was loaded from " << duidFile << " file."
             << LogEnd;
...
bool TCfgMgr::setDUID(const std::string &filename, TIfaceMgr &ifaceMgr) {
  ...
  if (found) {
    if (this->generateDUID(filename, realIface->getMac(), realIface->getMacLen(),
                           realIface->getHardwareType())) {
      if (this->DUIDType != DUID_TYPE_EN)
      ...
      Log(Crit) << "DUID creation: generation attempt based on " << realIface->getFullName()
                << " interface failed." << LogEnd;
      return false;
    }
  }

  Log(Crit) << "Cannot generate DUID, because there is no up and running "
               "interface with "
...
bool TCfgMgr::generateDUID(const std::string &duidFile, char *mac, int macLen, int macType) {
  ...
  switch (this->DUIDType) {
    case DUID_TYPE_LLT:
      ...
      */
      ...
      for (int i = 0; i < macLen; i++) DUID[i + 8] = mac[i];
      break;
    case DUID_TYPE_LL:
      ...
      writeUint16(DUID, this->DUIDType);
      writeUint16(DUID + 2, macType);
      for (int i = 0; i < macLen; i++) DUID[i + 4] = mac[i];
      break;
    case DUID_TYPE_EN:
  ...
  Log(Notice) << "DUID creation: Generating " << DUIDlen << "-bytes long " << duidType << " DUID."
              << LogEnd;
...
void TCfgMgr::setWorkdir(std::string workdir) { Workdir = workdir; }

string TCfgMgr::getWorkDir() { return Workdir; }

string TCfgMgr::getLogName() { return LogName; }

int TCfgMgr::getLogLevel() { return LogLevel; }

SPtr<TDUID> TCfgMgr::getDUID() { return DUID; }

#if !defined(MOD_SRV_DISABLE_DNSUPDATE) && !defined(MOD_CLNT_DISABLE_DNSUPDATE)
void TCfgMgr::addKey(SPtr<TSIGKey> key) { Keys_.push_back(key); }

SPtr<TSIGKey> TCfgMgr::getKey() {
  /// @todo: add some parameter that will pick the right key

  if (Keys_.empty()) return SPtr<TSIGKey>();
...
AuthProtocols TCfgMgr::getAuthProtocol() { return AuthProtocol_; }

AuthReplay TCfgMgr::getAuthReplay() { return AuthReplay_; }

uint8_t TCfgMgr::getAuthAlgorithm() { return AuthAlgorithm_; }

void TCfgMgr::setAuthDropUnauthenticated(bool drop) { AuthDropUnauthenticated_ = drop; }

bool TCfgMgr::getAuthDropUnauthenticated() { return AuthDropUnauthenticated_; }
...
std::string TCfgMgr::getAuthRealm() { return AuthRealm_; }
#endif
